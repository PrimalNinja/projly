...
// add the document components with this where the component is to be copied from an existing file, the componentid (just a suffix) is optional and should not be provided for the entry point to the document
function documentComponentCopy($objConn_a, $strDocumentID_a, $strFilename_a, $strSourcePath_a)
{
    if (dependencies('docs/documentClusterCopy') &&
		...
		dependencies('entity/dataaccess/documentrepository')) 
	{
		...
		if (strlen($strDocumentRepositoryID) > 0)
		{
			...
			$intFilesize = documentClusterCopy($strFilenameBase, $strFilenameBase . '-' . $strFilename_a, $strSourcePath_a, $strDocumentRoot);
