...
function actionMessageArchiveList($objConn_a, $strSecurityToken_a, $strDataID_a, $arrParameters_a) {
...
from (
    select distinct m.id, m.fromclient_id, m.fromuser_id, m.toclient_id, m.touser_id, fc.description from_clientname, fu.description from_username, tc.description to_clientname, tu.description to_username, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_subject subject, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_sentdatetime sentdatetime, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_isunread is_unread
    from ~TABLENAMEINTERNALMESSAGE~ m, ~TABLENAMEUSER~ tu, ~TABLENAMECLIENT~ tc, ~TABLENAMEUSER~ fu, ~TABLENAMECLIENT~ fc
    where m.touser_id = tu.id and m.toclient_id = tu.client_id and m.toclient_id = tc.id and m.fromuser_id = fu.id and m.fromclient_id = fu.client_id and m.fromclient_id = fc.id and m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_isarchived = 'Y' and m.touser_id = ~USERID~ and m.toclient_id = ~CLIENTID~ and m.client_id = ~CLIENTID~
...
select count(*) returnvalue
from (
    select distinct m.id, m.fromclient_id, m.fromuser_id, m.toclient_id, m.touser_id, fc.description from_clientname, fu.description from_username, tc.description to_clientname, tu.description to_username
    from ~TABLENAMEINTERNALMESSAGE~ m, ~TABLENAMEUSER~ tu, ~TABLENAMECLIENT~ tc, ~TABLENAMEUSER~ fu, ~TABLENAMECLIENT~ fc
    ...
    // fetch
    $strSQL = "
    select distinct m.id, m.fromclient_id, m.fromuser_id, m.toclient_id, m.touser_id, m.jsondata, fc.description from_clientname, fu.description from_username, tc.description to_clientname, tu.description to_username, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_subject subject, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_sentdatetime, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_isunread is_unread, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_isarchived is_archived, m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_isflagged is_flagged
    from ~TABLENAMEINTERNALMESSAGE~ m, ~TABLENAMEUSER~ tu, ~TABLENAMECLIENT~ tc, ~TABLENAMEUSER~ fu, ~TABLENAMECLIENT~ fc
    where m.touser_id = tu.id and m.toclient_id = tu.client_id and m.toclient_id = tc.id and m.fromuser_id = fu.id and m.fromclient_id = fu.client_id and m.fromclient_id = fc.id and m.g8938a0d4_8efc_41ed_a420_4af7b808b3f0_isarchived = 'Y' and m.touser_id = ~USERID~ and m.toclient_id = ~CLIENTID~ and m.client_id = ~CLIENTID~
" . dbBuildWhere('and', $arrFields, $arrFilter) . dbBuildOrderBy($arrFields, $arrOrder) . " limit ~OFFSET~,~LIMIT~"; // . NONAJAXGRIDLIMIT;
    ...
    while ($arrRow = dbReadRecord($objResult)) 
	{
        ...
        $arrResult[] = array(
            "id" => secureEntityValue('INTERNALMESSAGE', $arrRow['id']), //secureValue(MSG_MESSAGE, $arrRow['id']),
            "rownum" => $intRowNum,
