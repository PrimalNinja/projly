...
function actionMessageArchiveSet($objConn_a, $strSecurityToken_a, $strDataID_a, $arrParameters_a) { 
    ...
    $strIsArchived = getJSONParameter($arrParameters_a, 'isarchived');
    ...
    if ($strIsArchived == 'Y' || $strIsArchived == 'N') { 
        ...
        $arrJSONData = formValueUpdateBySectionCodeFieldCode($arrJSONData, 'g8938a0d4-8efc-41ed-a420-4af7b808b3f0', 'ISARCHIVED', $strIsArchived);
        ...
        /*
        $strSQL = "update ~TABLENAMEINTERNALMESSAGE~ set g8938a0d4_8efc_41ed_a420_4af7b808b3f0_isarchived = '~ISARCHIVED~' where client_id = ~CLIENTID~ and id = ~MESSAGEID~";
        $strSQL = str_replace('~TABLENAMEINTERNALMESSAGE~', ff($strTableNameInternalMessage), $strSQL);
        $strSQL = str_replace('~CLIENTID~', ff($strClientID), $strSQL);
        $strSQL = str_replace('~MESSAGEID~', ff($strMessageID), $strSQL);
        $strSQL = str_replace('~ISARCHIVED~', ff($strIsArchived), $strSQL);
