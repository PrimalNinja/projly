...
//
//	for each transaction that is PEN-PROCESS
...
//
function commitProducts($objConn_a, $strClientID_a)
{
	...
	if (dependencies('security/clientProductAdd,security/profilesInitialise'))
	{
		...
		$strSQL = "select id from ~TABLENAMETRANSACTIONPENDING~ where client_id = ~CLIENTID~ and g31f68ccc_e168_40b8_a7db_693353aa7aea_statuscode = 'PEN-PROCESS'";
		$strSQL = str_replace('~TABLENAMETRANSACTIONPENDING~', ff($strTableNameTransactionPending), $strSQL);
		$strSQL = str_replace('~CLIENTID~', ff($strClientID_a), $strSQL);
		$objResultTransactionPending = dbOpenRecordset($objConn_a, $strSQL, __FUNCTION__);

		while ($arrRowTransactionPending = dbReadRecord($objResultTransactionPending)) 
		{
			$strTransactionPendingID = $arrRowTransactionPending['id'];
			...
			while ($arrRow = dbReadRecord($objResult)) 
			{
				...
				if (strlen($strWorkQueueItemTypeID) > 0)
				{
					...
					// $strWorkQueueID = createApplication($objConn_a, $strTransactionPendingID, $strTransactionLinePendingID, $strApplicantID, $strApplicantType, $strWorkQueueItemTypeCode);
			...
			exposeEntityData($objConn_a, 'SYSTEMFORM', 'TRANSACTIONPENDING', $strTransactionPendingID, $strJSONData);
/*			
			if ($blnTransactionPending)
			{
