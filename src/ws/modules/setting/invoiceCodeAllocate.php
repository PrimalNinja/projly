<?php

// allocate an invoice code
function invoiceCodeAllocate($objConn_a, $strClientID_a)
{
	$strTableNameTransaction = getTableNameEntity("transaction", false);
	
    $strResult = '';

    $blnContinue = true;
    $intTry = 1;
    $intTimeout = 10;

    if (dependencies('setting/settingGet,setting/rnsCodeAllocate,setting/sequenceAllocate')) 
	{
		$strRNSIDPrefix = settingGet($objConn_a, 'CORE', 'RNSIDPFX', $strClientID_a, '', '', '', __FUNCTION__);
		$strRNSCode = rnsCodeAllocate($objConn_a, $strClientID_a, $strRNSIDPrefix);

        while ($blnContinue)
        {
            $strResult = sequenceAllocate($objConn_a, '', 'CORE', 'INVOICEID', $strClientID_a, '', $strRNSIDPrefix, $strRNSCode, '', __FUNCTION__);

            $strSQL = "select id returnvalue from ~TABLENAMETRANSACTION~ where client_id = ~CLIENTID~ and g31f68ccc_e168_40b8_a7db_693353aa7aea_receiptnumber = '~CODE~'";
			$strSQL = str_replace('~TABLENAMETRANSACTION~', ff($strTableNameTransaction), $strSQL);
			$strSQL = str_replace('~CLIENTID~', ff($strClientID_a), $strSQL);
            $strSQL = str_replace('~CODE~', ff($strResult), $strSQL);
            $intID = dbReadValue($objConn_a, $strSQL, __FUNCTION__);

            if ((strlen($intID) == 0) || ($intTry == $intTimeout))
            {
                $blnContinue = false;
            }

            $intTry++;
        }
    }

    return $strResult;
}