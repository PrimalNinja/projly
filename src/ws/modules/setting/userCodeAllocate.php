...
// allocate an user code
function userCodeAllocate($objConn_a, $strClientID_a)
{
    ...
    if (dependencies('setting/settingGet,setting/rnsCodeAllocate,setting/sequenceAllocate')) 
	{
        ...
        while ($blnContinue)
        {
            $strResult = sequenceAllocate($objConn_a, '', 'CORE', 'USERID', $strClientID_a, '', $strRNSIDPrefix, $strRNSCode, '', __FUNCTION__);
            
			$strSQL = "select id returnvalue from ~TABLENAMEUSER~ where client_id = ~CLIENTID~ and code = '~CODE~'";
