...
// allocate an projectparticipant code
function projectParticipantCodeAllocate($objConn_a, $strClientID_a)
{
    ...
    if (dependencies('setting/settingGet,setting/rnsCodeAllocate,setting/sequenceAllocate')) 
	{
        ...
        while ($blnContinue)
        {
            $strResult = sequenceAllocate($objConn_a, '', 'PROJLY', 'PARTICIPANTID', $strClientID_a, '', $strRNSIDPrefix, $strRNSCode, '', __FUNCTION__);

            $strSQL = "select id returnvalue from ~TABLENAMEPROJECTPARTICIPANT~ where client_id = ~CLIENTID~ and code = '~CODE~'";
