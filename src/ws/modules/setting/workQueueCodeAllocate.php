...
// allocate a project task code
function workQueueCodeAllocate($objConn_a, $strClientID_a)
{
    ...
    if (dependencies('setting/settingGet,setting/rnsCodeAllocate,setting/sequenceAllocate')) 
	{
        ...
        while ($blnContinue)
        {
            $strResult = sequenceAllocate($objConn_a, '', 'CORE', 'WORKQUEUEID', $strClientID_a, '', $strRNSIDPrefix, $strRNSCode, '', __FUNCTION__);

            $strSQL = "select id returnvalue from ~TABLENAMEWORKQUEUE~ where client_id = ~CLIENTID~ and code = '~CODE~'";
