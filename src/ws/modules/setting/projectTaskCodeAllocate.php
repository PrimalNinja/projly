...
// allocate a project code
function projectTaskCodeAllocate($objConn_a, $strClientID_a)
{
    ...
    if (dependencies('setting/settingGet,setting/rnsCodeAllocate,setting/sequenceAllocate')) 
	{
        ...
        while ($blnContinue)
        {
            $strResult = sequenceAllocate($objConn_a, '', 'PROJLY', 'PROJECTTASKID', $strClientID_a, '', $strRNSIDPrefix, $strRNSCode, '', __FUNCTION__);

            $strSQL = "select id returnvalue from ~TABLENAMEPROJECTTASK~ where client_id = ~CLIENTID~ and code = '~CODE~'";
