...
class UploadHandler
{
    ...
    {
        //@session_start(); // removed by mitsukibo
        return getSessionID();
    }
    ...
    // added by mitsukibo
    protected function getGUIDFilename($name,
        $type = null, $index = null, $content_range = null) {
        $name = uniqid('', true);
        return $name;
    }
    ...
    protected function handleFormData($file, $index)
    {
        ...
        // added by mitsukibo
        $this->strFlags = $_REQUEST['flags'];
        $this->strSecurityToken = $_REQUEST['securitytoken'];
        $this->strClientVersion = $_REQUEST['clientversion'];
        $this->strCallerID = $_REQUEST['callerid'];
        $this->strFunction = $_REQUEST['function'];
        $this->strFunctionID = $_REQUEST['functionid'];
        // end mitsukibo
    }
    ...
    protected function handleFileUpload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            // added by mitsukibo
            // add the uploaded files metatdata into JSON files
            $strFunctionIDField = "";
            //switch ($this->strFunction) {

                //case "import_generic":
                    //$strFunctionIDField = "id";
                    //break;

				// JC these shouldn't be here as they are not being processed against some identifer (such as file format)
				// it is possible to assign the upload direclty to the entity here, but we would have had to pass that id
				// into the 'functionid' within the upload form if we were to do that, for now the documentid is being returned
				// and a 2nd call is made to associate the documentid to the entity

            
            //}

            if (strlen($strFunctionIDField) > 0) {
                $this->strFunctionID = revertSecuredValue($this->strFunctionID, $strFunctionIDField, true);
            }

            $arrMetaData = array('flags' => $this->strFlags, 'securitytoken' => $this->strSecurityToken, 'clientversion' => $this->strClientVersion, 'callerid' => $this->strCallerID, 'function' => $this->strFunction, 'functionid' => $this->strFunctionID, 'filename' => $file->name, 'original_filename' => $name, 'size' => $file->size, 'type' => $file->type);
            $strJSON = json_encode($arrMetaData);
            file_put_contents($file_path . '.json', $strJSON);
