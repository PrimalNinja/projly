...
class SessionManagerPhP5
{
    ...
    public function SessionManager($strHostname_a, $strLogin_a, $strPassword_a, $strDatabase_a)
    {
    ...
    public function read($strSessionID_a)
    {
		...
		//file_put_contents("d:\dev\session.log", " sessions.php read " . $this->m_strSessionID . ":" . "\n", FILE_APPEND);	// DEBUGSESSION
		
		//logDebug("read session with: " . $this->m_strSessionID, '');
        $intTime = time();
    ...
    public function write($strSessionID_a, $strSessionData_a)
    {
        ...
        $strSQL = "replace t_sessions (session_id, session_data, expires) values ('~SESSIONID~', '~SESSIONDATA~', ~EXPIRETIME~)";
        $strSQL = str_replace('~SESSIONID~', ff($this->m_strSessionID), $strSQL);
