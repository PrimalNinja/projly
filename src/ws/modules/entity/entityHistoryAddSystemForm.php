...
function entityHistoryAddSystemForm($objConn_a, $strTableNameEntity_a, $strClientID_a, $strEntityDataID_a, $strEntityID_a, $strDataEntityID_a, $strCode_a, $strDescription_a, $strIsEnabled_a, $strDataClientID_a, $strJSONData_a, $strModifyUser_a, $strModifyDateTime_a) 
{ 
	$strResult = '';
	...
	$strSQL = "insert into ~TABLENAMEENTITY~ (client_id, entitydata_id, entity_id, dataentity_id, code, description, is_enabled, data_client_id, jsondata, modifyuser, modifydatetime) values(~CLIENTID~, ~ENTITYDATAID~, ~ENTITYID~, ~DATAENTITYID~, '~CODE~', '~DESCRIPTION~', '~IS_ENABLED~', ~DATACLIENTID~, '~JSONDATA~', '~MODIFYUSER~', '~MODIFYDATETIME~')";
	$strSQL = str_replace('~TABLENAMEENTITY~', ff($strTableNameEntity_a), $strSQL);
