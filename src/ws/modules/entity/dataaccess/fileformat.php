...
function add_fileformat($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_fileformat($objConn_a);
	return addbulk_fileformat($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_fileformat($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "fileformat");
	$strCode = "FILEFORMAT";
	$strDescription = "FILEFORMAT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g5ab8ab01-9cdd-450d-b439-de0d09dfeea5";
    ...
    $strSQL = "insert into ~TABLENAMEFILEFORMAT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMAT", $strResult, $strJSONData);
...
function delete_fileformat($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMAT = getTableNameEntity("fileformat", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformat($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMAT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMAT~', ff($strTableNameFILEFORMAT), $strSQL);
...
function deleteExtension_fileformat($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATExtension = getTableNameEntityExtension("fileformat");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformat($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMATEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATEXTENSION~', ff($strTableNameFILEFORMATExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_fileformat($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMAT = getTableNameEntity("fileformat", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILEFORMAT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILEFORMAT~', ff($strTableNameFILEFORMAT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILEFORMAT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMAT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_fileformat($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILEFORMAT = getTableNameEntity("fileformat", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILEFORMAT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMAT~', ff($strTableNameFILEFORMAT), $strSQL);
...
function fetchValue_fileformat($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMAT = getTableNameEntity("fileformat", false);	

	$strWhere = buildWhere_fileformat($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILEFORMAT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMAT~', ff($strTableNameFILEFORMAT), $strSQL);
...
function buildWhere_fileformat($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_fileformat($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILEFORMAT");
}