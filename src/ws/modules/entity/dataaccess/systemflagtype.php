...
function add_systemflagtype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemflagtype($objConn_a);
	return addbulk_systemflagtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemflagtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSYSTEMFLAGTYPE = getTableNameEntity("systemflagtype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "systemflagtype");
	$strCode = "SYSTEMFLAGTYPE";
	$strDescription = "SYSTEMFLAGTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g61034004-2d46-4646-aa9a-433053b5f12e";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMFLAGTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGTYPE~', ff($strTableNameSYSTEMFLAGTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMFLAGTYPE", $strResult, $strJSONData);
...
function delete_systemflagtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGTYPE = getTableNameEntity("systemflagtype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemflagtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMFLAGTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGTYPE~', ff($strTableNameSYSTEMFLAGTYPE), $strSQL);
...
function deleteExtension_systemflagtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGTYPEExtension = getTableNameEntityExtension("systemflagtype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemflagtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMFLAGTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGTYPEEXTENSION~', ff($strTableNameSYSTEMFLAGTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemflagtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGTYPE = getTableNameEntity("systemflagtype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMFLAGTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMFLAGTYPE~', ff($strTableNameSYSTEMFLAGTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMFLAGTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMFLAGTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemflagtype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMFLAGTYPE = getTableNameEntity("systemflagtype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMFLAGTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGTYPE~', ff($strTableNameSYSTEMFLAGTYPE), $strSQL);
...
function fetchValue_systemflagtype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGTYPE = getTableNameEntity("systemflagtype", false);	

	$strWhere = buildWhere_systemflagtype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMFLAGTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGTYPE~', ff($strTableNameSYSTEMFLAGTYPE), $strSQL);
...
function buildWhere_systemflagtype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemflagtype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMFLAGTYPE");
}