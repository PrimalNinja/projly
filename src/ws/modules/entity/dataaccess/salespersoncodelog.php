...
function add_salespersoncodelog($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_salespersoncodelog($objConn_a);
	return addbulk_salespersoncodelog($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_salespersoncodelog($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "salespersoncodelog");
	$strCode = "SALESPERSONCODELOG";
	$strDescription = "SALESPERSONCODELOG";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g8f690085-6d2f-4296-a16a-ff1d1d43b0bb";
    ...
    $strSQL = "insert into ~TABLENAMESALESPERSONCODELOG~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SALESPERSONCODELOG", $strResult, $strJSONData);
...
function delete_salespersoncodelog($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODELOG = getTableNameEntity("salespersoncodelog", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_salespersoncodelog($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESALESPERSONCODELOG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODELOG~', ff($strTableNameSALESPERSONCODELOG), $strSQL);
...
function deleteExtension_salespersoncodelog($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODELOGExtension = getTableNameEntityExtension("salespersoncodelog");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_salespersoncodelog($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESALESPERSONCODELOGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODELOGEXTENSION~', ff($strTableNameSALESPERSONCODELOGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_salespersoncodelog($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODELOG = getTableNameEntity("salespersoncodelog", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESALESPERSONCODELOG~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESALESPERSONCODELOG~', ff($strTableNameSALESPERSONCODELOG), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESALESPERSONCODELOG~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SALESPERSONCODELOG", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_salespersoncodelog($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSALESPERSONCODELOG = getTableNameEntity("salespersoncodelog", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESALESPERSONCODELOG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODELOG~', ff($strTableNameSALESPERSONCODELOG), $strSQL);
...
function fetchValue_salespersoncodelog($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODELOG = getTableNameEntity("salespersoncodelog", false);	

	$strWhere = buildWhere_salespersoncodelog($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESALESPERSONCODELOG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODELOG~', ff($strTableNameSALESPERSONCODELOG), $strSQL);
...
function buildWhere_salespersoncodelog($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_salespersoncodelog($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SALESPERSONCODELOG");
}