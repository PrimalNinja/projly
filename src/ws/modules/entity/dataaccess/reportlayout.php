...
function add_reportlayout($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_reportlayout($objConn_a);
	return addbulk_reportlayout($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_reportlayout($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameREPORTLAYOUT = getTableNameEntity("reportlayout", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "reportlayout");
	$strCode = "REPORTLAYOUT";
	$strDescription = "REPORTLAYOUT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g38e1d8b1-b01e-44d6-bd90-102f0aa92f60r";
    ...
    $strSQL = "insert into ~TABLENAMEREPORTLAYOUT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEREPORTLAYOUT~', ff($strTableNameREPORTLAYOUT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTLAYOUT", $strResult, $strJSONData);
...
function delete_reportlayout($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTLAYOUT = getTableNameEntity("reportlayout", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportlayout($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTLAYOUT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTLAYOUT~', ff($strTableNameREPORTLAYOUT), $strSQL);
...
function deleteExtension_reportlayout($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTLAYOUTExtension = getTableNameEntityExtension("reportlayout");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportlayout($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTLAYOUTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTLAYOUTEXTENSION~', ff($strTableNameREPORTLAYOUTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_reportlayout($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTLAYOUT = getTableNameEntity("reportlayout", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREPORTLAYOUT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREPORTLAYOUT~', ff($strTableNameREPORTLAYOUT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREPORTLAYOUT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTLAYOUT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_reportlayout($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREPORTLAYOUT = getTableNameEntity("reportlayout", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREPORTLAYOUT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTLAYOUT~', ff($strTableNameREPORTLAYOUT), $strSQL);
...
function fetchValue_reportlayout($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTLAYOUT = getTableNameEntity("reportlayout", false);	

	$strWhere = buildWhere_reportlayout($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREPORTLAYOUT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTLAYOUT~', ff($strTableNameREPORTLAYOUT), $strSQL);
...
function buildWhere_reportlayout($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_reportlayout($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REPORTLAYOUT");
}