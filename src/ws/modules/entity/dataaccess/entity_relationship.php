...
function add_entity_relationship($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_entity_relationship($objConn_a);
	return addbulk_entity_relationship($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_entity_relationship($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameENTITY_RELATIONSHIP = getTableNameEntity("entity_relationship", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "entity_relationship");
	$strCode = "ENTITY_RELATIONSHIP";
	$strDescription = "ENTITY_RELATIONSHIP";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gdc4a5462-7953-4be9-8fc6-38d46bcef202";
    ...
    $strSQL = "insert into ~TABLENAMEENTITY_RELATIONSHIP~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEENTITY_RELATIONSHIP~', ff($strTableNameENTITY_RELATIONSHIP), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ENTITY_RELATIONSHIP", $strResult, $strJSONData);
...
function delete_entity_relationship($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITY_RELATIONSHIP = getTableNameEntity("entity_relationship", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entity_relationship($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITY_RELATIONSHIP~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITY_RELATIONSHIP~', ff($strTableNameENTITY_RELATIONSHIP), $strSQL);
...
function deleteExtension_entity_relationship($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITY_RELATIONSHIPExtension = getTableNameEntityExtension("entity_relationship");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entity_relationship($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITY_RELATIONSHIPEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITY_RELATIONSHIPEXTENSION~', ff($strTableNameENTITY_RELATIONSHIPExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_entity_relationship($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITY_RELATIONSHIP = getTableNameEntity("entity_relationship", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEENTITY_RELATIONSHIP~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEENTITY_RELATIONSHIP~', ff($strTableNameENTITY_RELATIONSHIP), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEENTITY_RELATIONSHIP~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ENTITY_RELATIONSHIP", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_entity_relationship($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameENTITY_RELATIONSHIP = getTableNameEntity("entity_relationship", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEENTITY_RELATIONSHIP~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITY_RELATIONSHIP~', ff($strTableNameENTITY_RELATIONSHIP), $strSQL);
...
function fetchValue_entity_relationship($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITY_RELATIONSHIP = getTableNameEntity("entity_relationship", false);	

	$strWhere = buildWhere_entity_relationship($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEENTITY_RELATIONSHIP~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITY_RELATIONSHIP~', ff($strTableNameENTITY_RELATIONSHIP), $strSQL);
...
function buildWhere_entity_relationship($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_entity_relationship($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ENTITY_RELATIONSHIP");
}