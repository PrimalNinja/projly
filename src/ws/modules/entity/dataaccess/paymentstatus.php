...
function add_paymentstatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_paymentstatus($objConn_a);
	return addbulk_paymentstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_paymentstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "paymentstatus");
	$strCode = "PAYMENTSTATUS";
	$strDescription = "PAYMENTSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffb6a06aec-dd39-4675-826e-4c434f1e886f";
    ...
    $strSQL = "insert into ~TABLENAMEPAYMENTSTATUS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PAYMENTSTATUS", $strResult, $strJSONData);
...
function delete_paymentstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTSTATUS = getTableNameEntity("paymentstatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_paymentstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPAYMENTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTSTATUS~', ff($strTableNamePAYMENTSTATUS), $strSQL);
...
function deleteExtension_paymentstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTSTATUSExtension = getTableNameEntityExtension("paymentstatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_paymentstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPAYMENTSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTSTATUSEXTENSION~', ff($strTableNamePAYMENTSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_paymentstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTSTATUS = getTableNameEntity("paymentstatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPAYMENTSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPAYMENTSTATUS~', ff($strTableNamePAYMENTSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPAYMENTSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PAYMENTSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_paymentstatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePAYMENTSTATUS = getTableNameEntity("paymentstatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPAYMENTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTSTATUS~', ff($strTableNamePAYMENTSTATUS), $strSQL);
...
function fetchValue_paymentstatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTSTATUS = getTableNameEntity("paymentstatus", false);	

	$strWhere = buildWhere_paymentstatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPAYMENTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTSTATUS~', ff($strTableNamePAYMENTSTATUS), $strSQL);
...
function buildWhere_paymentstatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_paymentstatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PAYMENTSTATUS");
}