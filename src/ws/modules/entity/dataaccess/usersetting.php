...
function add_usersetting($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_usersetting($objConn_a);
	return addbulk_usersetting($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_usersetting($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
    ...
    $strSQL = "insert into ~TABLENAMEUSERSETTING~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "USERSETTING", $strResult, $strJSONData);
...
function delete_usersetting($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSETTING = getTableNameEntity("usersetting", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_usersetting($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSERSETTING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSETTING~', ff($strTableNameUSERSETTING), $strSQL);
...
function deleteExtension_usersetting($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSETTINGExtension = getTableNameEntityExtension("usersetting");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_usersetting($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSERSETTINGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSETTINGEXTENSION~', ff($strTableNameUSERSETTINGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_usersetting($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSETTING = getTableNameEntity("usersetting", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEUSERSETTING~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEUSERSETTING~', ff($strTableNameUSERSETTING), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEUSERSETTING~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "USERSETTING", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_usersetting($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameUSERSETTING = getTableNameEntity("usersetting", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEUSERSETTING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSETTING~', ff($strTableNameUSERSETTING), $strSQL);
...
function fetchValue_usersetting($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSETTING = getTableNameEntity("usersetting", false);	

	$strWhere = buildWhere_usersetting($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEUSERSETTING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSETTING~', ff($strTableNameUSERSETTING), $strSQL);
...
function buildWhere_usersetting($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_usersetting($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "USERSETTING");
}