...
function add_colour($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_colour($objConn_a);
	return addbulk_colour($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_colour($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCOLOUR = getTableNameEntity("colour", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "colour");
	$strCode = "COLOUR";
	$strDescription = "COLOUR";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "b1357cf9-db2f-4a3a-9094-1fd454026b43";
    ...
    $strSQL = "insert into ~TABLENAMECOLOUR~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECOLOUR~', ff($strTableNameCOLOUR), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "COLOUR", $strResult, $strJSONData);
...
function delete_colour($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCOLOUR = getTableNameEntity("colour", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_colour($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECOLOUR~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECOLOUR~', ff($strTableNameCOLOUR), $strSQL);
...
function deleteExtension_colour($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCOLOURExtension = getTableNameEntityExtension("colour");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_colour($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECOLOUREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECOLOUREXTENSION~', ff($strTableNameCOLOURExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_colour($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCOLOUR = getTableNameEntity("colour", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECOLOUR~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECOLOUR~', ff($strTableNameCOLOUR), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECOLOUR~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "COLOUR", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_colour($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCOLOUR = getTableNameEntity("colour", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECOLOUR~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECOLOUR~', ff($strTableNameCOLOUR), $strSQL);
...
function fetchValue_colour($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCOLOUR = getTableNameEntity("colour", false);	

	$strWhere = buildWhere_colour($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECOLOUR~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECOLOUR~', ff($strTableNameCOLOUR), $strSQL);
...
function buildWhere_colour($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_colour($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "COLOUR");
}