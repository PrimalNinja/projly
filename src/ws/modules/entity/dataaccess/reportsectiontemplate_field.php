...
function add_reportsectiontemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_reportsectiontemplate_field($objConn_a);
	return addbulk_reportsectiontemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_reportsectiontemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameREPORTSECTIONTEMPLATE_FIELD = getTableNameEntity("reportsectiontemplate_field", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "reportsectiontemplate_field");
	$strCode = "REPORTSECTIONTEMPLATE_FIELD";
	$strDescription = "REPORTSECTIONTEMPLATE_FIELD";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g5ad69f53-5c98-481e-a063-0e2913350e0cr";
    ...
    $strSQL = "insert into ~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~', ff($strTableNameREPORTSECTIONTEMPLATE_FIELD), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTSECTIONTEMPLATE_FIELD", $strResult, $strJSONData);
...
function delete_reportsectiontemplate_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATE_FIELD = getTableNameEntity("reportsectiontemplate_field", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportsectiontemplate_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~', ff($strTableNameREPORTSECTIONTEMPLATE_FIELD), $strSQL);
...
function deleteExtension_reportsectiontemplate_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATE_FIELDExtension = getTableNameEntityExtension("reportsectiontemplate_field");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportsectiontemplate_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTSECTIONTEMPLATE_FIELDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE_FIELDEXTENSION~', ff($strTableNameREPORTSECTIONTEMPLATE_FIELDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_reportsectiontemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATE_FIELD = getTableNameEntity("reportsectiontemplate_field", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~', ff($strTableNameREPORTSECTIONTEMPLATE_FIELD), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTSECTIONTEMPLATE_FIELD", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_reportsectiontemplate_field($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREPORTSECTIONTEMPLATE_FIELD = getTableNameEntity("reportsectiontemplate_field", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~', ff($strTableNameREPORTSECTIONTEMPLATE_FIELD), $strSQL);
...
function fetchValue_reportsectiontemplate_field($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATE_FIELD = getTableNameEntity("reportsectiontemplate_field", false);	

	$strWhere = buildWhere_reportsectiontemplate_field($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE_FIELD~', ff($strTableNameREPORTSECTIONTEMPLATE_FIELD), $strSQL);
...
function buildWhere_reportsectiontemplate_field($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_reportsectiontemplate_field($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REPORTSECTIONTEMPLATE_FIELD");
}