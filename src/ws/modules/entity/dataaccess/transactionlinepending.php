...
function add_transactionlinepending($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_transactionlinepending($objConn_a);
	return addbulk_transactionlinepending($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_transactionlinepending($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "TRANSACTIONLINEPENDING";
	$strDescription = "TRANSACTIONLINEPENDING";
    ...
    $strSQL = "insert into ~TABLENAMETRANSACTIONLINEPENDING~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONLINEPENDING", $strResult, $strJSONData);
...
function delete_transactionlinepending($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEPENDING = getTableNameEntity("transactionlinepending", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionlinepending($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONLINEPENDING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEPENDING~', ff($strTableNameTRANSACTIONLINEPENDING), $strSQL);
...
function deleteExtension_transactionlinepending($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEPENDINGExtension = getTableNameEntityExtension("transactionlinepending");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionlinepending($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONLINEPENDINGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEPENDINGEXTENSION~', ff($strTableNameTRANSACTIONLINEPENDINGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_transactionlinepending($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEPENDING = getTableNameEntity("transactionlinepending", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETRANSACTIONLINEPENDING~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETRANSACTIONLINEPENDING~', ff($strTableNameTRANSACTIONLINEPENDING), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETRANSACTIONLINEPENDING~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONLINEPENDING", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_transactionlinepending($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTRANSACTIONLINEPENDING = getTableNameEntity("transactionlinepending", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETRANSACTIONLINEPENDING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEPENDING~', ff($strTableNameTRANSACTIONLINEPENDING), $strSQL);
...
function fetchValue_transactionlinepending($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEPENDING = getTableNameEntity("transactionlinepending", false);	

	$strWhere = buildWhere_transactionlinepending($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETRANSACTIONLINEPENDING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEPENDING~', ff($strTableNameTRANSACTIONLINEPENDING), $strSQL);
...
function buildWhere_transactionlinepending($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_transactionlinepending($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TRANSACTIONLINEPENDING");
}