...
function add_displayorder($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_displayorder($objConn_a);
	return addbulk_displayorder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_displayorder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "displayorder");
	$strCode = "DISPLAYORDER";
	$strDescription = "DISPLAYORDER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "fff61f3462-0549-4f7d-bf93-a669c19a455b";
    ...
    $strSQL = "insert into ~TABLENAMEDISPLAYORDER~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DISPLAYORDER", $strResult, $strJSONData);
...
function delete_displayorder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDISPLAYORDER = getTableNameEntity("displayorder", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_displayorder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDISPLAYORDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDISPLAYORDER~', ff($strTableNameDISPLAYORDER), $strSQL);
...
function deleteExtension_displayorder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDISPLAYORDERExtension = getTableNameEntityExtension("displayorder");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_displayorder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDISPLAYORDEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDISPLAYORDEREXTENSION~', ff($strTableNameDISPLAYORDERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_displayorder($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDISPLAYORDER = getTableNameEntity("displayorder", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDISPLAYORDER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDISPLAYORDER~', ff($strTableNameDISPLAYORDER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDISPLAYORDER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DISPLAYORDER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_displayorder($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDISPLAYORDER = getTableNameEntity("displayorder", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDISPLAYORDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDISPLAYORDER~', ff($strTableNameDISPLAYORDER), $strSQL);
...
function fetchValue_displayorder($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDISPLAYORDER = getTableNameEntity("displayorder", false);	

	$strWhere = buildWhere_displayorder($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDISPLAYORDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDISPLAYORDER~', ff($strTableNameDISPLAYORDER), $strSQL);
...
function buildWhere_displayorder($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_displayorder($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DISPLAYORDER");
}