...
function add_icon($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_icon($objConn_a);
	return addbulk_icon($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_icon($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameICON = getTableNameEntity("icon", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "icon");
	$strCode = "ICON";
	$strDescription = "ICON";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g9e0695bf-4381-41d2-a438-02bc68ad0ba4";
    ...
    $strSQL = "insert into ~TABLENAMEICON~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEICON~', ff($strTableNameICON), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ICON", $strResult, $strJSONData);
...
function delete_icon($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameICON = getTableNameEntity("icon", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_icon($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEICON~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEICON~', ff($strTableNameICON), $strSQL);
...
function deleteExtension_icon($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameICONExtension = getTableNameEntityExtension("icon");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_icon($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEICONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEICONEXTENSION~', ff($strTableNameICONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_icon($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameICON = getTableNameEntity("icon", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEICON~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEICON~', ff($strTableNameICON), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEICON~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ICON", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_icon($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameICON = getTableNameEntity("icon", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEICON~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEICON~', ff($strTableNameICON), $strSQL);
...
function fetchValue_icon($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameICON = getTableNameEntity("icon", false);	

	$strWhere = buildWhere_icon($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEICON~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEICON~', ff($strTableNameICON), $strSQL);
...
function buildWhere_icon($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_icon($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ICON");
}