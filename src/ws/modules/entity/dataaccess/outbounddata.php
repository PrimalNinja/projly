...
function add_outbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_outbounddata($objConn_a);
	return addbulk_outbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_outbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g0d22a428-5fd1-4e58-8917-430169cc8d4d";
    ...
    $strSQL = "insert into ~TABLENAMEOUTBOUNDDATA~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "OUTBOUNDDATA", $strResult, $strJSONData);
...
function delete_outbounddata($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameOUTBOUNDDATA = getTableNameEntity("outbounddata", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_outbounddata($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEOUTBOUNDDATA~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEOUTBOUNDDATA~', ff($strTableNameOUTBOUNDDATA), $strSQL);
...
function deleteExtension_outbounddata($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameOUTBOUNDDATAExtension = getTableNameEntityExtension("outbounddata");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_outbounddata($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEOUTBOUNDDATAEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEOUTBOUNDDATAEXTENSION~', ff($strTableNameOUTBOUNDDATAExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_outbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameOUTBOUNDDATA = getTableNameEntity("outbounddata", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEOUTBOUNDDATA~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEOUTBOUNDDATA~', ff($strTableNameOUTBOUNDDATA), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEOUTBOUNDDATA~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "OUTBOUNDDATA", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_outbounddata($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameOUTBOUNDDATA = getTableNameEntity("outbounddata", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEOUTBOUNDDATA~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEOUTBOUNDDATA~', ff($strTableNameOUTBOUNDDATA), $strSQL);
...
function fetchValue_outbounddata($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameOUTBOUNDDATA = getTableNameEntity("outbounddata", false);	

	$strWhere = buildWhere_outbounddata($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEOUTBOUNDDATA~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEOUTBOUNDDATA~', ff($strTableNameOUTBOUNDDATA), $strSQL);
...
function buildWhere_outbounddata($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_outbounddata($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "OUTBOUNDDATA");
}