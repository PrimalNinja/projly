...
function add_projectcampaign($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectcampaign($objConn_a);
	return addbulk_projectcampaign($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectcampaign($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTCAMPAIGN = getTableNameEntity("projectcampaign", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectcampaign");
	$strCode = "PROJECTCAMPAIGN";
	$strDescription = "PROJECTCAMPAIGN";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g19482ba4-b97d-43a6-bb22-46b81c4ef296";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTCAMPAIGN~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGN~', ff($strTableNamePROJECTCAMPAIGN), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGN", $strResult, $strJSONData);
...
function delete_projectcampaign($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGN = getTableNameEntity("projectcampaign", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaign($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGN~', ff($strTableNamePROJECTCAMPAIGN), $strSQL);
...
function deleteExtension_projectcampaign($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNExtension = getTableNameEntityExtension("projectcampaign");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaign($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGNEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNEXTENSION~', ff($strTableNamePROJECTCAMPAIGNExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectcampaign($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGN = getTableNameEntity("projectcampaign", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTCAMPAIGN~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGN~', ff($strTableNamePROJECTCAMPAIGN), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTCAMPAIGN~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGN", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectcampaign($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTCAMPAIGN = getTableNameEntity("projectcampaign", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTCAMPAIGN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGN~', ff($strTableNamePROJECTCAMPAIGN), $strSQL);
...
function fetchValue_projectcampaign($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGN = getTableNameEntity("projectcampaign", false);	

	$strWhere = buildWhere_projectcampaign($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTCAMPAIGN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGN~', ff($strTableNamePROJECTCAMPAIGN), $strSQL);
...
function buildWhere_projectcampaign($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectcampaign($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTCAMPAIGN");
}