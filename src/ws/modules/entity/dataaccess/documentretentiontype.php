...
function add_documentretentiontype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_documentretentiontype($objConn_a);
	return addbulk_documentretentiontype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_documentretentiontype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameDOCUMENTRETENTIONTYPE = getTableNameEntity("documentretentiontype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "documentretentiontype");
	$strCode = "DOCUMENTRETENTIONTYPE";
	$strDescription = "DOCUMENTRETENTIONTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff190fe1f1-1101-4196-af9d-d33b29fcb719";
    ...
    $strSQL = "insert into ~TABLENAMEDOCUMENTRETENTIONTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEDOCUMENTRETENTIONTYPE~', ff($strTableNameDOCUMENTRETENTIONTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENTRETENTIONTYPE", $strResult, $strJSONData);
...
function delete_documentretentiontype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTRETENTIONTYPE = getTableNameEntity("documentretentiontype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_documentretentiontype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENTRETENTIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTRETENTIONTYPE~', ff($strTableNameDOCUMENTRETENTIONTYPE), $strSQL);
...
function deleteExtension_documentretentiontype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTRETENTIONTYPEExtension = getTableNameEntityExtension("documentretentiontype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_documentretentiontype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENTRETENTIONTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTRETENTIONTYPEEXTENSION~', ff($strTableNameDOCUMENTRETENTIONTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_documentretentiontype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTRETENTIONTYPE = getTableNameEntity("documentretentiontype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDOCUMENTRETENTIONTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDOCUMENTRETENTIONTYPE~', ff($strTableNameDOCUMENTRETENTIONTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDOCUMENTRETENTIONTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENTRETENTIONTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_documentretentiontype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDOCUMENTRETENTIONTYPE = getTableNameEntity("documentretentiontype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDOCUMENTRETENTIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTRETENTIONTYPE~', ff($strTableNameDOCUMENTRETENTIONTYPE), $strSQL);
...
function fetchValue_documentretentiontype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTRETENTIONTYPE = getTableNameEntity("documentretentiontype", false);	

	$strWhere = buildWhere_documentretentiontype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDOCUMENTRETENTIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTRETENTIONTYPE~', ff($strTableNameDOCUMENTRETENTIONTYPE), $strSQL);
...
function buildWhere_documentretentiontype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_documentretentiontype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DOCUMENTRETENTIONTYPE");
}