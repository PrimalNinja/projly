...
function add_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectparticipant($objConn_a);
	return addbulk_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "projectparticipant");
	$strCode = "PROJECTPARTICIPANT";
	$strDescription = "PROJECTPARTICIPANT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "fff8db5be0-1045-441f-b7a2-769fd8de46f0";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTPARTICIPANT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPARTICIPANT", $strResult, $strJSONData);
...
function delete_projectparticipant($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANT = getTableNameEntity("projectparticipant", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectparticipant($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPARTICIPANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANT~', ff($strTableNamePROJECTPARTICIPANT), $strSQL);
...
function deleteExtension_projectparticipant($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTExtension = getTableNameEntityExtension("projectparticipant");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectparticipant($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPARTICIPANTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTEXTENSION~', ff($strTableNamePROJECTPARTICIPANTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANT = getTableNameEntity("projectparticipant", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTPARTICIPANT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANT~', ff($strTableNamePROJECTPARTICIPANT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTPARTICIPANT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPARTICIPANT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectparticipant($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTPARTICIPANT = getTableNameEntity("projectparticipant", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTPARTICIPANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANT~', ff($strTableNamePROJECTPARTICIPANT), $strSQL);
...
function fetchValue_projectparticipant($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANT = getTableNameEntity("projectparticipant", false);	

	$strWhere = buildWhere_projectparticipant($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTPARTICIPANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANT~', ff($strTableNamePROJECTPARTICIPANT), $strSQL);
...
function buildWhere_projectparticipant($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectparticipant($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTPARTICIPANT");
}