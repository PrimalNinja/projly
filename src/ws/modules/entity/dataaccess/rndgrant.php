...
function add_rndgrant($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_rndgrant($objConn_a);
	return addbulk_rndgrant($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_rndgrant($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameRNDGRANT = getTableNameEntity("rndgrant", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "rndgrant");
	$strCode = "RNDGRANT";
	$strDescription = "RNDGRANT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g6677fd35-4344-4a1f-8947-fc7bac647014";
    ...
    $strSQL = "insert into ~TABLENAMERNDGRANT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMERNDGRANT~', ff($strTableNameRNDGRANT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "RNDGRANT", $strResult, $strJSONData);
...
function delete_rndgrant($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRNDGRANT = getTableNameEntity("rndgrant", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rndgrant($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMERNDGRANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERNDGRANT~', ff($strTableNameRNDGRANT), $strSQL);
...
function deleteExtension_rndgrant($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRNDGRANTExtension = getTableNameEntityExtension("rndgrant");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rndgrant($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMERNDGRANTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERNDGRANTEXTENSION~', ff($strTableNameRNDGRANTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_rndgrant($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRNDGRANT = getTableNameEntity("rndgrant", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMERNDGRANT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMERNDGRANT~', ff($strTableNameRNDGRANT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMERNDGRANT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "RNDGRANT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_rndgrant($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameRNDGRANT = getTableNameEntity("rndgrant", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMERNDGRANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERNDGRANT~', ff($strTableNameRNDGRANT), $strSQL);
...
function fetchValue_rndgrant($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRNDGRANT = getTableNameEntity("rndgrant", false);	

	$strWhere = buildWhere_rndgrant($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMERNDGRANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERNDGRANT~', ff($strTableNameRNDGRANT), $strSQL);
...
function buildWhere_rndgrant($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_rndgrant($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "RNDGRANT");
}