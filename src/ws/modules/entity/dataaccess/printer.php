...
function add_printer($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_printer($objConn_a);
	return addbulk_printer($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_printer($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "printer");
	$strCode = "PRINTER";
	$strDescription = "PRINTER";
    ...
    $strSQL = "insert into ~TABLENAMEPRINTER~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTER", $strResult, $strJSONData);
...
function delete_printer($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTER = getTableNameEntity("printer", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printer($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTER~', ff($strTableNamePRINTER), $strSQL);
...
function deleteExtension_printer($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERExtension = getTableNameEntityExtension("printer");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printer($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTEREXTENSION~', ff($strTableNamePRINTERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_printer($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTER = getTableNameEntity("printer", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPRINTER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPRINTER~', ff($strTableNamePRINTER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPRINTER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_printer($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePRINTER = getTableNameEntity("printer", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPRINTER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTER~', ff($strTableNamePRINTER), $strSQL);
...
function fetchValue_printer($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTER = getTableNameEntity("printer", false);	

	$strWhere = buildWhere_printer($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPRINTER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTER~', ff($strTableNamePRINTER), $strSQL);
...
function buildWhere_printer($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_printer($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PRINTER");
}