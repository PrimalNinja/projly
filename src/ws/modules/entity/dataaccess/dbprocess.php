...
function add_dbprocess($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_dbprocess($objConn_a);
	return addbulk_dbprocess($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_dbprocess($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g24e3d61f-aa39-4d5a-b654-cc55e2d69fc9";
    ...
    $strSQL = "insert into ~TABLENAMEDBPROCESS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DBPROCESS", $strResult, $strJSONData);
...
function delete_dbprocess($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDBPROCESS = getTableNameEntity("dbprocess", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_dbprocess($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDBPROCESS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDBPROCESS~', ff($strTableNameDBPROCESS), $strSQL);
...
function deleteExtension_dbprocess($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDBPROCESSExtension = getTableNameEntityExtension("dbprocess");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_dbprocess($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDBPROCESSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDBPROCESSEXTENSION~', ff($strTableNameDBPROCESSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_dbprocess($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDBPROCESS = getTableNameEntity("dbprocess", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDBPROCESS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDBPROCESS~', ff($strTableNameDBPROCESS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDBPROCESS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DBPROCESS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_dbprocess($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDBPROCESS = getTableNameEntity("dbprocess", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDBPROCESS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDBPROCESS~', ff($strTableNameDBPROCESS), $strSQL);
...
function fetchValue_dbprocess($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDBPROCESS = getTableNameEntity("dbprocess", false);	

	$strWhere = buildWhere_dbprocess($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDBPROCESS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDBPROCESS~', ff($strTableNameDBPROCESS), $strSQL);
...
function buildWhere_dbprocess($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_dbprocess($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DBPROCESS");
}