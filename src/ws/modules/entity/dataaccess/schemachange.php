...
function add_schemachange($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_schemachange($objConn_a);
	return addbulk_schemachange($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_schemachange($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "SCHEMACHANGE";
	$strDescription = "SCHEMACHANGE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g523b5f4a-fc16-4f51-ad0c-c1cee86223bd";
    ...
    $strSQL = "insert into ~TABLENAMESCHEMACHANGE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SCHEMACHANGE", $strResult, $strJSONData);
...
function delete_schemachange($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSCHEMACHANGE = getTableNameEntity("schemachange", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_schemachange($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESCHEMACHANGE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESCHEMACHANGE~', ff($strTableNameSCHEMACHANGE), $strSQL);
...
function deleteExtension_schemachange($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSCHEMACHANGEExtension = getTableNameEntityExtension("schemachange");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_schemachange($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESCHEMACHANGEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESCHEMACHANGEEXTENSION~', ff($strTableNameSCHEMACHANGEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_schemachange($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSCHEMACHANGE = getTableNameEntity("schemachange", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESCHEMACHANGE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESCHEMACHANGE~', ff($strTableNameSCHEMACHANGE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESCHEMACHANGE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SCHEMACHANGE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_schemachange($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSCHEMACHANGE = getTableNameEntity("schemachange", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESCHEMACHANGE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESCHEMACHANGE~', ff($strTableNameSCHEMACHANGE), $strSQL);
...
function fetchValue_schemachange($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSCHEMACHANGE = getTableNameEntity("schemachange", false);	

	$strWhere = buildWhere_schemachange($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESCHEMACHANGE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESCHEMACHANGE~', ff($strTableNameSCHEMACHANGE), $strSQL);
...
function buildWhere_schemachange($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_schemachange($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SCHEMACHANGE");
}