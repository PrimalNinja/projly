...
function add_receipt($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_receipt($objConn_a);
	return addbulk_receipt($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_receipt($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff6b4c87cd-9b74-40de-9375-83f280731846";
    ...
    $strSQL = "insert into ~TABLENAMERECEIPT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "RECEIPT", $strResult, $strJSONData);
...
function delete_receipt($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRECEIPT = getTableNameEntity("receipt", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_receipt($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMERECEIPT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERECEIPT~', ff($strTableNameRECEIPT), $strSQL);
...
function deleteExtension_receipt($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRECEIPTExtension = getTableNameEntityExtension("receipt");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_receipt($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMERECEIPTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERECEIPTEXTENSION~', ff($strTableNameRECEIPTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_receipt($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRECEIPT = getTableNameEntity("receipt", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMERECEIPT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMERECEIPT~', ff($strTableNameRECEIPT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMERECEIPT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "RECEIPT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_receipt($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameRECEIPT = getTableNameEntity("receipt", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMERECEIPT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERECEIPT~', ff($strTableNameRECEIPT), $strSQL);
...
function fetchValue_receipt($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRECEIPT = getTableNameEntity("receipt", false);	

	$strWhere = buildWhere_receipt($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMERECEIPT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERECEIPT~', ff($strTableNameRECEIPT), $strSQL);
...
function buildWhere_receipt($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_receipt($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "RECEIPT");
}