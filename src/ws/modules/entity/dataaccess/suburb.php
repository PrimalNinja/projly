...
function add_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_suburb($objConn_a);
	return addbulk_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "f33f67299-5383-4cd3-8455-89e470fb519a";
    ...
    $strSQL = "insert into ~TABLENAMESUBURB~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SUBURB", $strResult, $strJSONData);
...
function delete_suburb($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSUBURB = getTableNameEntity("suburb", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_suburb($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESUBURB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESUBURB~', ff($strTableNameSUBURB), $strSQL);
...
function deleteExtension_suburb($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSUBURBExtension = getTableNameEntityExtension("suburb");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_suburb($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESUBURBEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESUBURBEXTENSION~', ff($strTableNameSUBURBExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSUBURB = getTableNameEntity("suburb", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESUBURB~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESUBURB~', ff($strTableNameSUBURB), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESUBURB~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SUBURB", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_suburb($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSUBURB = getTableNameEntity("suburb", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESUBURB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESUBURB~', ff($strTableNameSUBURB), $strSQL);
...
function fetchValue_suburb($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSUBURB = getTableNameEntity("suburb", false);	

	$strWhere = buildWhere_suburb($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESUBURB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESUBURB~', ff($strTableNameSUBURB), $strSQL);
...
function buildWhere_suburb($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_suburb($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SUBURB");
}