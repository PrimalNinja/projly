...
function add_video($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_video($objConn_a);
	return addbulk_video($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_video($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "video");
	$strCode = "VIDEO";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gd04f7001-3055-42b5-8a1d-20f79967c656";
    ...
    $strSQL = "insert into ~TABLENAMEVIDEO~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "VIDEO", $strResult, $strJSONData);
...
function delete_video($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEO = getTableNameEntity("video", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_video($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEVIDEO~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEO~', ff($strTableNameVIDEO), $strSQL);
...
function deleteExtension_video($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOExtension = getTableNameEntityExtension("video");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_video($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEVIDEOEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOEXTENSION~', ff($strTableNameVIDEOExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_video($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEO = getTableNameEntity("video", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEVIDEO~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEVIDEO~', ff($strTableNameVIDEO), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEVIDEO~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "VIDEO", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_video($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameVIDEO = getTableNameEntity("video", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEVIDEO~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEO~', ff($strTableNameVIDEO), $strSQL);
...
function fetchValue_video($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEO = getTableNameEntity("video", false);	

	$strWhere = buildWhere_video($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEVIDEO~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEO~', ff($strTableNameVIDEO), $strSQL);
...
function buildWhere_video($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_video($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "VIDEO");
}