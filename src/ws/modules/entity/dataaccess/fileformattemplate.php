...
function add_fileformattemplate($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_fileformattemplate($objConn_a);
	return addbulk_fileformattemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_fileformattemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "fileformattemplate");
	$strCode = "FILEFORMATTEMPLATE";
	$strDescription = "FILEFORMATTEMPLATE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g3407c163-bd03-4e95-901c-4b99717cb598";
    ...
    $strSQL = "insert into ~TABLENAMEFILEFORMATTEMPLATE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMATTEMPLATE", $strResult, $strJSONData);
...
function delete_fileformattemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATE = getTableNameEntity("fileformattemplate", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformattemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMATTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE~', ff($strTableNameFILEFORMATTEMPLATE), $strSQL);
...
function deleteExtension_fileformattemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATEExtension = getTableNameEntityExtension("fileformattemplate");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformattemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMATTEMPLATEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATEEXTENSION~', ff($strTableNameFILEFORMATTEMPLATEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_fileformattemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATE = getTableNameEntity("fileformattemplate", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILEFORMATTEMPLATE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE~', ff($strTableNameFILEFORMATTEMPLATE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILEFORMATTEMPLATE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMATTEMPLATE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_fileformattemplate($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILEFORMATTEMPLATE = getTableNameEntity("fileformattemplate", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILEFORMATTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE~', ff($strTableNameFILEFORMATTEMPLATE), $strSQL);
...
function fetchValue_fileformattemplate($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATE = getTableNameEntity("fileformattemplate", false);	

	$strWhere = buildWhere_fileformattemplate($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILEFORMATTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE~', ff($strTableNameFILEFORMATTEMPLATE), $strSQL);
...
function buildWhere_fileformattemplate($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_fileformattemplate($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILEFORMATTEMPLATE");
}