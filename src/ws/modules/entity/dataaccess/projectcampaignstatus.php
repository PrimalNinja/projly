...
function add_projectcampaignstatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectcampaignstatus($objConn_a);
	return addbulk_projectcampaignstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectcampaignstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTCAMPAIGNSTATUS = getTableNameEntity("projectcampaignstatus", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectcampaignstatus");
	$strCode = "PROJECTCAMPAIGNSTATUS";
	$strDescription = "PROJECTCAMPAIGNSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gf4a72cf5-9539-44e1-b1b2-c593511e001a";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTCAMPAIGNSTATUS~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNSTATUS~', ff($strTableNamePROJECTCAMPAIGNSTATUS), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGNSTATUS", $strResult, $strJSONData);
...
function delete_projectcampaignstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNSTATUS = getTableNameEntity("projectcampaignstatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaignstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGNSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNSTATUS~', ff($strTableNamePROJECTCAMPAIGNSTATUS), $strSQL);
...
function deleteExtension_projectcampaignstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNSTATUSExtension = getTableNameEntityExtension("projectcampaignstatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaignstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGNSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNSTATUSEXTENSION~', ff($strTableNamePROJECTCAMPAIGNSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectcampaignstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNSTATUS = getTableNameEntity("projectcampaignstatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTCAMPAIGNSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNSTATUS~', ff($strTableNamePROJECTCAMPAIGNSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTCAMPAIGNSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGNSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectcampaignstatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTCAMPAIGNSTATUS = getTableNameEntity("projectcampaignstatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTCAMPAIGNSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNSTATUS~', ff($strTableNamePROJECTCAMPAIGNSTATUS), $strSQL);
...
function fetchValue_projectcampaignstatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNSTATUS = getTableNameEntity("projectcampaignstatus", false);	

	$strWhere = buildWhere_projectcampaignstatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTCAMPAIGNSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNSTATUS~', ff($strTableNamePROJECTCAMPAIGNSTATUS), $strSQL);
...
function buildWhere_projectcampaignstatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectcampaignstatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTCAMPAIGNSTATUS");
}