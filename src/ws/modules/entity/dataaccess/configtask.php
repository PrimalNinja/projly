...
function add_configtask($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_configtask($objConn_a);
	return addbulk_configtask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_configtask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "configtask");
	$strCode = "CONFIGTASK";
	$strDescription = "CONFIGTASK";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g15aeaa60-835c-4974-87c5-e82ec903629f";
    ...
    $strSQL = "insert into ~TABLENAMECONFIGTASK~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CONFIGTASK", $strResult, $strJSONData);
...
function delete_configtask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIGTASK = getTableNameEntity("configtask", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_configtask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONFIGTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIGTASK~', ff($strTableNameCONFIGTASK), $strSQL);
...
function deleteExtension_configtask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIGTASKExtension = getTableNameEntityExtension("configtask");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_configtask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONFIGTASKEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIGTASKEXTENSION~', ff($strTableNameCONFIGTASKExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_configtask($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIGTASK = getTableNameEntity("configtask", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECONFIGTASK~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECONFIGTASK~', ff($strTableNameCONFIGTASK), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECONFIGTASK~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CONFIGTASK", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_configtask($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCONFIGTASK = getTableNameEntity("configtask", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECONFIGTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIGTASK~', ff($strTableNameCONFIGTASK), $strSQL);
...
function fetchValue_configtask($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIGTASK = getTableNameEntity("configtask", false);	

	$strWhere = buildWhere_configtask($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECONFIGTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIGTASK~', ff($strTableNameCONFIGTASK), $strSQL);
...
function buildWhere_configtask($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_configtask($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CONFIGTASK");
}