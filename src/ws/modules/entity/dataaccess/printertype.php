...
function add_printertype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_printertype($objConn_a);
	return addbulk_printertype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_printertype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "printertype");
	$strCode = "PRINTERTYPE";
	$strDescription = "PRINTERTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g37723374-a432-4bbb-96f5-066f70db5cb1";
    ...
    $strSQL = "insert into ~TABLENAMEPRINTERTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPRINTERTYPE~', ff($strTableNamePRINTERTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTERTYPE", $strResult, $strJSONData);
...
function delete_printertype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERTYPE = getTableNameEntity("printertype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printertype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTERTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERTYPE~', ff($strTableNamePRINTERTYPE), $strSQL);
...
function deleteExtension_printertype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERTYPEExtension = getTableNameEntityExtension("printertype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printertype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTERTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERTYPEEXTENSION~', ff($strTableNamePRINTERTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_printertype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERTYPE = getTableNameEntity("printertype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPRINTERTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPRINTERTYPE~', ff($strTableNamePRINTERTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPRINTERTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTERTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_printertype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePRINTERTYPE = getTableNameEntity("printertype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPRINTERTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERTYPE~', ff($strTableNamePRINTERTYPE), $strSQL);
...
function fetchValue_printertype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERTYPE = getTableNameEntity("printertype", false);	

	$strWhere = buildWhere_printertype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPRINTERTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERTYPE~', ff($strTableNamePRINTERTYPE), $strSQL);
...
function buildWhere_printertype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_printertype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PRINTERTYPE");
}