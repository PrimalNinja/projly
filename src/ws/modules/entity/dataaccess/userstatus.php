...
function add_userstatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_userstatus($objConn_a);
	return addbulk_userstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_userstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "userstatus");
	$strCode = "USERSTATUS";
	$strDescription = "USERSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g72f0b41c-54b6-4f7b-b1a0-7cb716672fe1";
    ...
    $strSQL = "insert into ~TABLENAMEUSERSTATUS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "USERSTATUS", $strResult, $strJSONData);
...
function delete_userstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSTATUS = getTableNameEntity("userstatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_userstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSERSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSTATUS~', ff($strTableNameUSERSTATUS), $strSQL);
...
function deleteExtension_userstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSTATUSExtension = getTableNameEntityExtension("userstatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_userstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSERSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSTATUSEXTENSION~', ff($strTableNameUSERSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_userstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSTATUS = getTableNameEntity("userstatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEUSERSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEUSERSTATUS~', ff($strTableNameUSERSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEUSERSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "USERSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_userstatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameUSERSTATUS = getTableNameEntity("userstatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEUSERSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSTATUS~', ff($strTableNameUSERSTATUS), $strSQL);
...
function fetchValue_userstatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERSTATUS = getTableNameEntity("userstatus", false);	

	$strWhere = buildWhere_userstatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEUSERSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSERSTATUS~', ff($strTableNameUSERSTATUS), $strSQL);
...
function buildWhere_userstatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_userstatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "USERSTATUS");
}