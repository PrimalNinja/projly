...
function add_myclient($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_myclient($objConn_a);
	return addbulk_myclient($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_myclient($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "myclient");
	$strCode = "MYCLIENT";
	$strDescription = "MYCLIENT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g52b14f56-ab19-4418-bf74-b25b5c2519ee";
    ...
    $strSQL = "insert into ~TABLENAMEMYCLIENT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MYCLIENT", $strResult, $strJSONData);
...
function delete_myclient($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMYCLIENT = getTableNameEntity("myclient", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_myclient($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMYCLIENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMYCLIENT~', ff($strTableNameMYCLIENT), $strSQL);
...
function deleteExtension_myclient($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMYCLIENTExtension = getTableNameEntityExtension("myclient");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_myclient($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMYCLIENTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMYCLIENTEXTENSION~', ff($strTableNameMYCLIENTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_myclient($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMYCLIENT = getTableNameEntity("myclient", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMYCLIENT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMYCLIENT~', ff($strTableNameMYCLIENT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMYCLIENT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MYCLIENT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_myclient($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMYCLIENT = getTableNameEntity("myclient", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMYCLIENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMYCLIENT~', ff($strTableNameMYCLIENT), $strSQL);
...
function fetchValue_myclient($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMYCLIENT = getTableNameEntity("myclient", false);	

	$strWhere = buildWhere_myclient($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMYCLIENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMYCLIENT~', ff($strTableNameMYCLIENT), $strSQL);
...
function buildWhere_myclient($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_myclient($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MYCLIENT");
}