...
function add_integrationinboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationinboundplugin($objConn_a);
	return addbulk_integrationinboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationinboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "integrationinboundplugin");
	$strCode = "INTEGRATIONINBOUNDPLUGIN";
	$strDescription = "INTEGRATIONINBOUNDPLUGIN";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gcecf2cdd-fc49-47df-b9f9-02c951a4d41b";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONINBOUNDPLUGIN~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONINBOUNDPLUGIN", $strResult, $strJSONData);
...
function delete_integrationinboundplugin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDPLUGIN = getTableNameEntity("integrationinboundplugin", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationinboundplugin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONINBOUNDPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDPLUGIN~', ff($strTableNameINTEGRATIONINBOUNDPLUGIN), $strSQL);
...
function deleteExtension_integrationinboundplugin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDPLUGINExtension = getTableNameEntityExtension("integrationinboundplugin");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationinboundplugin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONINBOUNDPLUGINEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDPLUGINEXTENSION~', ff($strTableNameINTEGRATIONINBOUNDPLUGINExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationinboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDPLUGIN = getTableNameEntity("integrationinboundplugin", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONINBOUNDPLUGIN~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDPLUGIN~', ff($strTableNameINTEGRATIONINBOUNDPLUGIN), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONINBOUNDPLUGIN~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONINBOUNDPLUGIN", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationinboundplugin($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONINBOUNDPLUGIN = getTableNameEntity("integrationinboundplugin", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONINBOUNDPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDPLUGIN~', ff($strTableNameINTEGRATIONINBOUNDPLUGIN), $strSQL);
...
function fetchValue_integrationinboundplugin($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDPLUGIN = getTableNameEntity("integrationinboundplugin", false);	

	$strWhere = buildWhere_integrationinboundplugin($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONINBOUNDPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDPLUGIN~', ff($strTableNameINTEGRATIONINBOUNDPLUGIN), $strSQL);
...
function buildWhere_integrationinboundplugin($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationinboundplugin($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONINBOUNDPLUGIN");
}