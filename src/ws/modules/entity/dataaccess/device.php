...
function add_device($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_device($objConn_a);
	return addbulk_device($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_device($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "fff26af3dd-710e-4510-8153-058ed948e34e";
    ...
    $strSQL = "insert into ~TABLENAMEDEVICE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DEVICE", $strResult, $strJSONData);
...
function delete_device($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICE = getTableNameEntity("device", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_device($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDEVICE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICE~', ff($strTableNameDEVICE), $strSQL);
...
function deleteExtension_device($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICEExtension = getTableNameEntityExtension("device");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_device($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDEVICEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICEEXTENSION~', ff($strTableNameDEVICEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_device($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICE = getTableNameEntity("device", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDEVICE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDEVICE~', ff($strTableNameDEVICE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDEVICE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DEVICE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_device($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDEVICE = getTableNameEntity("device", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDEVICE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICE~', ff($strTableNameDEVICE), $strSQL);
...
function fetchValue_device($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICE = getTableNameEntity("device", false);	

	$strWhere = buildWhere_device($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDEVICE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICE~', ff($strTableNameDEVICE), $strSQL);
...
function buildWhere_device($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_device($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DEVICE");
}