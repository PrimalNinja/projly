...
function add_profile($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_profile($objConn_a);
	return addbulk_profile($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_profile($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "f9a0f134f-54b8-4e11-81b3-02d4f51dadd0";
    ...
    $strSQL = "insert into ~TABLENAMEPROFILE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROFILE", $strResult, $strJSONData);
...
function delete_profile($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROFILE = getTableNameEntity("profile", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_profile($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROFILE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROFILE~', ff($strTableNamePROFILE), $strSQL);
...
function deleteExtension_profile($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROFILEExtension = getTableNameEntityExtension("profile");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_profile($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROFILEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROFILEEXTENSION~', ff($strTableNamePROFILEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_profile($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROFILE = getTableNameEntity("profile", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROFILE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROFILE~', ff($strTableNamePROFILE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROFILE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROFILE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_profile($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROFILE = getTableNameEntity("profile", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROFILE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROFILE~', ff($strTableNamePROFILE), $strSQL);
...
function fetchValue_profile($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROFILE = getTableNameEntity("profile", false);	

	$strWhere = buildWhere_profile($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROFILE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROFILE~', ff($strTableNamePROFILE), $strSQL);
...
function buildWhere_profile($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_profile($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROFILE");
}