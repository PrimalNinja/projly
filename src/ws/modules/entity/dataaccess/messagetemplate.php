...
function add_messagetemplate($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_messagetemplate($objConn_a);
	return addbulk_messagetemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_messagetemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "messagetemplate");
	$strCode = "MESSAGETEMPLATE";
	$strDescription = "MESSAGETEMPLATE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff3cf93a11-6df2-4982-a62a-45dd3b699608";
    ...
    $strSQL = "insert into ~TABLENAMEMESSAGETEMPLATE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MESSAGETEMPLATE", $strResult, $strJSONData);
...
function delete_messagetemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATE = getTableNameEntity("messagetemplate", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_messagetemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMESSAGETEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATE~', ff($strTableNameMESSAGETEMPLATE), $strSQL);
...
function deleteExtension_messagetemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATEExtension = getTableNameEntityExtension("messagetemplate");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_messagetemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMESSAGETEMPLATEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATEEXTENSION~', ff($strTableNameMESSAGETEMPLATEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_messagetemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATE = getTableNameEntity("messagetemplate", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMESSAGETEMPLATE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATE~', ff($strTableNameMESSAGETEMPLATE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMESSAGETEMPLATE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MESSAGETEMPLATE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_messagetemplate($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMESSAGETEMPLATE = getTableNameEntity("messagetemplate", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMESSAGETEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATE~', ff($strTableNameMESSAGETEMPLATE), $strSQL);
...
function fetchValue_messagetemplate($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATE = getTableNameEntity("messagetemplate", false);	

	$strWhere = buildWhere_messagetemplate($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMESSAGETEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATE~', ff($strTableNameMESSAGETEMPLATE), $strSQL);
...
function buildWhere_messagetemplate($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_messagetemplate($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MESSAGETEMPLATE");
}