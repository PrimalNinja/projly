...
function add_applicationmodule($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_applicationmodule($objConn_a);
	return addbulk_applicationmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_applicationmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "applicationmodule");
	$strCode = "APPLICATIONMODULE";
	$strDescription = "APPLICATIONMODULE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g3afa0f8b-7930-4d98-b5e1-413215a5ce4e";
    ...
    $strSQL = "insert into ~TABLENAMEAPPLICATIONMODULE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "APPLICATIONMODULE", $strResult, $strJSONData);
...
function delete_applicationmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULE = getTableNameEntity("applicationmodule", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_applicationmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPPLICATIONMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE~', ff($strTableNameAPPLICATIONMODULE), $strSQL);
...
function deleteExtension_applicationmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULEExtension = getTableNameEntityExtension("applicationmodule");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_applicationmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPPLICATIONMODULEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULEEXTENSION~', ff($strTableNameAPPLICATIONMODULEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_applicationmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULE = getTableNameEntity("applicationmodule", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAPPLICATIONMODULE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE~', ff($strTableNameAPPLICATIONMODULE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAPPLICATIONMODULE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "APPLICATIONMODULE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_applicationmodule($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAPPLICATIONMODULE = getTableNameEntity("applicationmodule", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAPPLICATIONMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE~', ff($strTableNameAPPLICATIONMODULE), $strSQL);
...
function fetchValue_applicationmodule($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULE = getTableNameEntity("applicationmodule", false);	

	$strWhere = buildWhere_applicationmodule($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAPPLICATIONMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE~', ff($strTableNameAPPLICATIONMODULE), $strSQL);
...
function buildWhere_applicationmodule($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_applicationmodule($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "APPLICATIONMODULE");
}