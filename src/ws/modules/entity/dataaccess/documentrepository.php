...
function add_documentrepository($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_documentrepository($objConn_a);
	return addbulk_documentrepository($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_documentrepository($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "documentrepository");
	$strCode = "DOCUMENTREPOSITORY";
	$strDescription = "DOCUMENTREPOSITORY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g13c0b91f-45bd-492c-8b47-ce82b1757837";
    ...
    $strSQL = "insert into ~TABLENAMEDOCUMENTREPOSITORY~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENTREPOSITORY", $strResult, $strJSONData);
...
function delete_documentrepository($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTREPOSITORY = getTableNameEntity("documentrepository", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_documentrepository($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENTREPOSITORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTREPOSITORY~', ff($strTableNameDOCUMENTREPOSITORY), $strSQL);
...
function deleteExtension_documentrepository($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTREPOSITORYExtension = getTableNameEntityExtension("documentrepository");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_documentrepository($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENTREPOSITORYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTREPOSITORYEXTENSION~', ff($strTableNameDOCUMENTREPOSITORYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_documentrepository($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTREPOSITORY = getTableNameEntity("documentrepository", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDOCUMENTREPOSITORY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDOCUMENTREPOSITORY~', ff($strTableNameDOCUMENTREPOSITORY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDOCUMENTREPOSITORY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENTREPOSITORY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_documentrepository($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDOCUMENTREPOSITORY = getTableNameEntity("documentrepository", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDOCUMENTREPOSITORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTREPOSITORY~', ff($strTableNameDOCUMENTREPOSITORY), $strSQL);
...
function fetchValue_documentrepository($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTREPOSITORY = getTableNameEntity("documentrepository", false);	

	$strWhere = buildWhere_documentrepository($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDOCUMENTREPOSITORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTREPOSITORY~', ff($strTableNameDOCUMENTREPOSITORY), $strSQL);
...
function buildWhere_documentrepository($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_documentrepository($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DOCUMENTREPOSITORY");
}