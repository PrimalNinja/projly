...
function add_systemform($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemform($objConn_a);
	return addbulk_systemform($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemform($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "systemform");
	$strCode = "SYSTEMFORM";
	$strDescription = "SYSTEMFORM";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMFORM~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMFORM", $strResult, $strJSONData);
...
function delete_systemform($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFORM = getTableNameEntity("systemform", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemform($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMFORM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFORM~', ff($strTableNameSYSTEMFORM), $strSQL);
...
function deleteExtension_systemform($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFORMExtension = getTableNameEntityExtension("systemform");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemform($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMFORMEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFORMEXTENSION~', ff($strTableNameSYSTEMFORMExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemform($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFORM = getTableNameEntity("systemform", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMFORM~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMFORM~', ff($strTableNameSYSTEMFORM), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMFORM~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMFORM", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemform($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMFORM = getTableNameEntity("systemform", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMFORM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFORM~', ff($strTableNameSYSTEMFORM), $strSQL);
...
function fetchValue_systemform($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFORM = getTableNameEntity("systemform", false);	

	$strWhere = buildWhere_systemform($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMFORM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFORM~', ff($strTableNameSYSTEMFORM), $strSQL);
...
function buildWhere_systemform($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemform($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMFORM");
}