...
function add_serverprotocol($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_serverprotocol($objConn_a);
	return addbulk_serverprotocol($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_serverprotocol($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "serverprotocol");
	$strCode = "SERVERPROTOCOL";
	$strDescription = "SERVERPROTOCOL";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ge9e3eb2e-0386-4df7-8d6d-5eb36073260e";
    ...
    $strSQL = "insert into ~TABLENAMESERVERPROTOCOL~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SERVERPROTOCOL", $strResult, $strJSONData);
...
function delete_serverprotocol($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVERPROTOCOL = getTableNameEntity("serverprotocol", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_serverprotocol($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESERVERPROTOCOL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVERPROTOCOL~', ff($strTableNameSERVERPROTOCOL), $strSQL);
...
function deleteExtension_serverprotocol($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVERPROTOCOLExtension = getTableNameEntityExtension("serverprotocol");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_serverprotocol($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESERVERPROTOCOLEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVERPROTOCOLEXTENSION~', ff($strTableNameSERVERPROTOCOLExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_serverprotocol($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVERPROTOCOL = getTableNameEntity("serverprotocol", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESERVERPROTOCOL~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESERVERPROTOCOL~', ff($strTableNameSERVERPROTOCOL), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESERVERPROTOCOL~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SERVERPROTOCOL", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_serverprotocol($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSERVERPROTOCOL = getTableNameEntity("serverprotocol", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESERVERPROTOCOL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVERPROTOCOL~', ff($strTableNameSERVERPROTOCOL), $strSQL);
...
function fetchValue_serverprotocol($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVERPROTOCOL = getTableNameEntity("serverprotocol", false);	

	$strWhere = buildWhere_serverprotocol($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESERVERPROTOCOL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVERPROTOCOL~', ff($strTableNameSERVERPROTOCOL), $strSQL);
...
function buildWhere_serverprotocol($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_serverprotocol($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SERVERPROTOCOL");
}