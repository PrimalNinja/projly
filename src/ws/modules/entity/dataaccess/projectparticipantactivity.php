...
function add_projectparticipantactivity($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectparticipantactivity($objConn_a);
	return addbulk_projectparticipantactivity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectparticipantactivity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTPARTICIPANTACTIVITY = getTableNameEntity("projectparticipantactivity", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectparticipantactivity");
	$strCode = "PROJECTPARTICIPANTACTIVITY";
	$strDescription = "PROJECTPARTICIPANTACTIVITY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff27b72e84-73b1-4771-bfe5-c908f2fe50e2";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTPARTICIPANTACTIVITY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTACTIVITY~', ff($strTableNamePROJECTPARTICIPANTACTIVITY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPARTICIPANTACTIVITY", $strResult, $strJSONData);
...
function delete_projectparticipantactivity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTACTIVITY = getTableNameEntity("projectparticipantactivity", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectparticipantactivity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPARTICIPANTACTIVITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTACTIVITY~', ff($strTableNamePROJECTPARTICIPANTACTIVITY), $strSQL);
...
function deleteExtension_projectparticipantactivity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTACTIVITYExtension = getTableNameEntityExtension("projectparticipantactivity");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectparticipantactivity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPARTICIPANTACTIVITYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTACTIVITYEXTENSION~', ff($strTableNamePROJECTPARTICIPANTACTIVITYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectparticipantactivity($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTACTIVITY = getTableNameEntity("projectparticipantactivity", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTPARTICIPANTACTIVITY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTACTIVITY~', ff($strTableNamePROJECTPARTICIPANTACTIVITY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTPARTICIPANTACTIVITY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPARTICIPANTACTIVITY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectparticipantactivity($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTPARTICIPANTACTIVITY = getTableNameEntity("projectparticipantactivity", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTPARTICIPANTACTIVITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTACTIVITY~', ff($strTableNamePROJECTPARTICIPANTACTIVITY), $strSQL);
...
function fetchValue_projectparticipantactivity($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTACTIVITY = getTableNameEntity("projectparticipantactivity", false);	

	$strWhere = buildWhere_projectparticipantactivity($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTPARTICIPANTACTIVITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTACTIVITY~', ff($strTableNamePROJECTPARTICIPANTACTIVITY), $strSQL);
...
function buildWhere_projectparticipantactivity($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectparticipantactivity($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTPARTICIPANTACTIVITY");
}