...
function add_transactionlinehistory($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_transactionlinehistory($objConn_a);
	return addbulk_transactionlinehistory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_transactionlinehistory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "TRANSACTIONLINEHISTORY";
	$strDescription = "TRANSACTIONLINEHISTORY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gd6f57b36-a2bb-47b9-a6ed-7df0335fc0c2";
    ...
    $strSQL = "insert into ~TABLENAMETRANSACTIONLINEHISTORY~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONLINEHISTORY", $strResult, $strJSONData);
...
function delete_transactionlinehistory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEHISTORY = getTableNameEntity("transactionlinehistory", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionlinehistory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONLINEHISTORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEHISTORY~', ff($strTableNameTRANSACTIONLINEHISTORY), $strSQL);
...
function deleteExtension_transactionlinehistory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEHISTORYExtension = getTableNameEntityExtension("transactionlinehistory");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionlinehistory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONLINEHISTORYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEHISTORYEXTENSION~', ff($strTableNameTRANSACTIONLINEHISTORYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_transactionlinehistory($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEHISTORY = getTableNameEntity("transactionlinehistory", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETRANSACTIONLINEHISTORY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETRANSACTIONLINEHISTORY~', ff($strTableNameTRANSACTIONLINEHISTORY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETRANSACTIONLINEHISTORY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONLINEHISTORY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_transactionlinehistory($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTRANSACTIONLINEHISTORY = getTableNameEntity("transactionlinehistory", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETRANSACTIONLINEHISTORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEHISTORY~', ff($strTableNameTRANSACTIONLINEHISTORY), $strSQL);
...
function fetchValue_transactionlinehistory($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEHISTORY = getTableNameEntity("transactionlinehistory", false);	

	$strWhere = buildWhere_transactionlinehistory($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETRANSACTIONLINEHISTORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEHISTORY~', ff($strTableNameTRANSACTIONLINEHISTORY), $strSQL);
...
function buildWhere_transactionlinehistory($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_transactionlinehistory($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TRANSACTIONLINEHISTORY");
}