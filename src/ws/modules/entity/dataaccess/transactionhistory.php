...
function add_transactionhistory($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_transactionhistory($objConn_a);
	return addbulk_transactionhistory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_transactionhistory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "TRANSACTIONHISTORY";
	$strDescription = "TRANSACTIONHISTORY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g31f68ccc-e168-40b8-a7db-693353aa7aea";
    ...
    $strSQL = "insert into ~TABLENAMETRANSACTIONHISTORY~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONHISTORY", $strResult, $strJSONData);
...
function delete_transactionhistory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONHISTORY = getTableNameEntity("transactionhistory", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionhistory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONHISTORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONHISTORY~', ff($strTableNameTRANSACTIONHISTORY), $strSQL);
...
function deleteExtension_transactionhistory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONHISTORYExtension = getTableNameEntityExtension("transactionhistory");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionhistory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONHISTORYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONHISTORYEXTENSION~', ff($strTableNameTRANSACTIONHISTORYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_transactionhistory($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONHISTORY = getTableNameEntity("transactionhistory", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETRANSACTIONHISTORY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETRANSACTIONHISTORY~', ff($strTableNameTRANSACTIONHISTORY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETRANSACTIONHISTORY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONHISTORY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_transactionhistory($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTRANSACTIONHISTORY = getTableNameEntity("transactionhistory", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETRANSACTIONHISTORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONHISTORY~', ff($strTableNameTRANSACTIONHISTORY), $strSQL);
...
function fetchValue_transactionhistory($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONHISTORY = getTableNameEntity("transactionhistory", false);	

	$strWhere = buildWhere_transactionhistory($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETRANSACTIONHISTORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONHISTORY~', ff($strTableNameTRANSACTIONHISTORY), $strSQL);
...
function buildWhere_transactionhistory($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_transactionhistory($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TRANSACTIONHISTORY");
}