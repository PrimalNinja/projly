...
function add_module($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_module($objConn_a);
	return addbulk_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "module");
	$strCode = "MODULE";
	$strDescription = "MODULE";
    ...
    $strSQL = "insert into ~TABLENAMEMODULE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MODULE", $strResult, $strJSONData);
...
function delete_module($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULE = getTableNameEntity("module", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_module($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULE~', ff($strTableNameMODULE), $strSQL);
...
function deleteExtension_module($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULEExtension = getTableNameEntityExtension("module");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_module($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMODULEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULEEXTENSION~', ff($strTableNameMODULEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULE = getTableNameEntity("module", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMODULE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMODULE~', ff($strTableNameMODULE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMODULE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MODULE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_module($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMODULE = getTableNameEntity("module", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULE~', ff($strTableNameMODULE), $strSQL);
...
function fetchValue_module($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULE = getTableNameEntity("module", false);	

	$strWhere = buildWhere_module($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULE~', ff($strTableNameMODULE), $strSQL);
...
function buildWhere_module($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_module($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MODULE");
}