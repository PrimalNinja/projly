...
function add_formsection_field($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_formsection_field($objConn_a);
	return addbulk_formsection_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_formsection_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFORMSECTION_FIELD = getTableNameEntity("formsection_field", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "formsection_field");
	$strCode = "FORMSECTION_FIELD";
	$strDescription = "FORMSECTION_FIELD";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gfae1e5b0-bfcc-49bd-bad7-996129013235";
    ...
    $strSQL = "insert into ~TABLENAMEFORMSECTION_FIELD~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD~', ff($strTableNameFORMSECTION_FIELD), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FORMSECTION_FIELD", $strResult, $strJSONData);
...
function delete_formsection_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELD = getTableNameEntity("formsection_field", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formsection_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMSECTION_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD~', ff($strTableNameFORMSECTION_FIELD), $strSQL);
...
function deleteExtension_formsection_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELDExtension = getTableNameEntityExtension("formsection_field");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formsection_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMSECTION_FIELDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELDEXTENSION~', ff($strTableNameFORMSECTION_FIELDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_formsection_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELD = getTableNameEntity("formsection_field", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFORMSECTION_FIELD~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD~', ff($strTableNameFORMSECTION_FIELD), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFORMSECTION_FIELD~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FORMSECTION_FIELD", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_formsection_field($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFORMSECTION_FIELD = getTableNameEntity("formsection_field", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFORMSECTION_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD~', ff($strTableNameFORMSECTION_FIELD), $strSQL);
...
function fetchValue_formsection_field($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELD = getTableNameEntity("formsection_field", false);	

	$strWhere = buildWhere_formsection_field($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFORMSECTION_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD~', ff($strTableNameFORMSECTION_FIELD), $strSQL);
...
function buildWhere_formsection_field($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_formsection_field($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FORMSECTION_FIELD");
}