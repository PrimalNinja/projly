...
function add_smhypothesis($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_smhypothesis($objConn_a);
	return addbulk_smhypothesis($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_smhypothesis($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "smhypothesis");
	$strCode = "SMHYPOTHESIS";
	$strDescription = "SMHYPOTHESIS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gdde3142a-fa0e-47ef-8330-80e8b1c75efc";
    ...
    $strSQL = "insert into ~TABLENAMESMHYPOTHESIS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SMHYPOTHESIS", $strResult, $strJSONData);
...
function delete_smhypothesis($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMHYPOTHESIS = getTableNameEntity("smhypothesis", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smhypothesis($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMHYPOTHESIS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMHYPOTHESIS~', ff($strTableNameSMHYPOTHESIS), $strSQL);
...
function deleteExtension_smhypothesis($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMHYPOTHESISExtension = getTableNameEntityExtension("smhypothesis");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smhypothesis($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMHYPOTHESISEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMHYPOTHESISEXTENSION~', ff($strTableNameSMHYPOTHESISExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_smhypothesis($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMHYPOTHESIS = getTableNameEntity("smhypothesis", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESMHYPOTHESIS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESMHYPOTHESIS~', ff($strTableNameSMHYPOTHESIS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESMHYPOTHESIS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SMHYPOTHESIS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_smhypothesis($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSMHYPOTHESIS = getTableNameEntity("smhypothesis", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESMHYPOTHESIS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMHYPOTHESIS~', ff($strTableNameSMHYPOTHESIS), $strSQL);
...
function fetchValue_smhypothesis($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMHYPOTHESIS = getTableNameEntity("smhypothesis", false);	

	$strWhere = buildWhere_smhypothesis($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESMHYPOTHESIS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMHYPOTHESIS~', ff($strTableNameSMHYPOTHESIS), $strSQL);
...
function buildWhere_smhypothesis($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_smhypothesis($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SMHYPOTHESIS");
}