...
function add_user_profile($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_user_profile($objConn_a);
	return addbulk_user_profile($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_user_profile($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameUSER_PROFILE = getTableNameEntity("user_profile", false);	
	...
	$strCode = "USER_PROFILE";
	$strDescription = "USER_PROFILE";
    ...
    $strSQL = "insert into ~TABLENAMEUSER_PROFILE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEUSER_PROFILE~', ff($strTableNameUSER_PROFILE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "USER_PROFILE", $strResult, $strJSONData);
...
function delete_user_profile($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_PROFILE = getTableNameEntity("user_profile", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_user_profile($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSER_PROFILE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_PROFILE~', ff($strTableNameUSER_PROFILE), $strSQL);
...
function deleteExtension_user_profile($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_PROFILEExtension = getTableNameEntityExtension("user_profile");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_user_profile($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSER_PROFILEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_PROFILEEXTENSION~', ff($strTableNameUSER_PROFILEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_user_profile($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_PROFILE = getTableNameEntity("user_profile", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEUSER_PROFILE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEUSER_PROFILE~', ff($strTableNameUSER_PROFILE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEUSER_PROFILE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "USER_PROFILE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_user_profile($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameUSER_PROFILE = getTableNameEntity("user_profile", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEUSER_PROFILE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_PROFILE~', ff($strTableNameUSER_PROFILE), $strSQL);
...
function fetchValue_user_profile($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_PROFILE = getTableNameEntity("user_profile", false);	

	$strWhere = buildWhere_user_profile($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEUSER_PROFILE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_PROFILE~', ff($strTableNameUSER_PROFILE), $strSQL);
...
function buildWhere_user_profile($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_user_profile($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "USER_PROFILE");
}