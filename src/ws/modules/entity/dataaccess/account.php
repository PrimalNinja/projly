...
function add_account($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_account($objConn_a);
	return addbulk_account($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_account($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff7ae74c1e-baff-4966-8b2e-b21e40eb4437";
    ...
    $strSQL = "insert into ~TABLENAMEACCOUNT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ACCOUNT", $strResult, $strJSONData);
...
function delete_account($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNT = getTableNameEntity("account", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_account($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEACCOUNT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNT~', ff($strTableNameACCOUNT), $strSQL);
...
function deleteExtension_account($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNTExtension = getTableNameEntityExtension("account");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_account($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEACCOUNTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNTEXTENSION~', ff($strTableNameACCOUNTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_account($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNT = getTableNameEntity("account", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEACCOUNT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEACCOUNT~', ff($strTableNameACCOUNT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEACCOUNT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ACCOUNT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_account($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameACCOUNT = getTableNameEntity("account", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEACCOUNT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNT~', ff($strTableNameACCOUNT), $strSQL);
...
function fetchValue_account($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNT = getTableNameEntity("account", false);	

	$strWhere = buildWhere_account($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEACCOUNT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNT~', ff($strTableNameACCOUNT), $strSQL);
...
function buildWhere_account($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_account($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ACCOUNT");
}