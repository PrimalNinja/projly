...
function add_entityoperation_default($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_entityoperation_default($objConn_a);
	return addbulk_entityoperation_default($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_entityoperation_default($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameENTITYOPERATION_DEFAULT = getTableNameEntity("entityoperation_default", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "entityoperation_default");
	$strCode = "ENTITYOPERATION_DEFAULT";
	$strDescription = "ENTITYOPERATION_DEFAULT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff0963af8f-9bda-4fda-891f-2f1c2f4c4e89";
    ...
    $strSQL = "insert into ~TABLENAMEENTITYOPERATION_DEFAULT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION_DEFAULT~', ff($strTableNameENTITYOPERATION_DEFAULT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ENTITYOPERATION_DEFAULT", $strResult, $strJSONData);
...
function delete_entityoperation_default($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATION_DEFAULT = getTableNameEntity("entityoperation_default", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entityoperation_default($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITYOPERATION_DEFAULT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION_DEFAULT~', ff($strTableNameENTITYOPERATION_DEFAULT), $strSQL);
...
function deleteExtension_entityoperation_default($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATION_DEFAULTExtension = getTableNameEntityExtension("entityoperation_default");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entityoperation_default($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITYOPERATION_DEFAULTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION_DEFAULTEXTENSION~', ff($strTableNameENTITYOPERATION_DEFAULTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_entityoperation_default($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATION_DEFAULT = getTableNameEntity("entityoperation_default", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEENTITYOPERATION_DEFAULT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEENTITYOPERATION_DEFAULT~', ff($strTableNameENTITYOPERATION_DEFAULT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEENTITYOPERATION_DEFAULT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ENTITYOPERATION_DEFAULT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_entityoperation_default($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameENTITYOPERATION_DEFAULT = getTableNameEntity("entityoperation_default", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEENTITYOPERATION_DEFAULT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION_DEFAULT~', ff($strTableNameENTITYOPERATION_DEFAULT), $strSQL);
...
function fetchValue_entityoperation_default($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATION_DEFAULT = getTableNameEntity("entityoperation_default", false);	

	$strWhere = buildWhere_entityoperation_default($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEENTITYOPERATION_DEFAULT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION_DEFAULT~', ff($strTableNameENTITYOPERATION_DEFAULT), $strSQL);
...
function buildWhere_entityoperation_default($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_entityoperation_default($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ENTITYOPERATION_DEFAULT");
}