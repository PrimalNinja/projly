...
function add_system_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_system_systemmodule($objConn_a);
	return addbulk_system_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_system_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSYSTEM_SYSTEMMODULE = getTableNameEntity("system_systemmodule", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "system_systemmodule");
	$strCode = "SYSTEM_SYSTEMMODULE";
	$strDescription = "SYSTEM_SYSTEMMODULE";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEM_SYSTEMMODULE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESYSTEM_SYSTEMMODULE~', ff($strTableNameSYSTEM_SYSTEMMODULE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEM_SYSTEMMODULE", $strResult, $strJSONData);
...
function delete_system_systemmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEM_SYSTEMMODULE = getTableNameEntity("system_systemmodule", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_system_systemmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEM_SYSTEMMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEM_SYSTEMMODULE~', ff($strTableNameSYSTEM_SYSTEMMODULE), $strSQL);
...
function deleteExtension_system_systemmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEM_SYSTEMMODULEExtension = getTableNameEntityExtension("system_systemmodule");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_system_systemmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEM_SYSTEMMODULEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEM_SYSTEMMODULEEXTENSION~', ff($strTableNameSYSTEM_SYSTEMMODULEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_system_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEM_SYSTEMMODULE = getTableNameEntity("system_systemmodule", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEM_SYSTEMMODULE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEM_SYSTEMMODULE~', ff($strTableNameSYSTEM_SYSTEMMODULE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEM_SYSTEMMODULE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEM_SYSTEMMODULE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_system_systemmodule($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEM_SYSTEMMODULE = getTableNameEntity("system_systemmodule", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEM_SYSTEMMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEM_SYSTEMMODULE~', ff($strTableNameSYSTEM_SYSTEMMODULE), $strSQL);
...
function fetchValue_system_systemmodule($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEM_SYSTEMMODULE = getTableNameEntity("system_systemmodule", false);	

	$strWhere = buildWhere_system_systemmodule($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEM_SYSTEMMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEM_SYSTEMMODULE~', ff($strTableNameSYSTEM_SYSTEMMODULE), $strSQL);
...
function buildWhere_system_systemmodule($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_system_systemmodule($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEM_SYSTEMMODULE");
}