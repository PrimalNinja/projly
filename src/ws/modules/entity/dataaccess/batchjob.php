...
function add_batchjob($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_batchjob($objConn_a);
	return addbulk_batchjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_batchjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff1a767f7b-1c5d-411f-ae10-a5d486a10328";
    ...
    $strSQL = "insert into ~TABLENAMEBATCHJOB~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "BATCHJOB", $strResult, $strJSONData);
...
function delete_batchjob($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBATCHJOB = getTableNameEntity("batchjob", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_batchjob($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEBATCHJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBATCHJOB~', ff($strTableNameBATCHJOB), $strSQL);
...
function deleteExtension_batchjob($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBATCHJOBExtension = getTableNameEntityExtension("batchjob");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_batchjob($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEBATCHJOBEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBATCHJOBEXTENSION~', ff($strTableNameBATCHJOBExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_batchjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBATCHJOB = getTableNameEntity("batchjob", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEBATCHJOB~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEBATCHJOB~', ff($strTableNameBATCHJOB), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEBATCHJOB~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "BATCHJOB", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_batchjob($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameBATCHJOB = getTableNameEntity("batchjob", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEBATCHJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBATCHJOB~', ff($strTableNameBATCHJOB), $strSQL);
...
function fetchValue_batchjob($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBATCHJOB = getTableNameEntity("batchjob", false);	

	$strWhere = buildWhere_batchjob($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEBATCHJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBATCHJOB~', ff($strTableNameBATCHJOB), $strSQL);
...
function buildWhere_batchjob($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_batchjob($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "BATCHJOB");
}