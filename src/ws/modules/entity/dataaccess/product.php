...
function add_product($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_product($objConn_a);
	return addbulk_product($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_product($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "product");
	$strCode = "PRODUCT";
	$strDescription = "PRODUCT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gffe35a8ad-d290-4ae3-8800-f2935dd30d07";
    ...
    $strSQL = "insert into ~TABLENAMEPRODUCT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PRODUCT", $strResult, $strJSONData);
...
function delete_product($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCT = getTableNameEntity("product", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_product($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRODUCT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCT~', ff($strTableNamePRODUCT), $strSQL);
...
function deleteExtension_product($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCTExtension = getTableNameEntityExtension("product");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_product($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRODUCTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCTEXTENSION~', ff($strTableNamePRODUCTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_product($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCT = getTableNameEntity("product", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPRODUCT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPRODUCT~', ff($strTableNamePRODUCT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPRODUCT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PRODUCT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_product($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePRODUCT = getTableNameEntity("product", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPRODUCT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCT~', ff($strTableNamePRODUCT), $strSQL);
...
function fetchValue_product($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCT = getTableNameEntity("product", false);	

	$strWhere = buildWhere_product($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPRODUCT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCT~', ff($strTableNamePRODUCT), $strSQL);
...
function buildWhere_product($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_product($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PRODUCT");
}