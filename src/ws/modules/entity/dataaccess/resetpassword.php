...
function add_resetpassword($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_resetpassword($objConn_a);
	return addbulk_resetpassword($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_resetpassword($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "RESETPASSWORD";
	$strDescription = "RESETPASSWORD";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g14b59919-5bef-40e8-8eb5-a25b870230aa";
    ...
    $strSQL = "insert into ~TABLENAMERESETPASSWORD~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "RESETPASSWORD", $strResult, $strJSONData);
...
function delete_resetpassword($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRESETPASSWORD = getTableNameEntity("resetpassword", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_resetpassword($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMERESETPASSWORD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERESETPASSWORD~', ff($strTableNameRESETPASSWORD), $strSQL);
...
function deleteExtension_resetpassword($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRESETPASSWORDExtension = getTableNameEntityExtension("resetpassword");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_resetpassword($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMERESETPASSWORDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERESETPASSWORDEXTENSION~', ff($strTableNameRESETPASSWORDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_resetpassword($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRESETPASSWORD = getTableNameEntity("resetpassword", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMERESETPASSWORD~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMERESETPASSWORD~', ff($strTableNameRESETPASSWORD), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMERESETPASSWORD~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "RESETPASSWORD", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_resetpassword($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameRESETPASSWORD = getTableNameEntity("resetpassword", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMERESETPASSWORD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERESETPASSWORD~', ff($strTableNameRESETPASSWORD), $strSQL);
...
function fetchValue_resetpassword($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameRESETPASSWORD = getTableNameEntity("resetpassword", false);	

	$strWhere = buildWhere_resetpassword($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMERESETPASSWORD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMERESETPASSWORD~', ff($strTableNameRESETPASSWORD), $strSQL);
...
function buildWhere_resetpassword($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_resetpassword($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "RESETPASSWORD");
}