...
function add_settingmodule($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_settingmodule($objConn_a);
	return addbulk_settingmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_settingmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSETTINGMODULE = getTableNameEntity("settingmodule", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "settingmodule");
	$strCode = "SETTINGMODULE";
	$strDescription = "SETTINGMODULE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "f7b92ec7-4d8e-462f-8642-1a362e1f8602";
    ...
    $strSQL = "insert into ~TABLENAMESETTINGMODULE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESETTINGMODULE~', ff($strTableNameSETTINGMODULE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SETTINGMODULE", $strResult, $strJSONData);
...
function delete_settingmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSETTINGMODULE = getTableNameEntity("settingmodule", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_settingmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESETTINGMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESETTINGMODULE~', ff($strTableNameSETTINGMODULE), $strSQL);
...
function deleteExtension_settingmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSETTINGMODULEExtension = getTableNameEntityExtension("settingmodule");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_settingmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESETTINGMODULEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESETTINGMODULEEXTENSION~', ff($strTableNameSETTINGMODULEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_settingmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSETTINGMODULE = getTableNameEntity("settingmodule", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESETTINGMODULE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESETTINGMODULE~', ff($strTableNameSETTINGMODULE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESETTINGMODULE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SETTINGMODULE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_settingmodule($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSETTINGMODULE = getTableNameEntity("settingmodule", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESETTINGMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESETTINGMODULE~', ff($strTableNameSETTINGMODULE), $strSQL);
...
function fetchValue_settingmodule($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSETTINGMODULE = getTableNameEntity("settingmodule", false);	

	$strWhere = buildWhere_settingmodule($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESETTINGMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESETTINGMODULE~', ff($strTableNameSETTINGMODULE), $strSQL);
...
function buildWhere_settingmodule($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_settingmodule($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SETTINGMODULE");
}