...
function add_system($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_system($objConn_a);
	return addbulk_system($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_system($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "system");
	$strCode = "SYSTEM";
	$strDescription = "SYSTEM";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gd624043e-decc-4b2c-aea0-9b3009b0c034";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEM~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEM", $strResult, $strJSONData);
...
function delete_system($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEM = getTableNameEntity("system", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_system($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEM~', ff($strTableNameSYSTEM), $strSQL);
...
function deleteExtension_system($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMExtension = getTableNameEntityExtension("system");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_system($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMEXTENSION~', ff($strTableNameSYSTEMExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_system($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEM = getTableNameEntity("system", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEM~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEM~', ff($strTableNameSYSTEM), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEM~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEM", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_system($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEM = getTableNameEntity("system", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEM~', ff($strTableNameSYSTEM), $strSQL);
...
function fetchValue_system($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEM = getTableNameEntity("system", false);	

	$strWhere = buildWhere_system($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEM~', ff($strTableNameSYSTEM), $strSQL);
...
function buildWhere_system($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_system($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEM");
}