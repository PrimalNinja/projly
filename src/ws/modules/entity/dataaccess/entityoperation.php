...
function add_entityoperation($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_entityoperation($objConn_a);
	return addbulk_entityoperation($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_entityoperation($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "ENTITYOPERATION";
	$strDescription = "ENTITYOPERATION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff19a43370-beae-4bbf-aab7-f1c1ce50c50a";
    ...
    $strSQL = "insert into ~TABLENAMEENTITYOPERATION~ (~FIELDS~) values (~VALUES~)";
...
function delete_entityoperation($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATION = getTableNameEntity("entityoperation", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entityoperation($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITYOPERATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION~', ff($strTableNameENTITYOPERATION), $strSQL);
...
function deleteExtension_entityoperation($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATIONExtension = getTableNameEntityExtension("entityoperation");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entityoperation($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITYOPERATIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATIONEXTENSION~', ff($strTableNameENTITYOPERATIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_entityoperation($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATION = getTableNameEntity("entityoperation", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEENTITYOPERATION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEENTITYOPERATION~', ff($strTableNameENTITYOPERATION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEENTITYOPERATION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ENTITYOPERATION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_entityoperation($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameENTITYOPERATION = getTableNameEntity("entityoperation", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEENTITYOPERATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION~', ff($strTableNameENTITYOPERATION), $strSQL);
...
function fetchValue_entityoperation($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYOPERATION = getTableNameEntity("entityoperation", false);	

	$strWhere = buildWhere_entityoperation($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEENTITYOPERATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITYOPERATION~', ff($strTableNameENTITYOPERATION), $strSQL);
...
function buildWhere_entityoperation($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_entityoperation($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ENTITYOPERATION");
}