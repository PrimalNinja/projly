...
function add_rostertemplate_detail($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_rostertemplate_detail($objConn_a);
	return addbulk_rostertemplate_detail($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_rostertemplate_detail($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameROSTERTEMPLATE_DETAIL = getTableNameEntity("rostertemplate_detail", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "rostertemplate_detail");
	$strCode = "ROSTERTEMPLATE_DETAIL";
	$strDescription = "ROSTERTEMPLATE_DETAIL";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g842ac7fc-e994-459f-b510-48b8f8e74239";
    ...
    $strSQL = "insert into ~TABLENAMEROSTERTEMPLATE_DETAIL~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_DETAIL~', ff($strTableNameROSTERTEMPLATE_DETAIL), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTERTEMPLATE_DETAIL", $strResult, $strJSONData);
...
function delete_rostertemplate_detail($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_DETAIL = getTableNameEntity("rostertemplate_detail", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rostertemplate_detail($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTERTEMPLATE_DETAIL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_DETAIL~', ff($strTableNameROSTERTEMPLATE_DETAIL), $strSQL);
...
function deleteExtension_rostertemplate_detail($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_DETAILExtension = getTableNameEntityExtension("rostertemplate_detail");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rostertemplate_detail($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTERTEMPLATE_DETAILEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_DETAILEXTENSION~', ff($strTableNameROSTERTEMPLATE_DETAILExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_rostertemplate_detail($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_DETAIL = getTableNameEntity("rostertemplate_detail", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEROSTERTEMPLATE_DETAIL~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_DETAIL~', ff($strTableNameROSTERTEMPLATE_DETAIL), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEROSTERTEMPLATE_DETAIL~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTERTEMPLATE_DETAIL", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_rostertemplate_detail($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameROSTERTEMPLATE_DETAIL = getTableNameEntity("rostertemplate_detail", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEROSTERTEMPLATE_DETAIL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_DETAIL~', ff($strTableNameROSTERTEMPLATE_DETAIL), $strSQL);
...
function fetchValue_rostertemplate_detail($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_DETAIL = getTableNameEntity("rostertemplate_detail", false);	

	$strWhere = buildWhere_rostertemplate_detail($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEROSTERTEMPLATE_DETAIL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_DETAIL~', ff($strTableNameROSTERTEMPLATE_DETAIL), $strSQL);
...
function buildWhere_rostertemplate_detail($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_rostertemplate_detail($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ROSTERTEMPLATE_DETAIL");
}