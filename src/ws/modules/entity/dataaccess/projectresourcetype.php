...
function add_projectresourcetype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectresourcetype($objConn_a);
	return addbulk_projectresourcetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectresourcetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTRESOURCETYPE = getTableNameEntity("projectresourcetype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectresourcetype");
	$strCode = "PROJECTRESOURCETYPE";
	$strDescription = "PROJECTRESOURCETYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff9b2e1e41-f914-4ee1-85bc-ae42d204a0db";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTRESOURCETYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCETYPE~', ff($strTableNamePROJECTRESOURCETYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTRESOURCETYPE", $strResult, $strJSONData);
...
function delete_projectresourcetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCETYPE = getTableNameEntity("projectresourcetype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectresourcetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTRESOURCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCETYPE~', ff($strTableNamePROJECTRESOURCETYPE), $strSQL);
...
function deleteExtension_projectresourcetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCETYPEExtension = getTableNameEntityExtension("projectresourcetype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectresourcetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTRESOURCETYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCETYPEEXTENSION~', ff($strTableNamePROJECTRESOURCETYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectresourcetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCETYPE = getTableNameEntity("projectresourcetype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTRESOURCETYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTRESOURCETYPE~', ff($strTableNamePROJECTRESOURCETYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTRESOURCETYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTRESOURCETYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectresourcetype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTRESOURCETYPE = getTableNameEntity("projectresourcetype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTRESOURCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCETYPE~', ff($strTableNamePROJECTRESOURCETYPE), $strSQL);
...
function fetchValue_projectresourcetype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCETYPE = getTableNameEntity("projectresourcetype", false);	

	$strWhere = buildWhere_projectresourcetype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTRESOURCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCETYPE~', ff($strTableNamePROJECTRESOURCETYPE), $strSQL);
...
function buildWhere_projectresourcetype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectresourcetype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTRESOURCETYPE");
}