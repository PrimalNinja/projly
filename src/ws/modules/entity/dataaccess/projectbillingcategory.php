...
function add_projectbillingcategory($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectbillingcategory($objConn_a);
	return addbulk_projectbillingcategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectbillingcategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTBILLINGCATEGORY = getTableNameEntity("projectbillingcategory", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectbillingcategory");
	$strCode = "PROJECTBILLINGCATEGORY";
	$strDescription = "PROJECTBILLINGCATEGORY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffa69783fa-2aa4-429b-a7ac-ec5028c4632a";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTBILLINGCATEGORY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTBILLINGCATEGORY~', ff($strTableNamePROJECTBILLINGCATEGORY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTBILLINGCATEGORY", $strResult, $strJSONData);
...
function delete_projectbillingcategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTBILLINGCATEGORY = getTableNameEntity("projectbillingcategory", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectbillingcategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTBILLINGCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTBILLINGCATEGORY~', ff($strTableNamePROJECTBILLINGCATEGORY), $strSQL);
...
function deleteExtension_projectbillingcategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTBILLINGCATEGORYExtension = getTableNameEntityExtension("projectbillingcategory");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectbillingcategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTBILLINGCATEGORYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTBILLINGCATEGORYEXTENSION~', ff($strTableNamePROJECTBILLINGCATEGORYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectbillingcategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTBILLINGCATEGORY = getTableNameEntity("projectbillingcategory", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTBILLINGCATEGORY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTBILLINGCATEGORY~', ff($strTableNamePROJECTBILLINGCATEGORY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTBILLINGCATEGORY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTBILLINGCATEGORY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectbillingcategory($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTBILLINGCATEGORY = getTableNameEntity("projectbillingcategory", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTBILLINGCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTBILLINGCATEGORY~', ff($strTableNamePROJECTBILLINGCATEGORY), $strSQL);
...
function fetchValue_projectbillingcategory($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTBILLINGCATEGORY = getTableNameEntity("projectbillingcategory", false);	

	$strWhere = buildWhere_projectbillingcategory($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTBILLINGCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTBILLINGCATEGORY~', ff($strTableNamePROJECTBILLINGCATEGORY), $strSQL);
...
function buildWhere_projectbillingcategory($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectbillingcategory($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTBILLINGCATEGORY");
}