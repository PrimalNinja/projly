...
function add_theme($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_theme($objConn_a);
	return addbulk_theme($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_theme($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "theme");
	$strCode = "THEME";
	$strDescription = "THEME";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g358d3f55-95a2-488f-a490-ef86b645fd74";
    ...
    $strSQL = "insert into ~TABLENAMETHEME~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "THEME", $strResult, $strJSONData);
...
function delete_theme($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTHEME = getTableNameEntity("theme", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_theme($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETHEME~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETHEME~', ff($strTableNameTHEME), $strSQL);
...
function deleteExtension_theme($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTHEMEExtension = getTableNameEntityExtension("theme");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_theme($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETHEMEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETHEMEEXTENSION~', ff($strTableNameTHEMEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_theme($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTHEME = getTableNameEntity("theme", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETHEME~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETHEME~', ff($strTableNameTHEME), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETHEME~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "THEME", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_theme($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTHEME = getTableNameEntity("theme", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETHEME~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETHEME~', ff($strTableNameTHEME), $strSQL);
...
function fetchValue_theme($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTHEME = getTableNameEntity("theme", false);	

	$strWhere = buildWhere_theme($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETHEME~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETHEME~', ff($strTableNameTHEME), $strSQL);
...
function buildWhere_theme($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_theme($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "THEME");
}