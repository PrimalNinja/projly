...
function add_projectstakeholder($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectstakeholder($objConn_a);
	return addbulk_projectstakeholder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectstakeholder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTSTAKEHOLDER = getTableNameEntity("projectstakeholder", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectstakeholder");
	$strCode = "PROJECTSTAKEHOLDER";
	$strDescription = "PROJECTSTAKEHOLDER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff7ebe200b-59ad-4bce-aa21-f1c8e62a17f9";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTSTAKEHOLDER~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTSTAKEHOLDER~', ff($strTableNamePROJECTSTAKEHOLDER), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTSTAKEHOLDER", $strResult, $strJSONData);
...
function delete_projectstakeholder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTAKEHOLDER = getTableNameEntity("projectstakeholder", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectstakeholder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTSTAKEHOLDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTAKEHOLDER~', ff($strTableNamePROJECTSTAKEHOLDER), $strSQL);
...
function deleteExtension_projectstakeholder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTAKEHOLDERExtension = getTableNameEntityExtension("projectstakeholder");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectstakeholder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTSTAKEHOLDEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTAKEHOLDEREXTENSION~', ff($strTableNamePROJECTSTAKEHOLDERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectstakeholder($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTAKEHOLDER = getTableNameEntity("projectstakeholder", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTSTAKEHOLDER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTSTAKEHOLDER~', ff($strTableNamePROJECTSTAKEHOLDER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTSTAKEHOLDER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTSTAKEHOLDER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectstakeholder($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTSTAKEHOLDER = getTableNameEntity("projectstakeholder", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTSTAKEHOLDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTAKEHOLDER~', ff($strTableNamePROJECTSTAKEHOLDER), $strSQL);
...
function fetchValue_projectstakeholder($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTAKEHOLDER = getTableNameEntity("projectstakeholder", false);	

	$strWhere = buildWhere_projectstakeholder($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTSTAKEHOLDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTAKEHOLDER~', ff($strTableNamePROJECTSTAKEHOLDER), $strSQL);
...
function buildWhere_projectstakeholder($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectstakeholder($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTSTAKEHOLDER");
}