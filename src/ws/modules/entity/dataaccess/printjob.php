...
function add_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_printjob($objConn_a);
	return addbulk_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gb398c9db-2272-4748-907c-d9685b924329";
    ...
    $strSQL = "insert into ~TABLENAMEPRINTJOB~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTJOB", $strResult, $strJSONData);
...
function delete_printjob($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTJOB = getTableNameEntity("printjob", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printjob($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTJOB~', ff($strTableNamePRINTJOB), $strSQL);
...
function deleteExtension_printjob($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTJOBExtension = getTableNameEntityExtension("printjob");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printjob($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTJOBEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTJOBEXTENSION~', ff($strTableNamePRINTJOBExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTJOB = getTableNameEntity("printjob", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPRINTJOB~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPRINTJOB~', ff($strTableNamePRINTJOB), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPRINTJOB~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTJOB", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_printjob($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePRINTJOB = getTableNameEntity("printjob", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPRINTJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTJOB~', ff($strTableNamePRINTJOB), $strSQL);
...
function fetchValue_printjob($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTJOB = getTableNameEntity("printjob", false);	

	$strWhere = buildWhere_printjob($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPRINTJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTJOB~', ff($strTableNamePRINTJOB), $strSQL);
...
function buildWhere_printjob($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_printjob($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PRINTJOB");
}