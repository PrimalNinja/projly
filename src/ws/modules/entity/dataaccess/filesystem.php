...
function add_filesystem($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_filesystem($objConn_a);
	return addbulk_filesystem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_filesystem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFILESYSTEM = getTableNameEntity("filesystem", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "filesystem");
	$strCode = "FILESYSTEM";
	$strDescription = "FILESYSTEM";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g61f41517-8201-61f4-9612-61f415172";
    ...
    $strSQL = "insert into ~TABLENAMEFILESYSTEM~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFILESYSTEM~', ff($strTableNameFILESYSTEM), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILESYSTEM", $strResult, $strJSONData);
...
function delete_filesystem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILESYSTEM = getTableNameEntity("filesystem", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filesystem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILESYSTEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILESYSTEM~', ff($strTableNameFILESYSTEM), $strSQL);
...
function deleteExtension_filesystem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILESYSTEMExtension = getTableNameEntityExtension("filesystem");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filesystem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILESYSTEMEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILESYSTEMEXTENSION~', ff($strTableNameFILESYSTEMExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_filesystem($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILESYSTEM = getTableNameEntity("filesystem", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILESYSTEM~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILESYSTEM~', ff($strTableNameFILESYSTEM), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILESYSTEM~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILESYSTEM", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_filesystem($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILESYSTEM = getTableNameEntity("filesystem", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILESYSTEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILESYSTEM~', ff($strTableNameFILESYSTEM), $strSQL);
...
function fetchValue_filesystem($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILESYSTEM = getTableNameEntity("filesystem", false);	

	$strWhere = buildWhere_filesystem($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILESYSTEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILESYSTEM~', ff($strTableNameFILESYSTEM), $strSQL);
...
function buildWhere_filesystem($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_filesystem($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILESYSTEM");
}