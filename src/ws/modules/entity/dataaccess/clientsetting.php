...
function add_clientsetting($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_clientsetting($objConn_a);
	return addbulk_clientsetting($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_clientsetting($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "GENERALSETTINGS";
    ...
    $strSQL = "insert into ~TABLENAMECLIENTSETTING~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENTSETTING", $strResult, $strJSONData);
...
function delete_clientsetting($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTSETTING = getTableNameEntity("clientsetting", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_clientsetting($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENTSETTING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTSETTING~', ff($strTableNameCLIENTSETTING), $strSQL);
...
function deleteExtension_clientsetting($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTSETTINGExtension = getTableNameEntityExtension("clientsetting");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_clientsetting($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENTSETTINGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTSETTINGEXTENSION~', ff($strTableNameCLIENTSETTINGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_clientsetting($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTSETTING = getTableNameEntity("clientsetting", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECLIENTSETTING~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECLIENTSETTING~', ff($strTableNameCLIENTSETTING), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECLIENTSETTING~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENTSETTING", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_clientsetting($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCLIENTSETTING = getTableNameEntity("clientsetting", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECLIENTSETTING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTSETTING~', ff($strTableNameCLIENTSETTING), $strSQL);
...
function fetchValue_clientsetting($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTSETTING = getTableNameEntity("clientsetting", false);	

	$strWhere = buildWhere_clientsetting($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECLIENTSETTING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTSETTING~', ff($strTableNameCLIENTSETTING), $strSQL);
...
function buildWhere_clientsetting($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_clientsetting($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CLIENTSETTING");
}