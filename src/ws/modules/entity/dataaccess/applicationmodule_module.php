...
function add_applicationmodule_module($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_applicationmodule_module($objConn_a);
	return addbulk_applicationmodule_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_applicationmodule_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameAPPLICATIONMODULE_MODULE = getTableNameEntity("applicationmodule_module", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "applicationmodule_module");
	$strCode = "APPLICATIONMODULE_MODULE";
	$strDescription = "APPLICATIONMODULE_MODULE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gfc55f61d-d58a-431e-9e18-2d717043e068";
    ...
    $strSQL = "insert into ~TABLENAMEAPPLICATIONMODULE_MODULE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE_MODULE~', ff($strTableNameAPPLICATIONMODULE_MODULE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "APPLICATIONMODULE_MODULE", $strResult, $strJSONData);
...
function delete_applicationmodule_module($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULE_MODULE = getTableNameEntity("applicationmodule_module", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_applicationmodule_module($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPPLICATIONMODULE_MODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE_MODULE~', ff($strTableNameAPPLICATIONMODULE_MODULE), $strSQL);
...
function deleteExtension_applicationmodule_module($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULE_MODULEExtension = getTableNameEntityExtension("applicationmodule_module");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_applicationmodule_module($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPPLICATIONMODULE_MODULEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE_MODULEEXTENSION~', ff($strTableNameAPPLICATIONMODULE_MODULEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_applicationmodule_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULE_MODULE = getTableNameEntity("applicationmodule_module", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAPPLICATIONMODULE_MODULE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE_MODULE~', ff($strTableNameAPPLICATIONMODULE_MODULE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAPPLICATIONMODULE_MODULE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "APPLICATIONMODULE_MODULE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_applicationmodule_module($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAPPLICATIONMODULE_MODULE = getTableNameEntity("applicationmodule_module", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAPPLICATIONMODULE_MODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE_MODULE~', ff($strTableNameAPPLICATIONMODULE_MODULE), $strSQL);
...
function fetchValue_applicationmodule_module($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONMODULE_MODULE = getTableNameEntity("applicationmodule_module", false);	

	$strWhere = buildWhere_applicationmodule_module($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAPPLICATIONMODULE_MODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONMODULE_MODULE~', ff($strTableNameAPPLICATIONMODULE_MODULE), $strSQL);
...
function buildWhere_applicationmodule_module($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_applicationmodule_module($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "APPLICATIONMODULE_MODULE");
}