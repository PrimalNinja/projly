...
function add_branch($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_branch($objConn_a);
	return addbulk_branch($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_branch($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "branch");
	$strCode = "BRANCH";
	$strDescription = "BRANCH";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gebe5ea4c-ddc6-4fde-979d-117df042f43d";
    ...
    $strSQL = "insert into ~TABLENAMEBRANCH~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "BRANCH", $strResult, $strJSONData);
...
function delete_branch($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCH = getTableNameEntity("branch", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_branch($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEBRANCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCH~', ff($strTableNameBRANCH), $strSQL);
...
function deleteExtension_branch($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCHExtension = getTableNameEntityExtension("branch");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_branch($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEBRANCHEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCHEXTENSION~', ff($strTableNameBRANCHExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_branch($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCH = getTableNameEntity("branch", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEBRANCH~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEBRANCH~', ff($strTableNameBRANCH), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEBRANCH~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "BRANCH", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_branch($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameBRANCH = getTableNameEntity("branch", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEBRANCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCH~', ff($strTableNameBRANCH), $strSQL);
...
function fetchValue_branch($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCH = getTableNameEntity("branch", false);	

	$strWhere = buildWhere_branch($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEBRANCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCH~', ff($strTableNameBRANCH), $strSQL);
...
function buildWhere_branch($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_branch($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "BRANCH");
}