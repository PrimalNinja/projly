...
function add_integrationtaskoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationtaskoutbound($objConn_a);
	return addbulk_integrationtaskoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationtaskoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "INTEGRATIONTASKOUTBOUND";
	$strDescription = "INTEGRATIONTASKOUTBOUND";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g0db1c356-65dc-4b1b-8fb9-ebb5fc3df916";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONTASKOUTBOUND~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONTASKOUTBOUND", $strResult, $strJSONData);
...
function delete_integrationtaskoutbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKOUTBOUND = getTableNameEntity("integrationtaskoutbound", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationtaskoutbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONTASKOUTBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKOUTBOUND~', ff($strTableNameINTEGRATIONTASKOUTBOUND), $strSQL);
...
function deleteExtension_integrationtaskoutbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKOUTBOUNDExtension = getTableNameEntityExtension("integrationtaskoutbound");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationtaskoutbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONTASKOUTBOUNDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKOUTBOUNDEXTENSION~', ff($strTableNameINTEGRATIONTASKOUTBOUNDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationtaskoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKOUTBOUND = getTableNameEntity("integrationtaskoutbound", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONTASKOUTBOUND~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKOUTBOUND~', ff($strTableNameINTEGRATIONTASKOUTBOUND), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONTASKOUTBOUND~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONTASKOUTBOUND", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationtaskoutbound($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONTASKOUTBOUND = getTableNameEntity("integrationtaskoutbound", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONTASKOUTBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKOUTBOUND~', ff($strTableNameINTEGRATIONTASKOUTBOUND), $strSQL);
...
function fetchValue_integrationtaskoutbound($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKOUTBOUND = getTableNameEntity("integrationtaskoutbound", false);	

	$strWhere = buildWhere_integrationtaskoutbound($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONTASKOUTBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKOUTBOUND~', ff($strTableNameINTEGRATIONTASKOUTBOUND), $strSQL);
...
function buildWhere_integrationtaskoutbound($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationtaskoutbound($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONTASKOUTBOUND");
}