...
function add_formsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_formsection_field_property($objConn_a);
	return addbulk_formsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_formsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFORMSECTION_FIELD_PROPERTY = getTableNameEntity("formsection_field_property", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "formsection_field_property");
	$strCode = "FORMSECTION_FIELD_PROPERTY";
	$strDescription = "FORMSECTION_FIELD_PROPERTY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g93c39f5a-6910-492e-85fd-b781702bcf0d";
    ...
    $strSQL = "insert into ~TABLENAMEFORMSECTION_FIELD_PROPERTY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD_PROPERTY~', ff($strTableNameFORMSECTION_FIELD_PROPERTY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FORMSECTION_FIELD_PROPERTY", $strResult, $strJSONData);
...
function delete_formsection_field_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELD_PROPERTY = getTableNameEntity("formsection_field_property", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formsection_field_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMSECTION_FIELD_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD_PROPERTY~', ff($strTableNameFORMSECTION_FIELD_PROPERTY), $strSQL);
...
function deleteExtension_formsection_field_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELD_PROPERTYExtension = getTableNameEntityExtension("formsection_field_property");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formsection_field_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMSECTION_FIELD_PROPERTYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD_PROPERTYEXTENSION~', ff($strTableNameFORMSECTION_FIELD_PROPERTYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_formsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELD_PROPERTY = getTableNameEntity("formsection_field_property", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFORMSECTION_FIELD_PROPERTY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD_PROPERTY~', ff($strTableNameFORMSECTION_FIELD_PROPERTY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFORMSECTION_FIELD_PROPERTY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FORMSECTION_FIELD_PROPERTY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_formsection_field_property($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFORMSECTION_FIELD_PROPERTY = getTableNameEntity("formsection_field_property", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFORMSECTION_FIELD_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD_PROPERTY~', ff($strTableNameFORMSECTION_FIELD_PROPERTY), $strSQL);
...
function fetchValue_formsection_field_property($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTION_FIELD_PROPERTY = getTableNameEntity("formsection_field_property", false);	

	$strWhere = buildWhere_formsection_field_property($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFORMSECTION_FIELD_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTION_FIELD_PROPERTY~', ff($strTableNameFORMSECTION_FIELD_PROPERTY), $strSQL);
...
function buildWhere_formsection_field_property($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_formsection_field_property($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FORMSECTION_FIELD_PROPERTY");
}