...
function add_clientproduct($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_clientproduct($objConn_a);
	return addbulk_clientproduct($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_clientproduct($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "CLIENTPRODUCT";
	$strDescription = "CLIENTPRODUCT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff252aa5f0-e8df-4c52-a5c4-de677e1edf85";
    ...
    $strSQL = "insert into ~TABLENAMECLIENTPRODUCT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENTPRODUCT", $strResult, $strJSONData);
...
function delete_clientproduct($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTPRODUCT = getTableNameEntity("clientproduct", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_clientproduct($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENTPRODUCT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTPRODUCT~', ff($strTableNameCLIENTPRODUCT), $strSQL);
...
function deleteExtension_clientproduct($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTPRODUCTExtension = getTableNameEntityExtension("clientproduct");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_clientproduct($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENTPRODUCTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTPRODUCTEXTENSION~', ff($strTableNameCLIENTPRODUCTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_clientproduct($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTPRODUCT = getTableNameEntity("clientproduct", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECLIENTPRODUCT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECLIENTPRODUCT~', ff($strTableNameCLIENTPRODUCT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECLIENTPRODUCT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENTPRODUCT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_clientproduct($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCLIENTPRODUCT = getTableNameEntity("clientproduct", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECLIENTPRODUCT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTPRODUCT~', ff($strTableNameCLIENTPRODUCT), $strSQL);
...
function fetchValue_clientproduct($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTPRODUCT = getTableNameEntity("clientproduct", false);	

	$strWhere = buildWhere_clientproduct($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECLIENTPRODUCT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTPRODUCT~', ff($strTableNameCLIENTPRODUCT), $strSQL);
...
function buildWhere_clientproduct($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_clientproduct($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CLIENTPRODUCT");
}