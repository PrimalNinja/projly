...
function add_contactstatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_contactstatus($objConn_a);
	return addbulk_contactstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_contactstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCONTACTSTATUS = getTableNameEntity("contactstatus", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "contactstatus");
	$strCode = "CONTACTSTATUS";
	$strDescription = "CONTACTSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g2b9ef518-7a59-4fa2-930f-c797a9fc7162";
    ...
    $strSQL = "insert into ~TABLENAMECONTACTSTATUS~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECONTACTSTATUS~', ff($strTableNameCONTACTSTATUS), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CONTACTSTATUS", $strResult, $strJSONData);
...
function delete_contactstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTSTATUS = getTableNameEntity("contactstatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_contactstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONTACTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTSTATUS~', ff($strTableNameCONTACTSTATUS), $strSQL);
...
function deleteExtension_contactstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTSTATUSExtension = getTableNameEntityExtension("contactstatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_contactstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONTACTSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTSTATUSEXTENSION~', ff($strTableNameCONTACTSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_contactstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTSTATUS = getTableNameEntity("contactstatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECONTACTSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECONTACTSTATUS~', ff($strTableNameCONTACTSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECONTACTSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CONTACTSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_contactstatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCONTACTSTATUS = getTableNameEntity("contactstatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECONTACTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTSTATUS~', ff($strTableNameCONTACTSTATUS), $strSQL);
...
function fetchValue_contactstatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTSTATUS = getTableNameEntity("contactstatus", false);	

	$strWhere = buildWhere_contactstatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECONTACTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTSTATUS~', ff($strTableNameCONTACTSTATUS), $strSQL);
...
function buildWhere_contactstatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_contactstatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CONTACTSTATUS");
}