...
function add_branch_user($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_branch_user($objConn_a);
	return addbulk_branch_user($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_branch_user($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameBRANCH_USER = getTableNameEntity("branch_user", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "branch_user");
	$strCode = "BRANCH_USER";
	$strDescription = "BRANCH_USER";
    ...
    $strSQL = "insert into ~TABLENAMEBRANCH_USER~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEBRANCH_USER~', ff($strTableNameBRANCH_USER), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "BRANCH_USER", $strResult, $strJSONData);
...
function delete_branch_user($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCH_USER = getTableNameEntity("branch_user", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_branch_user($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEBRANCH_USER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCH_USER~', ff($strTableNameBRANCH_USER), $strSQL);
...
function deleteExtension_branch_user($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCH_USERExtension = getTableNameEntityExtension("branch_user");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_branch_user($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEBRANCH_USEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCH_USEREXTENSION~', ff($strTableNameBRANCH_USERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_branch_user($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCH_USER = getTableNameEntity("branch_user", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEBRANCH_USER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEBRANCH_USER~', ff($strTableNameBRANCH_USER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEBRANCH_USER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "BRANCH_USER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_branch_user($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameBRANCH_USER = getTableNameEntity("branch_user", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEBRANCH_USER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCH_USER~', ff($strTableNameBRANCH_USER), $strSQL);
...
function fetchValue_branch_user($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameBRANCH_USER = getTableNameEntity("branch_user", false);	

	$strWhere = buildWhere_branch_user($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEBRANCH_USER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEBRANCH_USER~', ff($strTableNameBRANCH_USER), $strSQL);
...
function buildWhere_branch_user($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_branch_user($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "BRANCH_USER");
}