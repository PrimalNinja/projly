...
function add_systemmodule_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemmodule_navitem($objConn_a);
	return addbulk_systemmodule_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemmodule_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSYSTEMMODULE_NAVITEM = getTableNameEntity("systemmodule_navitem", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "systemmodule_navitem");
	$strCode = "SYSTEMMODULE_NAVITEM";
	$strDescription = "SYSTEMMODULE_NAVITEM";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gc209b671-55f5-4c07-bfb9-a9231f3c0c20";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMMODULE_NAVITEM~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_NAVITEM~', ff($strTableNameSYSTEMMODULE_NAVITEM), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULE_NAVITEM", $strResult, $strJSONData);
...
function delete_systemmodule_navitem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_NAVITEM = getTableNameEntity("systemmodule_navitem", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmodule_navitem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULE_NAVITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_NAVITEM~', ff($strTableNameSYSTEMMODULE_NAVITEM), $strSQL);
...
function deleteExtension_systemmodule_navitem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_NAVITEMExtension = getTableNameEntityExtension("systemmodule_navitem");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmodule_navitem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULE_NAVITEMEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_NAVITEMEXTENSION~', ff($strTableNameSYSTEMMODULE_NAVITEMExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemmodule_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_NAVITEM = getTableNameEntity("systemmodule_navitem", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMMODULE_NAVITEM~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMMODULE_NAVITEM~', ff($strTableNameSYSTEMMODULE_NAVITEM), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMMODULE_NAVITEM~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULE_NAVITEM", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemmodule_navitem($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMMODULE_NAVITEM = getTableNameEntity("systemmodule_navitem", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMMODULE_NAVITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_NAVITEM~', ff($strTableNameSYSTEMMODULE_NAVITEM), $strSQL);
...
function fetchValue_systemmodule_navitem($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_NAVITEM = getTableNameEntity("systemmodule_navitem", false);	

	$strWhere = buildWhere_systemmodule_navitem($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMMODULE_NAVITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_NAVITEM~', ff($strTableNameSYSTEMMODULE_NAVITEM), $strSQL);
...
function buildWhere_systemmodule_navitem($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemmodule_navitem($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMMODULE_NAVITEM");
}