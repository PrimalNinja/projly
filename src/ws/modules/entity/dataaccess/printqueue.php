...
function add_printqueue($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_printqueue($objConn_a);
	return addbulk_printqueue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_printqueue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gff9b32c5-96c5-4850-8834-3ed24e70d727";
    ...
    $strSQL = "insert into ~TABLENAMEPRINTQUEUE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTQUEUE", $strResult, $strJSONData);
...
function delete_printqueue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTQUEUE = getTableNameEntity("printqueue", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printqueue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTQUEUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTQUEUE~', ff($strTableNamePRINTQUEUE), $strSQL);
...
function deleteExtension_printqueue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTQUEUEExtension = getTableNameEntityExtension("printqueue");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printqueue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTQUEUEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTQUEUEEXTENSION~', ff($strTableNamePRINTQUEUEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_printqueue($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTQUEUE = getTableNameEntity("printqueue", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPRINTQUEUE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPRINTQUEUE~', ff($strTableNamePRINTQUEUE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPRINTQUEUE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTQUEUE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_printqueue($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePRINTQUEUE = getTableNameEntity("printqueue", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPRINTQUEUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTQUEUE~', ff($strTableNamePRINTQUEUE), $strSQL);
...
function fetchValue_printqueue($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTQUEUE = getTableNameEntity("printqueue", false);	

	$strWhere = buildWhere_printqueue($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPRINTQUEUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTQUEUE~', ff($strTableNamePRINTQUEUE), $strSQL);
...
function buildWhere_printqueue($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_printqueue($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PRINTQUEUE");
}