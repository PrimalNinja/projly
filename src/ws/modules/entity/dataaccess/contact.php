...
function add_contact($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_contact($objConn_a);
	return addbulk_contact($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_contact($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCONTACT = getTableNameEntity("contact", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "contact");
	$strCode = "CONTACT";
	$strDescription = "CONTACT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g5ccad758-2962-475f-bc56-bb9b2d584efd";
    ...
    $strSQL = "insert into ~TABLENAMECONTACT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECONTACT~', ff($strTableNameCONTACT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CONTACT", $strResult, $strJSONData);
...
function delete_contact($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACT = getTableNameEntity("contact", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_contact($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONTACT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACT~', ff($strTableNameCONTACT), $strSQL);
...
function deleteExtension_contact($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTExtension = getTableNameEntityExtension("contact");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_contact($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONTACTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTEXTENSION~', ff($strTableNameCONTACTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_contact($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACT = getTableNameEntity("contact", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECONTACT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECONTACT~', ff($strTableNameCONTACT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECONTACT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CONTACT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_contact($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCONTACT = getTableNameEntity("contact", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECONTACT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACT~', ff($strTableNameCONTACT), $strSQL);
...
function fetchValue_contact($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACT = getTableNameEntity("contact", false);	

	$strWhere = buildWhere_contact($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECONTACT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACT~', ff($strTableNameCONTACT), $strSQL);
...
function buildWhere_contact($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_contact($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CONTACT");
}