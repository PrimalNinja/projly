...
function add_smresearch($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_smresearch($objConn_a);
	return addbulk_smresearch($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_smresearch($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSMRESEARCH = getTableNameEntity("smresearch", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "smresearch");
	$strCode = "SMRESEARCH";
	$strDescription = "SMRESEARCH";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g2e28006b-7af2-4cb6-adc5-f29f6758c63c";
    ...
    $strSQL = "insert into ~TABLENAMESMRESEARCH~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESMRESEARCH~', ff($strTableNameSMRESEARCH), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SMRESEARCH", $strResult, $strJSONData);
...
function delete_smresearch($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMRESEARCH = getTableNameEntity("smresearch", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smresearch($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMRESEARCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMRESEARCH~', ff($strTableNameSMRESEARCH), $strSQL);
...
function deleteExtension_smresearch($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMRESEARCHExtension = getTableNameEntityExtension("smresearch");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smresearch($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMRESEARCHEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMRESEARCHEXTENSION~', ff($strTableNameSMRESEARCHExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_smresearch($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMRESEARCH = getTableNameEntity("smresearch", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESMRESEARCH~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESMRESEARCH~', ff($strTableNameSMRESEARCH), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESMRESEARCH~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SMRESEARCH", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_smresearch($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSMRESEARCH = getTableNameEntity("smresearch", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESMRESEARCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMRESEARCH~', ff($strTableNameSMRESEARCH), $strSQL);
...
function fetchValue_smresearch($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMRESEARCH = getTableNameEntity("smresearch", false);	

	$strWhere = buildWhere_smresearch($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESMRESEARCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMRESEARCH~', ff($strTableNameSMRESEARCH), $strSQL);
...
function buildWhere_smresearch($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_smresearch($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SMRESEARCH");
}