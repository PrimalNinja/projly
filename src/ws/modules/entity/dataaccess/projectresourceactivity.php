...
function add_projectresourceactivity($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectresourceactivity($objConn_a);
	return addbulk_projectresourceactivity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectresourceactivity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTRESOURCEACTIVITY = getTableNameEntity("projectresourceactivity", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectresourceactivity");
	$strCode = "PROJECTRESOURCEACTIVITY";
	$strDescription = "PROJECTRESOURCEACTIVITY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff190fe024-9e8f-4815-a4e0-5f5dfd7a1e65";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTRESOURCEACTIVITY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCEACTIVITY~', ff($strTableNamePROJECTRESOURCEACTIVITY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTRESOURCEACTIVITY", $strResult, $strJSONData);
...
function delete_projectresourceactivity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCEACTIVITY = getTableNameEntity("projectresourceactivity", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectresourceactivity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTRESOURCEACTIVITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCEACTIVITY~', ff($strTableNamePROJECTRESOURCEACTIVITY), $strSQL);
...
function deleteExtension_projectresourceactivity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCEACTIVITYExtension = getTableNameEntityExtension("projectresourceactivity");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectresourceactivity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTRESOURCEACTIVITYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCEACTIVITYEXTENSION~', ff($strTableNamePROJECTRESOURCEACTIVITYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectresourceactivity($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCEACTIVITY = getTableNameEntity("projectresourceactivity", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTRESOURCEACTIVITY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTRESOURCEACTIVITY~', ff($strTableNamePROJECTRESOURCEACTIVITY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTRESOURCEACTIVITY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTRESOURCEACTIVITY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectresourceactivity($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTRESOURCEACTIVITY = getTableNameEntity("projectresourceactivity", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTRESOURCEACTIVITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCEACTIVITY~', ff($strTableNamePROJECTRESOURCEACTIVITY), $strSQL);
...
function fetchValue_projectresourceactivity($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCEACTIVITY = getTableNameEntity("projectresourceactivity", false);	

	$strWhere = buildWhere_projectresourceactivity($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTRESOURCEACTIVITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCEACTIVITY~', ff($strTableNamePROJECTRESOURCEACTIVITY), $strSQL);
...
function buildWhere_projectresourceactivity($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectresourceactivity($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTRESOURCEACTIVITY");
}