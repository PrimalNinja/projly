...
function add_desktopregion($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_desktopregion($objConn_a);
	return addbulk_desktopregion($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_desktopregion($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "desktopregion");
	$strCode = "DESKTOPREGION";
	$strDescription = "DESKTOPREGION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gd21ba3ef-6beb-4984-a42e-26518b6b7258";
    ...
    $strSQL = "insert into ~TABLENAMEDESKTOPREGION~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DESKTOPREGION", $strResult, $strJSONData);
...
function delete_desktopregion($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDESKTOPREGION = getTableNameEntity("desktopregion", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_desktopregion($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDESKTOPREGION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDESKTOPREGION~', ff($strTableNameDESKTOPREGION), $strSQL);
...
function deleteExtension_desktopregion($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDESKTOPREGIONExtension = getTableNameEntityExtension("desktopregion");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_desktopregion($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDESKTOPREGIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDESKTOPREGIONEXTENSION~', ff($strTableNameDESKTOPREGIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_desktopregion($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDESKTOPREGION = getTableNameEntity("desktopregion", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDESKTOPREGION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDESKTOPREGION~', ff($strTableNameDESKTOPREGION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDESKTOPREGION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DESKTOPREGION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_desktopregion($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDESKTOPREGION = getTableNameEntity("desktopregion", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDESKTOPREGION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDESKTOPREGION~', ff($strTableNameDESKTOPREGION), $strSQL);
...
function fetchValue_desktopregion($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDESKTOPREGION = getTableNameEntity("desktopregion", false);	

	$strWhere = buildWhere_desktopregion($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDESKTOPREGION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDESKTOPREGION~', ff($strTableNameDESKTOPREGION), $strSQL);
...
function buildWhere_desktopregion($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_desktopregion($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DESKTOPREGION");
}