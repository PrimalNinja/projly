...
function add_client($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_client($objConn_a);
	return addbulk_client($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_client($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "f8d9bfb3a-3eb0-4620-ad86-af7b4e222325";
    ...
    $strSQL = "insert into ~TABLENAMECLIENT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENT", $strResult, $strJSONData);
...
function delete_client($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENT = getTableNameEntity("client", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_client($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENT~', ff($strTableNameCLIENT), $strSQL);
...
function deleteExtension_client($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTExtension = getTableNameEntityExtension("client");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_client($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTEXTENSION~', ff($strTableNameCLIENTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_client($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENT = getTableNameEntity("client", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECLIENT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECLIENT~', ff($strTableNameCLIENT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECLIENT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_client($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCLIENT = getTableNameEntity("client", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECLIENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENT~', ff($strTableNameCLIENT), $strSQL);
...
function fetchValue_client($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENT = getTableNameEntity("client", false);	

	$strWhere = buildWhere_client($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECLIENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENT~', ff($strTableNameCLIENT), $strSQL);
...
function buildWhere_client($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_client($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CLIENT");
}