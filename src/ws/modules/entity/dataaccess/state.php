...
function add_state($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_state($objConn_a);
	return addbulk_state($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_state($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "b7fd75cf-b2d7-4069-8d7f-795622cb0fce";
    ...
    $strSQL = "insert into ~TABLENAMESTATE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "STATE", $strResult, $strJSONData);
...
function delete_state($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTATE = getTableNameEntity("state", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_state($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESTATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTATE~', ff($strTableNameSTATE), $strSQL);
...
function deleteExtension_state($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTATEExtension = getTableNameEntityExtension("state");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_state($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESTATEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTATEEXTENSION~', ff($strTableNameSTATEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_state($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTATE = getTableNameEntity("state", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESTATE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESTATE~', ff($strTableNameSTATE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESTATE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "STATE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_state($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSTATE = getTableNameEntity("state", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESTATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTATE~', ff($strTableNameSTATE), $strSQL);
...
function fetchValue_state($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTATE = getTableNameEntity("state", false);	

	$strWhere = buildWhere_state($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESTATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTATE~', ff($strTableNameSTATE), $strSQL);
...
function buildWhere_state($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_state($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "STATE");
}