...
function add_startupitem($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_startupitem($objConn_a);
	return addbulk_startupitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_startupitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "startupitem");
	$strCode = "STARTUPITEM";
	$strDescription = "STARTUPITEM";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffa23f2204-c11e-41f3-8c3e-85234a8b683c";
    ...
    $strSQL = "insert into ~TABLENAMESTARTUPITEM~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "STARTUPITEM", $strResult, $strJSONData);
...
function delete_startupitem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTARTUPITEM = getTableNameEntity("startupitem", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_startupitem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESTARTUPITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTARTUPITEM~', ff($strTableNameSTARTUPITEM), $strSQL);
...
function deleteExtension_startupitem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTARTUPITEMExtension = getTableNameEntityExtension("startupitem");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_startupitem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESTARTUPITEMEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTARTUPITEMEXTENSION~', ff($strTableNameSTARTUPITEMExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_startupitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTARTUPITEM = getTableNameEntity("startupitem", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESTARTUPITEM~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESTARTUPITEM~', ff($strTableNameSTARTUPITEM), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESTARTUPITEM~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "STARTUPITEM", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_startupitem($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSTARTUPITEM = getTableNameEntity("startupitem", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESTARTUPITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTARTUPITEM~', ff($strTableNameSTARTUPITEM), $strSQL);
...
function fetchValue_startupitem($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSTARTUPITEM = getTableNameEntity("startupitem", false);	

	$strWhere = buildWhere_startupitem($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESTARTUPITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESTARTUPITEM~', ff($strTableNameSTARTUPITEM), $strSQL);
...
function buildWhere_startupitem($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_startupitem($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "STARTUPITEM");
}