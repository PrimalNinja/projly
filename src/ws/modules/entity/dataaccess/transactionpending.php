...
function add_transactionpending($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_transactionpending($objConn_a);
	return addbulk_transactionpending($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_transactionpending($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "TRANSACTIONPENDING";
	$strDescription = "TRANSACTIONPENDING";
    ...
    $strSQL = "insert into ~TABLENAMETRANSACTIONPENDING~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONPENDING", $strResult, $strJSONData);
...
function delete_transactionpending($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONPENDING = getTableNameEntity("transactionpending", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionpending($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONPENDING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONPENDING~', ff($strTableNameTRANSACTIONPENDING), $strSQL);
...
function deleteExtension_transactionpending($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONPENDINGExtension = getTableNameEntityExtension("transactionpending");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionpending($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONPENDINGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONPENDINGEXTENSION~', ff($strTableNameTRANSACTIONPENDINGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_transactionpending($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONPENDING = getTableNameEntity("transactionpending", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETRANSACTIONPENDING~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETRANSACTIONPENDING~', ff($strTableNameTRANSACTIONPENDING), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETRANSACTIONPENDING~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONPENDING", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_transactionpending($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTRANSACTIONPENDING = getTableNameEntity("transactionpending", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETRANSACTIONPENDING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONPENDING~', ff($strTableNameTRANSACTIONPENDING), $strSQL);
...
function fetchValue_transactionpending($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONPENDING = getTableNameEntity("transactionpending", false);	

	$strWhere = buildWhere_transactionpending($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETRANSACTIONPENDING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONPENDING~', ff($strTableNameTRANSACTIONPENDING), $strSQL);
...
function buildWhere_transactionpending($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_transactionpending($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TRANSACTIONPENDING");
}