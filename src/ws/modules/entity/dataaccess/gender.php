...
function add_gender($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_gender($objConn_a);
	return addbulk_gender($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_gender($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameGENDER = getTableNameEntity("gender", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "gender");
	$strCode = "GENDER";
	$strDescription = "GENDER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "30151969-c000-4465-b7bc-0efe5507dd0d";
    ...
    $strSQL = "insert into ~TABLENAMEGENDER~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEGENDER~', ff($strTableNameGENDER), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "GENDER", $strResult, $strJSONData);
...
function delete_gender($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameGENDER = getTableNameEntity("gender", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_gender($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEGENDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEGENDER~', ff($strTableNameGENDER), $strSQL);
...
function deleteExtension_gender($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameGENDERExtension = getTableNameEntityExtension("gender");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_gender($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEGENDEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEGENDEREXTENSION~', ff($strTableNameGENDERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_gender($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameGENDER = getTableNameEntity("gender", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEGENDER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEGENDER~', ff($strTableNameGENDER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEGENDER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "GENDER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_gender($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameGENDER = getTableNameEntity("gender", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEGENDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEGENDER~', ff($strTableNameGENDER), $strSQL);
...
function fetchValue_gender($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameGENDER = getTableNameEntity("gender", false);	

	$strWhere = buildWhere_gender($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEGENDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEGENDER~', ff($strTableNameGENDER), $strSQL);
...
function buildWhere_gender($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_gender($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "GENDER");
}