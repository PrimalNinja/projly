...
function add_server($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_server($objConn_a);
	return addbulk_server($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_server($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "server");
	$strCode = "SERVER";
	$strDescription = "SERVER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gafb65c33-7df8-4b01-a919-4df125c08f0e";
    ...
    $strSQL = "insert into ~TABLENAMESERVER~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SERVER", $strResult, $strJSONData);
...
function delete_server($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVER = getTableNameEntity("server", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_server($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESERVER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVER~', ff($strTableNameSERVER), $strSQL);
...
function deleteExtension_server($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVERExtension = getTableNameEntityExtension("server");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_server($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESERVEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVEREXTENSION~', ff($strTableNameSERVERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_server($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVER = getTableNameEntity("server", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESERVER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESERVER~', ff($strTableNameSERVER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESERVER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SERVER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_server($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSERVER = getTableNameEntity("server", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESERVER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVER~', ff($strTableNameSERVER), $strSQL);
...
function fetchValue_server($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSERVER = getTableNameEntity("server", false);	

	$strWhere = buildWhere_server($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESERVER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESERVER~', ff($strTableNameSERVER), $strSQL);
...
function buildWhere_server($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_server($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SERVER");
}