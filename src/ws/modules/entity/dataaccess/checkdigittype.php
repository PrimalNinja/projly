...
function add_checkdigittype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_checkdigittype($objConn_a);
	return addbulk_checkdigittype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_checkdigittype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "checkdigittype");
	$strCode = "CHECKDIGITTYPE";
	$strDescription = "CHECKDIGITTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g6f651abc-37e3-4eaf-9eba-5bf62014992d";
    ...
    $strSQL = "insert into ~TABLENAMECHECKDIGITTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECHECKDIGITTYPE~', ff($strTableNameCHECKDIGITTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CHECKDIGITTYPE", $strResult, $strJSONData);
...
function delete_checkdigittype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHECKDIGITTYPE = getTableNameEntity("checkdigittype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_checkdigittype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECHECKDIGITTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHECKDIGITTYPE~', ff($strTableNameCHECKDIGITTYPE), $strSQL);
...
function deleteExtension_checkdigittype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHECKDIGITTYPEExtension = getTableNameEntityExtension("checkdigittype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_checkdigittype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECHECKDIGITTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHECKDIGITTYPEEXTENSION~', ff($strTableNameCHECKDIGITTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_checkdigittype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHECKDIGITTYPE = getTableNameEntity("checkdigittype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECHECKDIGITTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECHECKDIGITTYPE~', ff($strTableNameCHECKDIGITTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECHECKDIGITTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CHECKDIGITTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_checkdigittype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCHECKDIGITTYPE = getTableNameEntity("checkdigittype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECHECKDIGITTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHECKDIGITTYPE~', ff($strTableNameCHECKDIGITTYPE), $strSQL);
...
function fetchValue_checkdigittype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHECKDIGITTYPE = getTableNameEntity("checkdigittype", false);	

	$strWhere = buildWhere_checkdigittype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECHECKDIGITTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHECKDIGITTYPE~', ff($strTableNameCHECKDIGITTYPE), $strSQL);
...
function buildWhere_checkdigittype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_checkdigittype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CHECKDIGITTYPE");
}