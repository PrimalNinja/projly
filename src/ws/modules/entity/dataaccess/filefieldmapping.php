...
function add_filefieldmapping($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_filefieldmapping($objConn_a);
	return addbulk_filefieldmapping($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_filefieldmapping($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "filefieldmapping");
	$strCode = "FILEFIELDMAPPING";
	$strDescription = "FILEFIELDMAPPING";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g3b85cacf-9b3f-464b-b76b-2f2fa7b5fcc0";
    ...
    $strSQL = "insert into ~TABLENAMEFILEFIELDMAPPING~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFIELDMAPPING", $strResult, $strJSONData);
...
function delete_filefieldmapping($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDMAPPING = getTableNameEntity("filefieldmapping", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filefieldmapping($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFIELDMAPPING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDMAPPING~', ff($strTableNameFILEFIELDMAPPING), $strSQL);
...
function deleteExtension_filefieldmapping($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDMAPPINGExtension = getTableNameEntityExtension("filefieldmapping");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filefieldmapping($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFIELDMAPPINGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDMAPPINGEXTENSION~', ff($strTableNameFILEFIELDMAPPINGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_filefieldmapping($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDMAPPING = getTableNameEntity("filefieldmapping", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILEFIELDMAPPING~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILEFIELDMAPPING~', ff($strTableNameFILEFIELDMAPPING), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILEFIELDMAPPING~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFIELDMAPPING", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_filefieldmapping($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILEFIELDMAPPING = getTableNameEntity("filefieldmapping", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILEFIELDMAPPING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDMAPPING~', ff($strTableNameFILEFIELDMAPPING), $strSQL);
...
function fetchValue_filefieldmapping($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDMAPPING = getTableNameEntity("filefieldmapping", false);	

	$strWhere = buildWhere_filefieldmapping($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILEFIELDMAPPING~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDMAPPING~', ff($strTableNameFILEFIELDMAPPING), $strSQL);
...
function buildWhere_filefieldmapping($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_filefieldmapping($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILEFIELDMAPPING");
}