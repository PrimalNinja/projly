...
function add_log_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_log_printjob($objConn_a);
	return addbulk_log_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_log_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameLOG_PRINTJOB = getTableNameEntity("log_printjob", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "log_printjob");
	$strCode = "LOG_PRINTJOB";
	$strDescription = "LOG_PRINTJOB";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gbd1ae958-c387-4282-aea3-2289a5f673d1";
    ...
    $strSQL = "insert into ~TABLENAMELOG_PRINTJOB~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMELOG_PRINTJOB~', ff($strTableNameLOG_PRINTJOB), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "LOG_PRINTJOB", $strResult, $strJSONData);
...
function delete_log_printjob($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLOG_PRINTJOB = getTableNameEntity("log_printjob", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_log_printjob($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMELOG_PRINTJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELOG_PRINTJOB~', ff($strTableNameLOG_PRINTJOB), $strSQL);
...
function deleteExtension_log_printjob($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLOG_PRINTJOBExtension = getTableNameEntityExtension("log_printjob");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_log_printjob($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMELOG_PRINTJOBEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELOG_PRINTJOBEXTENSION~', ff($strTableNameLOG_PRINTJOBExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_log_printjob($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLOG_PRINTJOB = getTableNameEntity("log_printjob", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMELOG_PRINTJOB~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMELOG_PRINTJOB~', ff($strTableNameLOG_PRINTJOB), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMELOG_PRINTJOB~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "LOG_PRINTJOB", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_log_printjob($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameLOG_PRINTJOB = getTableNameEntity("log_printjob", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMELOG_PRINTJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELOG_PRINTJOB~', ff($strTableNameLOG_PRINTJOB), $strSQL);
...
function fetchValue_log_printjob($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLOG_PRINTJOB = getTableNameEntity("log_printjob", false);	

	$strWhere = buildWhere_log_printjob($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMELOG_PRINTJOB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELOG_PRINTJOB~', ff($strTableNameLOG_PRINTJOB), $strSQL);
...
function buildWhere_log_printjob($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_log_printjob($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "LOG_PRINTJOB");
}