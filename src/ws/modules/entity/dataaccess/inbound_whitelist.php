...
function add_inbound_whitelist($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_inbound_whitelist($objConn_a);
	return addbulk_inbound_whitelist($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_inbound_whitelist($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameINBOUND_WHITELIST = getTableNameEntity("inbound_whitelist", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "inbound_whitelist");
	$strCode = "INBOUND_WHITELIST";
	$strDescription = "INBOUND_WHITELIST";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gff06200b-9c01-4408-a038-d07c85e5132e";
    ...
    $strSQL = "insert into ~TABLENAMEINBOUND_WHITELIST~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEINBOUND_WHITELIST~', ff($strTableNameINBOUND_WHITELIST), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INBOUND_WHITELIST", $strResult, $strJSONData);
...
function delete_inbound_whitelist($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUND_WHITELIST = getTableNameEntity("inbound_whitelist", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_inbound_whitelist($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINBOUND_WHITELIST~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUND_WHITELIST~', ff($strTableNameINBOUND_WHITELIST), $strSQL);
...
function deleteExtension_inbound_whitelist($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUND_WHITELISTExtension = getTableNameEntityExtension("inbound_whitelist");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_inbound_whitelist($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINBOUND_WHITELISTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUND_WHITELISTEXTENSION~', ff($strTableNameINBOUND_WHITELISTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_inbound_whitelist($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUND_WHITELIST = getTableNameEntity("inbound_whitelist", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINBOUND_WHITELIST~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINBOUND_WHITELIST~', ff($strTableNameINBOUND_WHITELIST), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINBOUND_WHITELIST~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INBOUND_WHITELIST", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_inbound_whitelist($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINBOUND_WHITELIST = getTableNameEntity("inbound_whitelist", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINBOUND_WHITELIST~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUND_WHITELIST~', ff($strTableNameINBOUND_WHITELIST), $strSQL);
...
function fetchValue_inbound_whitelist($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUND_WHITELIST = getTableNameEntity("inbound_whitelist", false);	

	$strWhere = buildWhere_inbound_whitelist($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINBOUND_WHITELIST~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUND_WHITELIST~', ff($strTableNameINBOUND_WHITELIST), $strSQL);
...
function buildWhere_inbound_whitelist($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_inbound_whitelist($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INBOUND_WHITELIST");
}