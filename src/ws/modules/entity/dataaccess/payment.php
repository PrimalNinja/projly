...
function add_payment($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_payment($objConn_a);
	return addbulk_payment($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_payment($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePAYMENT = getTableNameEntity("payment", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "payment");
	$strCode = "PAYMENT";
	$strDescription = "PAYMENT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffeea380fb-ab55-4443-bd6c-3ea1acfe9032";
    ...
    $strSQL = "insert into ~TABLENAMEPAYMENT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPAYMENT~', ff($strTableNamePAYMENT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PAYMENT", $strResult, $strJSONData);
...
function delete_payment($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENT = getTableNameEntity("payment", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_payment($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPAYMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENT~', ff($strTableNamePAYMENT), $strSQL);
...
function deleteExtension_payment($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTExtension = getTableNameEntityExtension("payment");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_payment($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPAYMENTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTEXTENSION~', ff($strTableNamePAYMENTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_payment($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENT = getTableNameEntity("payment", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPAYMENT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPAYMENT~', ff($strTableNamePAYMENT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPAYMENT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PAYMENT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_payment($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePAYMENT = getTableNameEntity("payment", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPAYMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENT~', ff($strTableNamePAYMENT), $strSQL);
...
function fetchValue_payment($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENT = getTableNameEntity("payment", false);	

	$strWhere = buildWhere_payment($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPAYMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENT~', ff($strTableNamePAYMENT), $strSQL);
...
function buildWhere_payment($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_payment($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PAYMENT");
}