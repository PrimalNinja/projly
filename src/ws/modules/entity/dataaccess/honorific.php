...
function add_honorific($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_honorific($objConn_a);
	return addbulk_honorific($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_honorific($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameHONORIFIC = getTableNameEntity("honorific", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "honorific");
	$strCode = "HONORIFIC";
	$strDescription = "HONORIFIC";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "f44c0d8d-dbc9-418b-9a64-3cf3fc7b2c4d";
    ...
    $strSQL = "insert into ~TABLENAMEHONORIFIC~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEHONORIFIC~', ff($strTableNameHONORIFIC), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "HONORIFIC", $strResult, $strJSONData);
...
function delete_honorific($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameHONORIFIC = getTableNameEntity("honorific", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_honorific($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEHONORIFIC~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEHONORIFIC~', ff($strTableNameHONORIFIC), $strSQL);
...
function deleteExtension_honorific($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameHONORIFICExtension = getTableNameEntityExtension("honorific");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_honorific($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEHONORIFICEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEHONORIFICEXTENSION~', ff($strTableNameHONORIFICExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_honorific($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameHONORIFIC = getTableNameEntity("honorific", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEHONORIFIC~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEHONORIFIC~', ff($strTableNameHONORIFIC), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEHONORIFIC~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "HONORIFIC", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_honorific($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameHONORIFIC = getTableNameEntity("honorific", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEHONORIFIC~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEHONORIFIC~', ff($strTableNameHONORIFIC), $strSQL);
...
function fetchValue_honorific($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameHONORIFIC = getTableNameEntity("honorific", false);	

	$strWhere = buildWhere_honorific($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEHONORIFIC~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEHONORIFIC~', ff($strTableNameHONORIFIC), $strSQL);
...
function buildWhere_honorific($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_honorific($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "HONORIFIC");
}