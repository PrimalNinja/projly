...
function add_projectcampaigntask($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectcampaigntask($objConn_a);
	return addbulk_projectcampaigntask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectcampaigntask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTCAMPAIGNTASK = getTableNameEntity("projectcampaigntask", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectcampaigntask");
	$strCode = "PROJECTCAMPAIGNTASK";
	$strDescription = "PROJECTCAMPAIGNTASK";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g4ec1b214-f01d-4c37-beda-aa6a8dbbf78c";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTCAMPAIGNTASK~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTASK~', ff($strTableNamePROJECTCAMPAIGNTASK), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGNTASK", $strResult, $strJSONData);
...
function delete_projectcampaigntask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTASK = getTableNameEntity("projectcampaigntask", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaigntask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGNTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTASK~', ff($strTableNamePROJECTCAMPAIGNTASK), $strSQL);
...
function deleteExtension_projectcampaigntask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTASKExtension = getTableNameEntityExtension("projectcampaigntask");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaigntask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGNTASKEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTASKEXTENSION~', ff($strTableNamePROJECTCAMPAIGNTASKExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectcampaigntask($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTASK = getTableNameEntity("projectcampaigntask", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTCAMPAIGNTASK~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTASK~', ff($strTableNamePROJECTCAMPAIGNTASK), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTCAMPAIGNTASK~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGNTASK", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectcampaigntask($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTCAMPAIGNTASK = getTableNameEntity("projectcampaigntask", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTCAMPAIGNTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTASK~', ff($strTableNamePROJECTCAMPAIGNTASK), $strSQL);
...
function fetchValue_projectcampaigntask($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTASK = getTableNameEntity("projectcampaigntask", false);	

	$strWhere = buildWhere_projectcampaigntask($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTCAMPAIGNTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTASK~', ff($strTableNamePROJECTCAMPAIGNTASK), $strSQL);
...
function buildWhere_projectcampaigntask($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectcampaigntask($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTCAMPAIGNTASK");
}