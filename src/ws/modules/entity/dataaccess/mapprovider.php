...
function add_mapprovider($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_mapprovider($objConn_a);
	return addbulk_mapprovider($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_mapprovider($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameMAPPROVIDER = getTableNameEntity("mapprovider", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "mapprovider");
	$strCode = "MAPPROVIDER";
	$strDescription = "MAPPROVIDER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ga1440578-9e84-4c54-88b8-56d5808e18e7";
    ...
    $strSQL = "insert into ~TABLENAMEMAPPROVIDER~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEMAPPROVIDER~', ff($strTableNameMAPPROVIDER), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MAPPROVIDER", $strResult, $strJSONData);
...
function delete_mapprovider($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMAPPROVIDER = getTableNameEntity("mapprovider", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_mapprovider($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMAPPROVIDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMAPPROVIDER~', ff($strTableNameMAPPROVIDER), $strSQL);
...
function deleteExtension_mapprovider($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMAPPROVIDERExtension = getTableNameEntityExtension("mapprovider");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_mapprovider($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMAPPROVIDEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMAPPROVIDEREXTENSION~', ff($strTableNameMAPPROVIDERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_mapprovider($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMAPPROVIDER = getTableNameEntity("mapprovider", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMAPPROVIDER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMAPPROVIDER~', ff($strTableNameMAPPROVIDER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMAPPROVIDER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MAPPROVIDER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_mapprovider($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMAPPROVIDER = getTableNameEntity("mapprovider", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMAPPROVIDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMAPPROVIDER~', ff($strTableNameMAPPROVIDER), $strSQL);
...
function fetchValue_mapprovider($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMAPPROVIDER = getTableNameEntity("mapprovider", false);	

	$strWhere = buildWhere_mapprovider($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMAPPROVIDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMAPPROVIDER~', ff($strTableNameMAPPROVIDER), $strSQL);
...
function buildWhere_mapprovider($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_mapprovider($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MAPPROVIDER");
}