...
function add_softwarelicence($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_softwarelicence($objConn_a);
	return addbulk_softwarelicence($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_softwarelicence($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSOFTWARELICENCE = getTableNameEntity("softwarelicence", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "softwarelicence");
	$strCode = "SOFTWARELICENCE";
	$strDescription = "SOFTWARELICENCE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g3a39f0ae-ef99-4f73-bd8f-a31eaf2a1bc8";
    ...
    $strSQL = "insert into ~TABLENAMESOFTWARELICENCE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCE~', ff($strTableNameSOFTWARELICENCE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SOFTWARELICENCE", $strResult, $strJSONData);
...
function delete_softwarelicence($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCE = getTableNameEntity("softwarelicence", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_softwarelicence($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESOFTWARELICENCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCE~', ff($strTableNameSOFTWARELICENCE), $strSQL);
...
function deleteExtension_softwarelicence($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCEExtension = getTableNameEntityExtension("softwarelicence");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_softwarelicence($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESOFTWARELICENCEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCEEXTENSION~', ff($strTableNameSOFTWARELICENCEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_softwarelicence($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCE = getTableNameEntity("softwarelicence", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESOFTWARELICENCE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESOFTWARELICENCE~', ff($strTableNameSOFTWARELICENCE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESOFTWARELICENCE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SOFTWARELICENCE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_softwarelicence($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSOFTWARELICENCE = getTableNameEntity("softwarelicence", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESOFTWARELICENCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCE~', ff($strTableNameSOFTWARELICENCE), $strSQL);
...
function fetchValue_softwarelicence($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCE = getTableNameEntity("softwarelicence", false);	

	$strWhere = buildWhere_softwarelicence($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESOFTWARELICENCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCE~', ff($strTableNameSOFTWARELICENCE), $strSQL);
...
function buildWhere_softwarelicence($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_softwarelicence($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SOFTWARELICENCE");
}