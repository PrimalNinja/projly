...
function add_layout_formsection($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_layout_formsection($objConn_a);
	return addbulk_layout_formsection($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_layout_formsection($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameLAYOUT_FORMSECTION = getTableNameEntity("layout_formsection", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "layout_formsection");
	$strCode = "LAYOUT_FORMSECTION";
	$strDescription = "LAYOUT_FORMSECTION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g85af610e-698b-4353-bb44-5bf56c91eadf";
    ...
    $strSQL = "insert into ~TABLENAMELAYOUT_FORMSECTION~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMELAYOUT_FORMSECTION~', ff($strTableNameLAYOUT_FORMSECTION), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "LAYOUT_FORMSECTION", $strResult, $strJSONData);
...
function delete_layout_formsection($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_FORMSECTION = getTableNameEntity("layout_formsection", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_layout_formsection($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMELAYOUT_FORMSECTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_FORMSECTION~', ff($strTableNameLAYOUT_FORMSECTION), $strSQL);
...
function deleteExtension_layout_formsection($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_FORMSECTIONExtension = getTableNameEntityExtension("layout_formsection");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_layout_formsection($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMELAYOUT_FORMSECTIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_FORMSECTIONEXTENSION~', ff($strTableNameLAYOUT_FORMSECTIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_layout_formsection($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_FORMSECTION = getTableNameEntity("layout_formsection", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMELAYOUT_FORMSECTION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMELAYOUT_FORMSECTION~', ff($strTableNameLAYOUT_FORMSECTION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMELAYOUT_FORMSECTION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "LAYOUT_FORMSECTION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_layout_formsection($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameLAYOUT_FORMSECTION = getTableNameEntity("layout_formsection", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMELAYOUT_FORMSECTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_FORMSECTION~', ff($strTableNameLAYOUT_FORMSECTION), $strSQL);
...
function fetchValue_layout_formsection($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_FORMSECTION = getTableNameEntity("layout_formsection", false);	

	$strWhere = buildWhere_layout_formsection($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMELAYOUT_FORMSECTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_FORMSECTION~', ff($strTableNameLAYOUT_FORMSECTION), $strSQL);
...
function buildWhere_layout_formsection($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_layout_formsection($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "LAYOUT_FORMSECTION");
}