...
function add_systemmodule_entity($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemmodule_entity($objConn_a);
	return addbulk_systemmodule_entity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemmodule_entity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSYSTEMMODULE_ENTITY = getTableNameEntity("systemmodule_entity", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "systemmodule_entity");
	$strCode = "SYSTEMMODULE_ENTITY";
	$strDescription = "SYSTEMMODULE_ENTITY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g4f6f774a-fe22-46c1-819b-ad161593721b";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMMODULE_ENTITY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_ENTITY~', ff($strTableNameSYSTEMMODULE_ENTITY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULE_ENTITY", $strResult, $strJSONData);
...
function delete_systemmodule_entity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_ENTITY = getTableNameEntity("systemmodule_entity", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmodule_entity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULE_ENTITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_ENTITY~', ff($strTableNameSYSTEMMODULE_ENTITY), $strSQL);
...
function deleteExtension_systemmodule_entity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_ENTITYExtension = getTableNameEntityExtension("systemmodule_entity");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmodule_entity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULE_ENTITYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_ENTITYEXTENSION~', ff($strTableNameSYSTEMMODULE_ENTITYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemmodule_entity($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_ENTITY = getTableNameEntity("systemmodule_entity", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMMODULE_ENTITY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMMODULE_ENTITY~', ff($strTableNameSYSTEMMODULE_ENTITY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMMODULE_ENTITY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULE_ENTITY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemmodule_entity($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMMODULE_ENTITY = getTableNameEntity("systemmodule_entity", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMMODULE_ENTITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_ENTITY~', ff($strTableNameSYSTEMMODULE_ENTITY), $strSQL);
...
function fetchValue_systemmodule_entity($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE_ENTITY = getTableNameEntity("systemmodule_entity", false);	

	$strWhere = buildWhere_systemmodule_entity($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMMODULE_ENTITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE_ENTITY~', ff($strTableNameSYSTEMMODULE_ENTITY), $strSQL);
...
function buildWhere_systemmodule_entity($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemmodule_entity($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMMODULE_ENTITY");
}