...
function add_formdatatype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_formdatatype($objConn_a);
	return addbulk_formdatatype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_formdatatype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFORMDATATYPE = getTableNameEntity("formdatatype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "formdatatype");
	$strCode = "FORMDATATYPE";
	$strDescription = "FORMDATATYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g1639f36b-f6e9-415c-8b6b-96d684f1bebc";
    ...
    $strSQL = "insert into ~TABLENAMEFORMDATATYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE~', ff($strTableNameFORMDATATYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FORMDATATYPE", $strResult, $strJSONData);
...
function delete_formdatatype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPE = getTableNameEntity("formdatatype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formdatatype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMDATATYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE~', ff($strTableNameFORMDATATYPE), $strSQL);
...
function deleteExtension_formdatatype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPEExtension = getTableNameEntityExtension("formdatatype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formdatatype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMDATATYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPEEXTENSION~', ff($strTableNameFORMDATATYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_formdatatype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPE = getTableNameEntity("formdatatype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFORMDATATYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFORMDATATYPE~', ff($strTableNameFORMDATATYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFORMDATATYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FORMDATATYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_formdatatype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFORMDATATYPE = getTableNameEntity("formdatatype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFORMDATATYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE~', ff($strTableNameFORMDATATYPE), $strSQL);
...
function fetchValue_formdatatype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPE = getTableNameEntity("formdatatype", false);	

	$strWhere = buildWhere_formdatatype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFORMDATATYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE~', ff($strTableNameFORMDATATYPE), $strSQL);
...
function buildWhere_formdatatype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_formdatatype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FORMDATATYPE");
}