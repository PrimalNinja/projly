...
function add_paymentmethod($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_paymentmethod($objConn_a);
	return addbulk_paymentmethod($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_paymentmethod($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "paymentmethod");
	$strCode = "PAYMENTMETHOD";
	$strDescription = "PAYMENTMETHOD";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffb775d2a9-59b7-453b-91ce-7621548ffe81";
    ...
    $strSQL = "insert into ~TABLENAMEPAYMENTMETHOD~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PAYMENTMETHOD", $strResult, $strJSONData);
...
function delete_paymentmethod($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTMETHOD = getTableNameEntity("paymentmethod", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_paymentmethod($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPAYMENTMETHOD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTMETHOD~', ff($strTableNamePAYMENTMETHOD), $strSQL);
...
function deleteExtension_paymentmethod($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTMETHODExtension = getTableNameEntityExtension("paymentmethod");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_paymentmethod($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPAYMENTMETHODEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTMETHODEXTENSION~', ff($strTableNamePAYMENTMETHODExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_paymentmethod($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTMETHOD = getTableNameEntity("paymentmethod", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPAYMENTMETHOD~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPAYMENTMETHOD~', ff($strTableNamePAYMENTMETHOD), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPAYMENTMETHOD~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PAYMENTMETHOD", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_paymentmethod($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePAYMENTMETHOD = getTableNameEntity("paymentmethod", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPAYMENTMETHOD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTMETHOD~', ff($strTableNamePAYMENTMETHOD), $strSQL);
...
function fetchValue_paymentmethod($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePAYMENTMETHOD = getTableNameEntity("paymentmethod", false);	

	$strWhere = buildWhere_paymentmethod($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPAYMENTMETHOD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPAYMENTMETHOD~', ff($strTableNamePAYMENTMETHOD), $strSQL);
...
function buildWhere_paymentmethod($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_paymentmethod($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PAYMENTMETHOD");
}