...
function add_securitydashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_securitydashboard_chart($objConn_a);
	return addbulk_securitydashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_securitydashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSECURITYDASHBOARD_CHART = getTableNameEntity("securitydashboard_chart", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "securitydashboard_chart");
	$strCode = "SECURITYDASHBOARD_CHART";
	$strDescription = "SECURITYDASHBOARD_CHART";
    ...
    $strSQL = "insert into ~TABLENAMESECURITYDASHBOARD_CHART~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESECURITYDASHBOARD_CHART~', ff($strTableNameSECURITYDASHBOARD_CHART), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SECURITYDASHBOARD_CHART", $strResult, $strJSONData);
...
function delete_securitydashboard_chart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYDASHBOARD_CHART = getTableNameEntity("securitydashboard_chart", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_securitydashboard_chart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESECURITYDASHBOARD_CHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYDASHBOARD_CHART~', ff($strTableNameSECURITYDASHBOARD_CHART), $strSQL);
...
function deleteExtension_securitydashboard_chart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYDASHBOARD_CHARTExtension = getTableNameEntityExtension("securitydashboard_chart");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_securitydashboard_chart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESECURITYDASHBOARD_CHARTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYDASHBOARD_CHARTEXTENSION~', ff($strTableNameSECURITYDASHBOARD_CHARTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_securitydashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYDASHBOARD_CHART = getTableNameEntity("securitydashboard_chart", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESECURITYDASHBOARD_CHART~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESECURITYDASHBOARD_CHART~', ff($strTableNameSECURITYDASHBOARD_CHART), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESECURITYDASHBOARD_CHART~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SECURITYDASHBOARD_CHART", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_securitydashboard_chart($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSECURITYDASHBOARD_CHART = getTableNameEntity("securitydashboard_chart", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESECURITYDASHBOARD_CHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYDASHBOARD_CHART~', ff($strTableNameSECURITYDASHBOARD_CHART), $strSQL);
...
function fetchValue_securitydashboard_chart($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYDASHBOARD_CHART = getTableNameEntity("securitydashboard_chart", false);	

	$strWhere = buildWhere_securitydashboard_chart($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESECURITYDASHBOARD_CHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYDASHBOARD_CHART~', ff($strTableNameSECURITYDASHBOARD_CHART), $strSQL);
...
function buildWhere_securitydashboard_chart($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_securitydashboard_chart($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SECURITYDASHBOARD_CHART");
}