...
function add_permission($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_permission($objConn_a);
	return addbulk_permission($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_permission($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "0000eae3-e5b8-4ebb-a3a8-50220cee15d5";
    ...
    $strSQL = "insert into ~TABLENAMEPERMISSION~ (~FIELDS~) values (~VALUES~)";
...
function delete_permission($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSION = getTableNameEntity("permission", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_permission($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPERMISSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSION~', ff($strTableNamePERMISSION), $strSQL);
...
function deleteExtension_permission($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSIONExtension = getTableNameEntityExtension("permission");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_permission($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPERMISSIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSIONEXTENSION~', ff($strTableNamePERMISSIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_permission($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSION = getTableNameEntity("permission", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPERMISSION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPERMISSION~', ff($strTableNamePERMISSION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPERMISSION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PERMISSION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_permission($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePERMISSION = getTableNameEntity("permission", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPERMISSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSION~', ff($strTableNamePERMISSION), $strSQL);
...
function fetchValue_permission($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSION = getTableNameEntity("permission", false);	

	$strWhere = buildWhere_permission($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPERMISSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSION~', ff($strTableNamePERMISSION), $strSQL);
...
function buildWhere_permission($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_permission($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PERMISSION");
}