...
function add_projectpriority($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectpriority($objConn_a);
	return addbulk_projectpriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectpriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTPRIORITY = getTableNameEntity("projectpriority", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectpriority");
	$strCode = "PROJECTPRIORITY";
	$strDescription = "PROJECTPRIORITY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g138df671-161e-4dc9-a518-58a284695503";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTPRIORITY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTPRIORITY~', ff($strTableNamePROJECTPRIORITY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPRIORITY", $strResult, $strJSONData);
...
function delete_projectpriority($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPRIORITY = getTableNameEntity("projectpriority", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectpriority($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPRIORITY~', ff($strTableNamePROJECTPRIORITY), $strSQL);
...
function deleteExtension_projectpriority($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPRIORITYExtension = getTableNameEntityExtension("projectpriority");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectpriority($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPRIORITYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPRIORITYEXTENSION~', ff($strTableNamePROJECTPRIORITYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectpriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPRIORITY = getTableNameEntity("projectpriority", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTPRIORITY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTPRIORITY~', ff($strTableNamePROJECTPRIORITY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTPRIORITY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPRIORITY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectpriority($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTPRIORITY = getTableNameEntity("projectpriority", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPRIORITY~', ff($strTableNamePROJECTPRIORITY), $strSQL);
...
function fetchValue_projectpriority($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPRIORITY = getTableNameEntity("projectpriority", false);	

	$strWhere = buildWhere_projectpriority($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPRIORITY~', ff($strTableNamePROJECTPRIORITY), $strSQL);
...
function buildWhere_projectpriority($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectpriority($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTPRIORITY");
}