...
function add_authenticationtype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_authenticationtype($objConn_a);
	return addbulk_authenticationtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_authenticationtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "authenticationtype");
	$strCode = "AUTHENTICATIONTYPE";
	$strDescription = "AUTHENTICATIONTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g020beb65-e230-47eb-894f-4c5941335e41";
    ...
    $strSQL = "insert into ~TABLENAMEAUTHENTICATIONTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "AUTHENTICATIONTYPE", $strResult, $strJSONData);
...
function delete_authenticationtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONTYPE = getTableNameEntity("authenticationtype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_authenticationtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAUTHENTICATIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONTYPE~', ff($strTableNameAUTHENTICATIONTYPE), $strSQL);
...
function deleteExtension_authenticationtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONTYPEExtension = getTableNameEntityExtension("authenticationtype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_authenticationtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAUTHENTICATIONTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONTYPEEXTENSION~', ff($strTableNameAUTHENTICATIONTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_authenticationtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONTYPE = getTableNameEntity("authenticationtype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAUTHENTICATIONTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAUTHENTICATIONTYPE~', ff($strTableNameAUTHENTICATIONTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAUTHENTICATIONTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "AUTHENTICATIONTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_authenticationtype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAUTHENTICATIONTYPE = getTableNameEntity("authenticationtype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAUTHENTICATIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONTYPE~', ff($strTableNameAUTHENTICATIONTYPE), $strSQL);
...
function fetchValue_authenticationtype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONTYPE = getTableNameEntity("authenticationtype", false);	

	$strWhere = buildWhere_authenticationtype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAUTHENTICATIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONTYPE~', ff($strTableNameAUTHENTICATIONTYPE), $strSQL);
...
function buildWhere_authenticationtype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_authenticationtype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "AUTHENTICATIONTYPE");
}