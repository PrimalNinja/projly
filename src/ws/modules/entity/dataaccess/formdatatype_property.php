...
function add_formdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_formdatatype_property($objConn_a);
	return addbulk_formdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_formdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFORMDATATYPE_PROPERTY = getTableNameEntity("formdatatype_property", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "formdatatype_property");
	$strCode = "FORMDATATYPE_PROPERTY";
	$strDescription = "FORMDATATYPE_PROPERTY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g937f8b88-eca3-447b-942c-9fd3e8926e83";
    ...
    $strSQL = "insert into ~TABLENAMEFORMDATATYPE_PROPERTY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE_PROPERTY~', ff($strTableNameFORMDATATYPE_PROPERTY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FORMDATATYPE_PROPERTY", $strResult, $strJSONData);
...
function delete_formdatatype_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPE_PROPERTY = getTableNameEntity("formdatatype_property", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formdatatype_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMDATATYPE_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE_PROPERTY~', ff($strTableNameFORMDATATYPE_PROPERTY), $strSQL);
...
function deleteExtension_formdatatype_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPE_PROPERTYExtension = getTableNameEntityExtension("formdatatype_property");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formdatatype_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMDATATYPE_PROPERTYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE_PROPERTYEXTENSION~', ff($strTableNameFORMDATATYPE_PROPERTYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_formdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPE_PROPERTY = getTableNameEntity("formdatatype_property", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFORMDATATYPE_PROPERTY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFORMDATATYPE_PROPERTY~', ff($strTableNameFORMDATATYPE_PROPERTY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFORMDATATYPE_PROPERTY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FORMDATATYPE_PROPERTY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_formdatatype_property($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFORMDATATYPE_PROPERTY = getTableNameEntity("formdatatype_property", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFORMDATATYPE_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE_PROPERTY~', ff($strTableNameFORMDATATYPE_PROPERTY), $strSQL);
...
function fetchValue_formdatatype_property($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMDATATYPE_PROPERTY = getTableNameEntity("formdatatype_property", false);	

	$strWhere = buildWhere_formdatatype_property($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFORMDATATYPE_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMDATATYPE_PROPERTY~', ff($strTableNameFORMDATATYPE_PROPERTY), $strSQL);
...
function buildWhere_formdatatype_property($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_formdatatype_property($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FORMDATATYPE_PROPERTY");
}