...
function add_mobileform($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_mobileform($objConn_a);
	return addbulk_mobileform($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_mobileform($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "mobileform");
	$strCode = "MOBILEFORM";
	$strDescription = "MOBILEFORM";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g1d033c60-b8bd-4925-97ab-41efd975e746";
    ...
    $strSQL = "insert into ~TABLENAMEMOBILEFORM~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MOBILEFORM", $strResult, $strJSONData);
...
function delete_mobileform($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMOBILEFORM = getTableNameEntity("mobileform", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_mobileform($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMOBILEFORM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMOBILEFORM~', ff($strTableNameMOBILEFORM), $strSQL);
...
function deleteExtension_mobileform($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMOBILEFORMExtension = getTableNameEntityExtension("mobileform");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_mobileform($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMOBILEFORMEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMOBILEFORMEXTENSION~', ff($strTableNameMOBILEFORMExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_mobileform($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMOBILEFORM = getTableNameEntity("mobileform", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMOBILEFORM~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMOBILEFORM~', ff($strTableNameMOBILEFORM), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMOBILEFORM~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MOBILEFORM", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_mobileform($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMOBILEFORM = getTableNameEntity("mobileform", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMOBILEFORM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMOBILEFORM~', ff($strTableNameMOBILEFORM), $strSQL);
...
function fetchValue_mobileform($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMOBILEFORM = getTableNameEntity("mobileform", false);	

	$strWhere = buildWhere_mobileform($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMOBILEFORM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMOBILEFORM~', ff($strTableNameMOBILEFORM), $strSQL);
...
function buildWhere_mobileform($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_mobileform($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MOBILEFORM");
}