...
function add_integrationinboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationinboundtype($objConn_a);
	return addbulk_integrationinboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationinboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "integrationinboundtype");
	$strCode = "INTEGRATIONINBOUNDTYPE";
	$strDescription = "INTEGRATIONINBOUNDTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g6e6314de-2b08-4dac-90fc-9ecdab41fefc";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONINBOUNDTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONINBOUNDTYPE", $strResult, $strJSONData);
...
function delete_integrationinboundtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDTYPE = getTableNameEntity("integrationinboundtype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationinboundtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONINBOUNDTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDTYPE~', ff($strTableNameINTEGRATIONINBOUNDTYPE), $strSQL);
...
function deleteExtension_integrationinboundtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDTYPEExtension = getTableNameEntityExtension("integrationinboundtype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationinboundtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONINBOUNDTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDTYPEEXTENSION~', ff($strTableNameINTEGRATIONINBOUNDTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationinboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDTYPE = getTableNameEntity("integrationinboundtype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONINBOUNDTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDTYPE~', ff($strTableNameINTEGRATIONINBOUNDTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONINBOUNDTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONINBOUNDTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationinboundtype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONINBOUNDTYPE = getTableNameEntity("integrationinboundtype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONINBOUNDTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDTYPE~', ff($strTableNameINTEGRATIONINBOUNDTYPE), $strSQL);
...
function fetchValue_integrationinboundtype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDTYPE = getTableNameEntity("integrationinboundtype", false);	

	$strWhere = buildWhere_integrationinboundtype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONINBOUNDTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDTYPE~', ff($strTableNameINTEGRATIONINBOUNDTYPE), $strSQL);
...
function buildWhere_integrationinboundtype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationinboundtype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONINBOUNDTYPE");
}