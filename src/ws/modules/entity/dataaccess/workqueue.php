...
function add_workqueue($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_workqueue($objConn_a);
	return addbulk_workqueue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_workqueue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "workqueue");
	$strCode = "WORKQUEUE";
	$strDescription = "WORKQUEUE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "WORKQUEUEITEMHEADER";
    ...
    $strSQL = "insert into ~TABLENAMEWORKQUEUE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "WORKQUEUE", $strResult, $strJSONData);
...
function delete_workqueue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUE = getTableNameEntity("workqueue", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_workqueue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEWORKQUEUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUE~', ff($strTableNameWORKQUEUE), $strSQL);
...
function deleteExtension_workqueue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUEExtension = getTableNameEntityExtension("workqueue");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_workqueue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEWORKQUEUEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUEEXTENSION~', ff($strTableNameWORKQUEUEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_workqueue($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUE = getTableNameEntity("workqueue", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEWORKQUEUE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEWORKQUEUE~', ff($strTableNameWORKQUEUE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEWORKQUEUE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "WORKQUEUE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_workqueue($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameWORKQUEUE = getTableNameEntity("workqueue", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEWORKQUEUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUE~', ff($strTableNameWORKQUEUE), $strSQL);
...
function fetchValue_workqueue($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUE = getTableNameEntity("workqueue", false);	

	$strWhere = buildWhere_workqueue($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEWORKQUEUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUE~', ff($strTableNameWORKQUEUE), $strSQL);
...
function buildWhere_workqueue($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_workqueue($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "WORKQUEUE");
}