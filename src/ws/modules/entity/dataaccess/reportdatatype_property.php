...
function add_reportdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_reportdatatype_property($objConn_a);
	return addbulk_reportdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_reportdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameREPORTDATATYPE_PROPERTY = getTableNameEntity("reportdatatype_property", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "reportdatatype_property");
	$strCode = "REPORTDATATYPE_PROPERTY";
	$strDescription = "REPORTDATATYPE_PROPERTY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g937f8b88-eca3-447b-942c-9fd3e8926e83r";
    ...
    $strSQL = "insert into ~TABLENAMEREPORTDATATYPE_PROPERTY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEREPORTDATATYPE_PROPERTY~', ff($strTableNameREPORTDATATYPE_PROPERTY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTDATATYPE_PROPERTY", $strResult, $strJSONData);
...
function delete_reportdatatype_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTDATATYPE_PROPERTY = getTableNameEntity("reportdatatype_property", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportdatatype_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTDATATYPE_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTDATATYPE_PROPERTY~', ff($strTableNameREPORTDATATYPE_PROPERTY), $strSQL);
...
function deleteExtension_reportdatatype_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTDATATYPE_PROPERTYExtension = getTableNameEntityExtension("reportdatatype_property");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportdatatype_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTDATATYPE_PROPERTYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTDATATYPE_PROPERTYEXTENSION~', ff($strTableNameREPORTDATATYPE_PROPERTYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_reportdatatype_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTDATATYPE_PROPERTY = getTableNameEntity("reportdatatype_property", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREPORTDATATYPE_PROPERTY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREPORTDATATYPE_PROPERTY~', ff($strTableNameREPORTDATATYPE_PROPERTY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREPORTDATATYPE_PROPERTY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTDATATYPE_PROPERTY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_reportdatatype_property($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREPORTDATATYPE_PROPERTY = getTableNameEntity("reportdatatype_property", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREPORTDATATYPE_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTDATATYPE_PROPERTY~', ff($strTableNameREPORTDATATYPE_PROPERTY), $strSQL);
...
function fetchValue_reportdatatype_property($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTDATATYPE_PROPERTY = getTableNameEntity("reportdatatype_property", false);	

	$strWhere = buildWhere_reportdatatype_property($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREPORTDATATYPE_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTDATATYPE_PROPERTY~', ff($strTableNameREPORTDATATYPE_PROPERTY), $strSQL);
...
function buildWhere_reportdatatype_property($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_reportdatatype_property($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REPORTDATATYPE_PROPERTY");
}