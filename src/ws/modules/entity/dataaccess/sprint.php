...
function add_sprint($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_sprint($objConn_a);
	return addbulk_sprint($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_sprint($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSPRINT = getTableNameEntity("sprint", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "sprint");
	$strCode = "SPRINT";
	$strDescription = "SPRINT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ga627d4c2-b700-48ab-8722-e19c42b41036";
    ...
    $strSQL = "insert into ~TABLENAMESPRINT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESPRINT~', ff($strTableNameSPRINT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SPRINT", $strResult, $strJSONData);
...
function delete_sprint($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSPRINT = getTableNameEntity("sprint", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_sprint($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESPRINT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESPRINT~', ff($strTableNameSPRINT), $strSQL);
...
function deleteExtension_sprint($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSPRINTExtension = getTableNameEntityExtension("sprint");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_sprint($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESPRINTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESPRINTEXTENSION~', ff($strTableNameSPRINTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_sprint($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSPRINT = getTableNameEntity("sprint", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESPRINT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESPRINT~', ff($strTableNameSPRINT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESPRINT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SPRINT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_sprint($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSPRINT = getTableNameEntity("sprint", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESPRINT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESPRINT~', ff($strTableNameSPRINT), $strSQL);
...
function fetchValue_sprint($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSPRINT = getTableNameEntity("sprint", false);	

	$strWhere = buildWhere_sprint($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESPRINT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESPRINT~', ff($strTableNameSPRINT), $strSQL);
...
function buildWhere_sprint($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_sprint($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SPRINT");
}