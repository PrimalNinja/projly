...
function add_sequence($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_sequence($objConn_a);
	return addbulk_sequence($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_sequence($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "sequence");
	$strCode = "SEQUENCE";
	$strDescription = "SEQUENCE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g9a1757b7-1a28-408e-8f6f-201a26e9b9ac";
    ...
    $strSQL = "insert into ~TABLENAMESEQUENCE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SEQUENCE", $strResult, $strJSONData);
...
function delete_sequence($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCE = getTableNameEntity("sequence", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_sequence($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESEQUENCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCE~', ff($strTableNameSEQUENCE), $strSQL);
...
function deleteExtension_sequence($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCEExtension = getTableNameEntityExtension("sequence");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_sequence($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESEQUENCEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCEEXTENSION~', ff($strTableNameSEQUENCEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_sequence($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCE = getTableNameEntity("sequence", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESEQUENCE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESEQUENCE~', ff($strTableNameSEQUENCE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESEQUENCE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SEQUENCE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_sequence($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSEQUENCE = getTableNameEntity("sequence", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESEQUENCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCE~', ff($strTableNameSEQUENCE), $strSQL);
...
function fetchValue_sequence($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCE = getTableNameEntity("sequence", false);	

	$strWhere = buildWhere_sequence($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESEQUENCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCE~', ff($strTableNameSEQUENCE), $strSQL);
...
function buildWhere_sequence($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_sequence($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SEQUENCE");
}