...
function add_securitychart($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_securitychart($objConn_a);
	return addbulk_securitychart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_securitychart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSECURITYCHART = getTableNameEntity("securitychart", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "securitychart");
	$strCode = "SECURITYCHART";
	$strDescription = "SECURITYCHART";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gbded9f88-7284-4065-a7ed-2067f04bcc4d";
    ...
    $strSQL = "insert into ~TABLENAMESECURITYCHART~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESECURITYCHART~', ff($strTableNameSECURITYCHART), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SECURITYCHART", $strResult, $strJSONData);
...
function delete_securitychart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYCHART = getTableNameEntity("securitychart", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_securitychart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESECURITYCHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYCHART~', ff($strTableNameSECURITYCHART), $strSQL);
...
function deleteExtension_securitychart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYCHARTExtension = getTableNameEntityExtension("securitychart");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_securitychart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESECURITYCHARTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYCHARTEXTENSION~', ff($strTableNameSECURITYCHARTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_securitychart($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYCHART = getTableNameEntity("securitychart", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESECURITYCHART~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESECURITYCHART~', ff($strTableNameSECURITYCHART), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESECURITYCHART~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SECURITYCHART", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_securitychart($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSECURITYCHART = getTableNameEntity("securitychart", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESECURITYCHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYCHART~', ff($strTableNameSECURITYCHART), $strSQL);
...
function fetchValue_securitychart($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSECURITYCHART = getTableNameEntity("securitychart", false);	

	$strWhere = buildWhere_securitychart($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESECURITYCHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESECURITYCHART~', ff($strTableNameSECURITYCHART), $strSQL);
...
function buildWhere_securitychart($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_securitychart($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SECURITYCHART");
}