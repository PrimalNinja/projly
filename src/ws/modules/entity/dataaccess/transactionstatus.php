...
function add_transactionstatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_transactionstatus($objConn_a);
	return addbulk_transactionstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_transactionstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "transactionstatus");
	$strCode = "TRANSACTIONSTATUS";
	$strDescription = "TRANSACTIONSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g5af50eef-305c-454a-8800-b2b3b98d497e";
    ...
    $strSQL = "insert into ~TABLENAMETRANSACTIONSTATUS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONSTATUS", $strResult, $strJSONData);
...
function delete_transactionstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONSTATUS = getTableNameEntity("transactionstatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONSTATUS~', ff($strTableNameTRANSACTIONSTATUS), $strSQL);
...
function deleteExtension_transactionstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONSTATUSExtension = getTableNameEntityExtension("transactionstatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONSTATUSEXTENSION~', ff($strTableNameTRANSACTIONSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_transactionstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONSTATUS = getTableNameEntity("transactionstatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETRANSACTIONSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETRANSACTIONSTATUS~', ff($strTableNameTRANSACTIONSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETRANSACTIONSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_transactionstatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTRANSACTIONSTATUS = getTableNameEntity("transactionstatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETRANSACTIONSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONSTATUS~', ff($strTableNameTRANSACTIONSTATUS), $strSQL);
...
function fetchValue_transactionstatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONSTATUS = getTableNameEntity("transactionstatus", false);	

	$strWhere = buildWhere_transactionstatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETRANSACTIONSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONSTATUS~', ff($strTableNameTRANSACTIONSTATUS), $strSQL);
...
function buildWhere_transactionstatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_transactionstatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TRANSACTIONSTATUS");
}