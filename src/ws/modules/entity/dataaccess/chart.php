...
function add_chart($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_chart($objConn_a);
	return addbulk_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCHART = getTableNameEntity("chart", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "chart");
	$strCode = "CHART";
	$strDescription = "CHART";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g30930604-fe31-4d45-9b7d-179655de65a0";
    ...
    $strSQL = "insert into ~TABLENAMECHART~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECHART~', ff($strTableNameCHART), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CHART", $strResult, $strJSONData);
...
function delete_chart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHART = getTableNameEntity("chart", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_chart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHART~', ff($strTableNameCHART), $strSQL);
...
function deleteExtension_chart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHARTExtension = getTableNameEntityExtension("chart");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_chart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECHARTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHARTEXTENSION~', ff($strTableNameCHARTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHART = getTableNameEntity("chart", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECHART~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECHART~', ff($strTableNameCHART), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECHART~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CHART", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_chart($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCHART = getTableNameEntity("chart", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHART~', ff($strTableNameCHART), $strSQL);
...
function fetchValue_chart($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHART = getTableNameEntity("chart", false);	

	$strWhere = buildWhere_chart($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHART~', ff($strTableNameCHART), $strSQL);
...
function buildWhere_chart($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_chart($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CHART");
}