...
function add_smtest($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_smtest($objConn_a);
	return addbulk_smtest($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_smtest($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "smtest");
	$strCode = "SMTEST";
	$strDescription = "SMTEST";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gf0e9753a-a982-4dbe-9fa4-1f52580dd5f9";
    ...
    $strSQL = "insert into ~TABLENAMESMTEST~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SMTEST", $strResult, $strJSONData);
...
function delete_smtest($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMTEST = getTableNameEntity("smtest", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smtest($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMTEST~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMTEST~', ff($strTableNameSMTEST), $strSQL);
...
function deleteExtension_smtest($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMTESTExtension = getTableNameEntityExtension("smtest");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smtest($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMTESTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMTESTEXTENSION~', ff($strTableNameSMTESTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_smtest($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMTEST = getTableNameEntity("smtest", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESMTEST~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESMTEST~', ff($strTableNameSMTEST), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESMTEST~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SMTEST", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_smtest($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSMTEST = getTableNameEntity("smtest", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESMTEST~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMTEST~', ff($strTableNameSMTEST), $strSQL);
...
function fetchValue_smtest($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMTEST = getTableNameEntity("smtest", false);	

	$strWhere = buildWhere_smtest($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESMTEST~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMTEST~', ff($strTableNameSMTEST), $strSQL);
...
function buildWhere_smtest($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_smtest($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SMTEST");
}