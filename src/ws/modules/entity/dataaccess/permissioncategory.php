...
function add_permissioncategory($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_permissioncategory($objConn_a);
	return addbulk_permissioncategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_permissioncategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "permissioncategory");
	$strCode = "PERMISSIONCATEGORY";
	$strDescription = "PERMISSIONCATEGORY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "f8b3c85a6-ea90-4610-be1c-f8945f809def";
    ...
    $strSQL = "insert into ~TABLENAMEPERMISSIONCATEGORY~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PERMISSIONCATEGORY", $strResult, $strJSONData);
...
function delete_permissioncategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSIONCATEGORY = getTableNameEntity("permissioncategory", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_permissioncategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPERMISSIONCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSIONCATEGORY~', ff($strTableNamePERMISSIONCATEGORY), $strSQL);
...
function deleteExtension_permissioncategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSIONCATEGORYExtension = getTableNameEntityExtension("permissioncategory");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_permissioncategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPERMISSIONCATEGORYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSIONCATEGORYEXTENSION~', ff($strTableNamePERMISSIONCATEGORYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_permissioncategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSIONCATEGORY = getTableNameEntity("permissioncategory", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPERMISSIONCATEGORY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPERMISSIONCATEGORY~', ff($strTableNamePERMISSIONCATEGORY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPERMISSIONCATEGORY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PERMISSIONCATEGORY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_permissioncategory($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePERMISSIONCATEGORY = getTableNameEntity("permissioncategory", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPERMISSIONCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSIONCATEGORY~', ff($strTableNamePERMISSIONCATEGORY), $strSQL);
...
function fetchValue_permissioncategory($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePERMISSIONCATEGORY = getTableNameEntity("permissioncategory", false);	

	$strWhere = buildWhere_permissioncategory($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPERMISSIONCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPERMISSIONCATEGORY~', ff($strTableNamePERMISSIONCATEGORY), $strSQL);
...
function buildWhere_permissioncategory($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_permissioncategory($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PERMISSIONCATEGORY");
}