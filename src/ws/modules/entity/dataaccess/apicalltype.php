...
function add_apicalltype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_apicalltype($objConn_a);
	return addbulk_apicalltype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_apicalltype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "apicalltype");
	$strCode = "APICALLTYPE";
	$strDescription = "APICALLTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gc015151c-199a-4013-a494-e7bebcef0c1d";
    ...
    $strSQL = "insert into ~TABLENAMEAPICALLTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "APICALLTYPE", $strResult, $strJSONData);
...
function delete_apicalltype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALLTYPE = getTableNameEntity("apicalltype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_apicalltype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPICALLTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALLTYPE~', ff($strTableNameAPICALLTYPE), $strSQL);
...
function deleteExtension_apicalltype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALLTYPEExtension = getTableNameEntityExtension("apicalltype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_apicalltype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPICALLTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALLTYPEEXTENSION~', ff($strTableNameAPICALLTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_apicalltype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALLTYPE = getTableNameEntity("apicalltype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAPICALLTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAPICALLTYPE~', ff($strTableNameAPICALLTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAPICALLTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "APICALLTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_apicalltype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAPICALLTYPE = getTableNameEntity("apicalltype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAPICALLTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALLTYPE~', ff($strTableNameAPICALLTYPE), $strSQL);
...
function fetchValue_apicalltype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALLTYPE = getTableNameEntity("apicalltype", false);	

	$strWhere = buildWhere_apicalltype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAPICALLTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALLTYPE~', ff($strTableNameAPICALLTYPE), $strSQL);
...
function buildWhere_apicalltype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_apicalltype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "APICALLTYPE");
}