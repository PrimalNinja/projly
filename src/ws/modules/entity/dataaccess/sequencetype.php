...
function add_sequencetype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_sequencetype($objConn_a);
	return addbulk_sequencetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_sequencetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "sequencetype");
	$strCode = "SEQUENCETYPE";
	$strDescription = "SEQUENCETYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g33922222-6db3-49cd-a346-1649c6473afc";
    ...
    $strSQL = "insert into ~TABLENAMESEQUENCETYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESEQUENCETYPE~', ff($strTableNameSEQUENCETYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SEQUENCETYPE", $strResult, $strJSONData);
...
function delete_sequencetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCETYPE = getTableNameEntity("sequencetype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_sequencetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESEQUENCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCETYPE~', ff($strTableNameSEQUENCETYPE), $strSQL);
...
function deleteExtension_sequencetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCETYPEExtension = getTableNameEntityExtension("sequencetype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_sequencetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESEQUENCETYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCETYPEEXTENSION~', ff($strTableNameSEQUENCETYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_sequencetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCETYPE = getTableNameEntity("sequencetype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESEQUENCETYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESEQUENCETYPE~', ff($strTableNameSEQUENCETYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESEQUENCETYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SEQUENCETYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_sequencetype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSEQUENCETYPE = getTableNameEntity("sequencetype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESEQUENCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCETYPE~', ff($strTableNameSEQUENCETYPE), $strSQL);
...
function fetchValue_sequencetype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSEQUENCETYPE = getTableNameEntity("sequencetype", false);	

	$strWhere = buildWhere_sequencetype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESEQUENCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESEQUENCETYPE~', ff($strTableNameSEQUENCETYPE), $strSQL);
...
function buildWhere_sequencetype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_sequencetype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SEQUENCETYPE");
}