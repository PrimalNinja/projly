...
function add_systemmoduletask($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemmoduletask($objConn_a);
	return addbulk_systemmoduletask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemmoduletask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "systemmoduletask");
	$strCode = "SYSTEMMODULETASK";
	$strDescription = "SYSTEMMODULETASK";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g4768521c-4d28-41fe-a8f8-6e0ec6cfee71";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMMODULETASK~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULETASK", $strResult, $strJSONData);
...
function delete_systemmoduletask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULETASK = getTableNameEntity("systemmoduletask", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmoduletask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULETASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULETASK~', ff($strTableNameSYSTEMMODULETASK), $strSQL);
...
function deleteExtension_systemmoduletask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULETASKExtension = getTableNameEntityExtension("systemmoduletask");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmoduletask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULETASKEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULETASKEXTENSION~', ff($strTableNameSYSTEMMODULETASKExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemmoduletask($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULETASK = getTableNameEntity("systemmoduletask", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMMODULETASK~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMMODULETASK~', ff($strTableNameSYSTEMMODULETASK), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMMODULETASK~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULETASK", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemmoduletask($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMMODULETASK = getTableNameEntity("systemmoduletask", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMMODULETASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULETASK~', ff($strTableNameSYSTEMMODULETASK), $strSQL);
...
function fetchValue_systemmoduletask($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULETASK = getTableNameEntity("systemmoduletask", false);	

	$strWhere = buildWhere_systemmoduletask($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMMODULETASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULETASK~', ff($strTableNameSYSTEMMODULETASK), $strSQL);
...
function buildWhere_systemmoduletask($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemmoduletask($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMMODULETASK");
}