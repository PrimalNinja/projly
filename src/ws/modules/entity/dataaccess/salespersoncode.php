...
function add_salespersoncode($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_salespersoncode($objConn_a);
	return addbulk_salespersoncode($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_salespersoncode($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "SALESPERSONCODE";
	$strDescription = "SALESPERSONCODE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g7ea44fdf-b30a-47df-80d4-18057254752e";
    ...
    $strSQL = "insert into ~TABLENAMESALESPERSONCODE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SALESPERSONCODE", $strResult, $strJSONData);
...
function delete_salespersoncode($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODE = getTableNameEntity("salespersoncode", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_salespersoncode($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESALESPERSONCODE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODE~', ff($strTableNameSALESPERSONCODE), $strSQL);
...
function deleteExtension_salespersoncode($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODEExtension = getTableNameEntityExtension("salespersoncode");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_salespersoncode($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESALESPERSONCODEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODEEXTENSION~', ff($strTableNameSALESPERSONCODEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_salespersoncode($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODE = getTableNameEntity("salespersoncode", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESALESPERSONCODE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESALESPERSONCODE~', ff($strTableNameSALESPERSONCODE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESALESPERSONCODE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SALESPERSONCODE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_salespersoncode($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSALESPERSONCODE = getTableNameEntity("salespersoncode", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESALESPERSONCODE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODE~', ff($strTableNameSALESPERSONCODE), $strSQL);
...
function fetchValue_salespersoncode($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSALESPERSONCODE = getTableNameEntity("salespersoncode", false);	

	$strWhere = buildWhere_salespersoncode($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESALESPERSONCODE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESALESPERSONCODE~', ff($strTableNameSALESPERSONCODE), $strSQL);
...
function buildWhere_salespersoncode($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_salespersoncode($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SALESPERSONCODE");
}