...
function add_negotiateddiscount($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_negotiateddiscount($objConn_a);
	return addbulk_negotiateddiscount($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_negotiateddiscount($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "negotiateddiscount");
	$strCode = "NEGOTIATEDDISCOUNT";
	$strDescription = "NEGOTIATEDDISCOUNT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g5ca0bf62-1e0b-4d30-ba0b-c531b5e79e71";
    ...
    $strSQL = "insert into ~TABLENAMENEGOTIATEDDISCOUNT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "NEGOTIATEDDISCOUNT", $strResult, $strJSONData);
...
function delete_negotiateddiscount($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDDISCOUNT = getTableNameEntity("negotiateddiscount", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_negotiateddiscount($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMENEGOTIATEDDISCOUNT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDDISCOUNT~', ff($strTableNameNEGOTIATEDDISCOUNT), $strSQL);
...
function deleteExtension_negotiateddiscount($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDDISCOUNTExtension = getTableNameEntityExtension("negotiateddiscount");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_negotiateddiscount($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMENEGOTIATEDDISCOUNTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDDISCOUNTEXTENSION~', ff($strTableNameNEGOTIATEDDISCOUNTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_negotiateddiscount($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDDISCOUNT = getTableNameEntity("negotiateddiscount", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMENEGOTIATEDDISCOUNT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMENEGOTIATEDDISCOUNT~', ff($strTableNameNEGOTIATEDDISCOUNT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMENEGOTIATEDDISCOUNT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "NEGOTIATEDDISCOUNT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_negotiateddiscount($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameNEGOTIATEDDISCOUNT = getTableNameEntity("negotiateddiscount", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMENEGOTIATEDDISCOUNT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDDISCOUNT~', ff($strTableNameNEGOTIATEDDISCOUNT), $strSQL);
...
function fetchValue_negotiateddiscount($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDDISCOUNT = getTableNameEntity("negotiateddiscount", false);	

	$strWhere = buildWhere_negotiateddiscount($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMENEGOTIATEDDISCOUNT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDDISCOUNT~', ff($strTableNameNEGOTIATEDDISCOUNT), $strSQL);
...
function buildWhere_negotiateddiscount($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_negotiateddiscount($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "NEGOTIATEDDISCOUNT");
}