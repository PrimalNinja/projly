...
function add_rostertemplate_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_rostertemplate_projectparticipant($objConn_a);
	return addbulk_rostertemplate_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_rostertemplate_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT = getTableNameEntity("rostertemplate_projectparticipant", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "rostertemplate_projectparticipant");
	$strCode = "ROSTERTEMPLATE_PROJECTPARTICIPANT";
	$strDescription = "ROSTERTEMPLATE_PROJECTPARTICIPANT";
    ...
    $strSQL = "insert into ~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~', ff($strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTERTEMPLATE_PROJECTPARTICIPANT", $strResult, $strJSONData);
...
function delete_rostertemplate_projectparticipant($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT = getTableNameEntity("rostertemplate_projectparticipant", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rostertemplate_projectparticipant($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~', ff($strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT), $strSQL);
...
function deleteExtension_rostertemplate_projectparticipant($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_PROJECTPARTICIPANTExtension = getTableNameEntityExtension("rostertemplate_projectparticipant");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rostertemplate_projectparticipant($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANTEXTENSION~', ff($strTableNameROSTERTEMPLATE_PROJECTPARTICIPANTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_rostertemplate_projectparticipant($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT = getTableNameEntity("rostertemplate_projectparticipant", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~', ff($strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTERTEMPLATE_PROJECTPARTICIPANT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_rostertemplate_projectparticipant($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT = getTableNameEntity("rostertemplate_projectparticipant", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~', ff($strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT), $strSQL);
...
function fetchValue_rostertemplate_projectparticipant($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT = getTableNameEntity("rostertemplate_projectparticipant", false);	

	$strWhere = buildWhere_rostertemplate_projectparticipant($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE_PROJECTPARTICIPANT~', ff($strTableNameROSTERTEMPLATE_PROJECTPARTICIPANT), $strSQL);
...
function buildWhere_rostertemplate_projectparticipant($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_rostertemplate_projectparticipant($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ROSTERTEMPLATE_PROJECTPARTICIPANT");
}