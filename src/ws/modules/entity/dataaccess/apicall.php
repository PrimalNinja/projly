...
function add_apicall($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_apicall($objConn_a);
	return addbulk_apicall($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_apicall($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ga0c53bc1-9f46-489f-be5e-09c27f7ef3f6";
    ...
    $strSQL = "insert into ~TABLENAMEAPICALL~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "APICALL", $strResult, $strJSONData);
...
function delete_apicall($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALL = getTableNameEntity("apicall", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_apicall($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPICALL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALL~', ff($strTableNameAPICALL), $strSQL);
...
function deleteExtension_apicall($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALLExtension = getTableNameEntityExtension("apicall");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_apicall($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPICALLEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALLEXTENSION~', ff($strTableNameAPICALLExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_apicall($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALL = getTableNameEntity("apicall", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAPICALL~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAPICALL~', ff($strTableNameAPICALL), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAPICALL~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "APICALL", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_apicall($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAPICALL = getTableNameEntity("apicall", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAPICALL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALL~', ff($strTableNameAPICALL), $strSQL);
...
function fetchValue_apicall($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPICALL = getTableNameEntity("apicall", false);	

	$strWhere = buildWhere_apicall($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAPICALL~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPICALL~', ff($strTableNameAPICALL), $strSQL);
...
function buildWhere_apicall($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_apicall($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "APICALL");
}