...
function add_todostatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_todostatus($objConn_a);
	return addbulk_todostatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_todostatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameTODOSTATUS = getTableNameEntity("todostatus", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "todostatus");
	$strCode = "TODOSTATUS";
	$strDescription = "TODOSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g5ca187b1-6ada-445c-9157-11dceec45b52";
    ...
    $strSQL = "insert into ~TABLENAMETODOSTATUS~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMETODOSTATUS~', ff($strTableNameTODOSTATUS), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TODOSTATUS", $strResult, $strJSONData);
...
function delete_todostatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODOSTATUS = getTableNameEntity("todostatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_todostatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETODOSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODOSTATUS~', ff($strTableNameTODOSTATUS), $strSQL);
...
function deleteExtension_todostatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODOSTATUSExtension = getTableNameEntityExtension("todostatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_todostatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETODOSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODOSTATUSEXTENSION~', ff($strTableNameTODOSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_todostatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODOSTATUS = getTableNameEntity("todostatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETODOSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETODOSTATUS~', ff($strTableNameTODOSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETODOSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TODOSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_todostatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTODOSTATUS = getTableNameEntity("todostatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETODOSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODOSTATUS~', ff($strTableNameTODOSTATUS), $strSQL);
...
function fetchValue_todostatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODOSTATUS = getTableNameEntity("todostatus", false);	

	$strWhere = buildWhere_todostatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETODOSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODOSTATUS~', ff($strTableNameTODOSTATUS), $strSQL);
...
function buildWhere_todostatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_todostatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TODOSTATUS");
}