...
function add_registration($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_registration($objConn_a);
	return addbulk_registration($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_registration($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffdcbf4797-ab6f-4cd7-bf5b-a74097e8e303";
    ...
    $strSQL = "insert into ~TABLENAMEREGISTRATION~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REGISTRATION", $strResult, $strJSONData);
...
function delete_registration($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATION = getTableNameEntity("registration", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_registration($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREGISTRATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATION~', ff($strTableNameREGISTRATION), $strSQL);
...
function deleteExtension_registration($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATIONExtension = getTableNameEntityExtension("registration");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_registration($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREGISTRATIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATIONEXTENSION~', ff($strTableNameREGISTRATIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_registration($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATION = getTableNameEntity("registration", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREGISTRATION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREGISTRATION~', ff($strTableNameREGISTRATION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREGISTRATION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REGISTRATION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_registration($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREGISTRATION = getTableNameEntity("registration", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREGISTRATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATION~', ff($strTableNameREGISTRATION), $strSQL);
...
function fetchValue_registration($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATION = getTableNameEntity("registration", false);	

	$strWhere = buildWhere_registration($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREGISTRATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATION~', ff($strTableNameREGISTRATION), $strSQL);
...
function buildWhere_registration($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_registration($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REGISTRATION");
}