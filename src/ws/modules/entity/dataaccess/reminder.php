...
function add_reminder($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_reminder($objConn_a);
	return addbulk_reminder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_reminder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "reminder");
	$strCode = "REMINDER";
	$strDescription = "REMINDER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "adee66ca-8443-4e3e-8ddf-b5a480bf2b49";
    ...
    $strSQL = "insert into ~TABLENAMEREMINDER~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REMINDER", $strResult, $strJSONData);
...
function delete_reminder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREMINDER = getTableNameEntity("reminder", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reminder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREMINDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREMINDER~', ff($strTableNameREMINDER), $strSQL);
...
function deleteExtension_reminder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREMINDERExtension = getTableNameEntityExtension("reminder");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reminder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREMINDEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREMINDEREXTENSION~', ff($strTableNameREMINDERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_reminder($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREMINDER = getTableNameEntity("reminder", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREMINDER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREMINDER~', ff($strTableNameREMINDER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREMINDER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REMINDER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_reminder($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREMINDER = getTableNameEntity("reminder", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREMINDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREMINDER~', ff($strTableNameREMINDER), $strSQL);
...
function fetchValue_reminder($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREMINDER = getTableNameEntity("reminder", false);	

	$strWhere = buildWhere_reminder($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREMINDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREMINDER~', ff($strTableNameREMINDER), $strSQL);
...
function buildWhere_reminder($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_reminder($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REMINDER");
}