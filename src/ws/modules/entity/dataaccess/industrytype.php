...
function add_industrytype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_industrytype($objConn_a);
	return addbulk_industrytype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_industrytype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameINDUSTRYTYPE = getTableNameEntity("industrytype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "industrytype");
	$strCode = "INDUSTRYTYPE";
	$strDescription = "INDUSTRYTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff466e2c8a-2df0-4b50-873a-48e415f80fb4";
    ...
    $strSQL = "insert into ~TABLENAMEINDUSTRYTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEINDUSTRYTYPE~', ff($strTableNameINDUSTRYTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INDUSTRYTYPE", $strResult, $strJSONData);
...
function delete_industrytype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINDUSTRYTYPE = getTableNameEntity("industrytype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_industrytype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINDUSTRYTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINDUSTRYTYPE~', ff($strTableNameINDUSTRYTYPE), $strSQL);
...
function deleteExtension_industrytype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINDUSTRYTYPEExtension = getTableNameEntityExtension("industrytype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_industrytype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINDUSTRYTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINDUSTRYTYPEEXTENSION~', ff($strTableNameINDUSTRYTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_industrytype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINDUSTRYTYPE = getTableNameEntity("industrytype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINDUSTRYTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINDUSTRYTYPE~', ff($strTableNameINDUSTRYTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINDUSTRYTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INDUSTRYTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_industrytype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINDUSTRYTYPE = getTableNameEntity("industrytype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINDUSTRYTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINDUSTRYTYPE~', ff($strTableNameINDUSTRYTYPE), $strSQL);
...
function fetchValue_industrytype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINDUSTRYTYPE = getTableNameEntity("industrytype", false);	

	$strWhere = buildWhere_industrytype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINDUSTRYTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINDUSTRYTYPE~', ff($strTableNameINDUSTRYTYPE), $strSQL);
...
function buildWhere_industrytype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_industrytype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INDUSTRYTYPE");
}