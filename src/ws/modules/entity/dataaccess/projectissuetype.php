...
function add_projectissuetype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectissuetype($objConn_a);
	return addbulk_projectissuetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectissuetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTISSUETYPE = getTableNameEntity("projectissuetype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectissuetype");
	$strCode = "PROJECTISSUETYPE";
	$strDescription = "PROJECTISSUETYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffe82d8d7d-0047-4c1b-93d8-00331143173a";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTISSUETYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTISSUETYPE~', ff($strTableNamePROJECTISSUETYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUETYPE", $strResult, $strJSONData);
...
function delete_projectissuetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUETYPE = getTableNameEntity("projectissuetype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissuetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUETYPE~', ff($strTableNamePROJECTISSUETYPE), $strSQL);
...
function deleteExtension_projectissuetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUETYPEExtension = getTableNameEntityExtension("projectissuetype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissuetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUETYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUETYPEEXTENSION~', ff($strTableNamePROJECTISSUETYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectissuetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUETYPE = getTableNameEntity("projectissuetype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTISSUETYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTISSUETYPE~', ff($strTableNamePROJECTISSUETYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTISSUETYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUETYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectissuetype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTISSUETYPE = getTableNameEntity("projectissuetype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTISSUETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUETYPE~', ff($strTableNamePROJECTISSUETYPE), $strSQL);
...
function fetchValue_projectissuetype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUETYPE = getTableNameEntity("projectissuetype", false);	

	$strWhere = buildWhere_projectissuetype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTISSUETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUETYPE~', ff($strTableNamePROJECTISSUETYPE), $strSQL);
...
function buildWhere_projectissuetype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectissuetype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTISSUETYPE");
}