...
function add_systembuildtask($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systembuildtask($objConn_a);
	return addbulk_systembuildtask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systembuildtask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "systembuildtask");
	$strCode = "SYSTEMBUILDTASK";
	$strDescription = "SYSTEMBUILDTASK";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gd8fac874-b432-4019-b5b0-abefded76d05";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMBUILDTASK~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMBUILDTASK", $strResult, $strJSONData);
...
function delete_systembuildtask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMBUILDTASK = getTableNameEntity("systembuildtask", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systembuildtask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMBUILDTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMBUILDTASK~', ff($strTableNameSYSTEMBUILDTASK), $strSQL);
...
function deleteExtension_systembuildtask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMBUILDTASKExtension = getTableNameEntityExtension("systembuildtask");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systembuildtask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMBUILDTASKEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMBUILDTASKEXTENSION~', ff($strTableNameSYSTEMBUILDTASKExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systembuildtask($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMBUILDTASK = getTableNameEntity("systembuildtask", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMBUILDTASK~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMBUILDTASK~', ff($strTableNameSYSTEMBUILDTASK), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMBUILDTASK~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMBUILDTASK", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systembuildtask($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMBUILDTASK = getTableNameEntity("systembuildtask", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMBUILDTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMBUILDTASK~', ff($strTableNameSYSTEMBUILDTASK), $strSQL);
...
function fetchValue_systembuildtask($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMBUILDTASK = getTableNameEntity("systembuildtask", false);	

	$strWhere = buildWhere_systembuildtask($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMBUILDTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMBUILDTASK~', ff($strTableNameSYSTEMBUILDTASK), $strSQL);
...
function buildWhere_systembuildtask($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systembuildtask($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMBUILDTASK");
}