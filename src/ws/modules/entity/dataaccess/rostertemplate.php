...
function add_rostertemplate($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_rostertemplate($objConn_a);
	return addbulk_rostertemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_rostertemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "rostertemplate");
	$strCode = "ROSTERTEMPLATE";
	$strDescription = "ROSTERTEMPLATE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g796a111e-0b17-4965-8683-af5528feab09";
    ...
    $strSQL = "insert into ~TABLENAMEROSTERTEMPLATE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTERTEMPLATE", $strResult, $strJSONData);
...
function delete_rostertemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE = getTableNameEntity("rostertemplate", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rostertemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTERTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE~', ff($strTableNameROSTERTEMPLATE), $strSQL);
...
function deleteExtension_rostertemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATEExtension = getTableNameEntityExtension("rostertemplate");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_rostertemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTERTEMPLATEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATEEXTENSION~', ff($strTableNameROSTERTEMPLATEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_rostertemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE = getTableNameEntity("rostertemplate", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEROSTERTEMPLATE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE~', ff($strTableNameROSTERTEMPLATE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEROSTERTEMPLATE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTERTEMPLATE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_rostertemplate($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameROSTERTEMPLATE = getTableNameEntity("rostertemplate", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEROSTERTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE~', ff($strTableNameROSTERTEMPLATE), $strSQL);
...
function fetchValue_rostertemplate($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERTEMPLATE = getTableNameEntity("rostertemplate", false);	

	$strWhere = buildWhere_rostertemplate($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEROSTERTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTERTEMPLATE~', ff($strTableNameROSTERTEMPLATE), $strSQL);
...
function buildWhere_rostertemplate($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_rostertemplate($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ROSTERTEMPLATE");
}