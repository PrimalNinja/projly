...
function add_fileformat_field($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_fileformat_field($objConn_a);
	return addbulk_fileformat_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_fileformat_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFILEFORMAT_FIELD = getTableNameEntity("fileformat_field", false);	
	...
	$strCode = "FILEFORMAT_FIELD";
	$strDescription = "FILEFORMAT_FIELD";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gdca0c616-9e33-4f80-adb1-c964e9f44713";
    ...
    $strSQL = "insert into ~TABLENAMEFILEFORMAT_FIELD~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFILEFORMAT_FIELD~', ff($strTableNameFILEFORMAT_FIELD), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMAT_FIELD", $strResult, $strJSONData);
...
function delete_fileformat_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMAT_FIELD = getTableNameEntity("fileformat_field", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformat_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMAT_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMAT_FIELD~', ff($strTableNameFILEFORMAT_FIELD), $strSQL);
...
function deleteExtension_fileformat_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMAT_FIELDExtension = getTableNameEntityExtension("fileformat_field");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformat_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMAT_FIELDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMAT_FIELDEXTENSION~', ff($strTableNameFILEFORMAT_FIELDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_fileformat_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMAT_FIELD = getTableNameEntity("fileformat_field", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILEFORMAT_FIELD~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILEFORMAT_FIELD~', ff($strTableNameFILEFORMAT_FIELD), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILEFORMAT_FIELD~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMAT_FIELD", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_fileformat_field($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILEFORMAT_FIELD = getTableNameEntity("fileformat_field", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILEFORMAT_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMAT_FIELD~', ff($strTableNameFILEFORMAT_FIELD), $strSQL);
...
function fetchValue_fileformat_field($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMAT_FIELD = getTableNameEntity("fileformat_field", false);	

	$strWhere = buildWhere_fileformat_field($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILEFORMAT_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMAT_FIELD~', ff($strTableNameFILEFORMAT_FIELD), $strSQL);
...
function buildWhere_fileformat_field($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_fileformat_field($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILEFORMAT_FIELD");
}