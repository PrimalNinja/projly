...
function add_application($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_application($objConn_a);
	return addbulk_application($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_application($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "application");
	$strCode = "APPLICATION";
	$strDescription = "APPLICATION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g375c1fc8-2001-4235-bb44-ec10fd82afc8";
    ...
    $strSQL = "insert into ~TABLENAMEAPPLICATION~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "APPLICATION", $strResult, $strJSONData);
...
function delete_application($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATION = getTableNameEntity("application", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_application($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPPLICATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATION~', ff($strTableNameAPPLICATION), $strSQL);
...
function deleteExtension_application($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATIONExtension = getTableNameEntityExtension("application");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_application($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPPLICATIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATIONEXTENSION~', ff($strTableNameAPPLICATIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_application($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATION = getTableNameEntity("application", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAPPLICATION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAPPLICATION~', ff($strTableNameAPPLICATION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAPPLICATION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "APPLICATION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_application($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAPPLICATION = getTableNameEntity("application", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAPPLICATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATION~', ff($strTableNameAPPLICATION), $strSQL);
...
function fetchValue_application($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPPLICATION = getTableNameEntity("application", false);	

	$strWhere = buildWhere_application($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAPPLICATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPPLICATION~', ff($strTableNameAPPLICATION), $strSQL);
...
function buildWhere_application($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_application($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "APPLICATION");
}