...
function add_registrationtype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_registrationtype($objConn_a);
	return addbulk_registrationtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_registrationtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "registrationtype");
	$strCode = "REGISTRATIONTYPE";
	$strDescription = "REGISTRATIONTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff4909db1e-8bee-4c2b-be10-6d0853789635";
    ...
    $strSQL = "insert into ~TABLENAMEREGISTRATIONTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REGISTRATIONTYPE", $strResult, $strJSONData);
...
function delete_registrationtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATIONTYPE = getTableNameEntity("registrationtype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_registrationtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREGISTRATIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATIONTYPE~', ff($strTableNameREGISTRATIONTYPE), $strSQL);
...
function deleteExtension_registrationtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATIONTYPEExtension = getTableNameEntityExtension("registrationtype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_registrationtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREGISTRATIONTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATIONTYPEEXTENSION~', ff($strTableNameREGISTRATIONTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_registrationtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATIONTYPE = getTableNameEntity("registrationtype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREGISTRATIONTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREGISTRATIONTYPE~', ff($strTableNameREGISTRATIONTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREGISTRATIONTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REGISTRATIONTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_registrationtype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREGISTRATIONTYPE = getTableNameEntity("registrationtype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREGISTRATIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATIONTYPE~', ff($strTableNameREGISTRATIONTYPE), $strSQL);
...
function fetchValue_registrationtype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREGISTRATIONTYPE = getTableNameEntity("registrationtype", false);	

	$strWhere = buildWhere_registrationtype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREGISTRATIONTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREGISTRATIONTYPE~', ff($strTableNameREGISTRATIONTYPE), $strSQL);
...
function buildWhere_registrationtype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_registrationtype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REGISTRATIONTYPE");
}