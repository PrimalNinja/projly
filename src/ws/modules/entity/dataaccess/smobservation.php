...
function add_smobservation($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_smobservation($objConn_a);
	return addbulk_smobservation($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_smobservation($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "smobservation");
	$strCode = "SMOBSERVATION";
	$strDescription = "SMOBSERVATION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g0eb51378-ffab-4b39-88fe-44137b85dd30";
    ...
    $strSQL = "insert into ~TABLENAMESMOBSERVATION~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SMOBSERVATION", $strResult, $strJSONData);
...
function delete_smobservation($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMOBSERVATION = getTableNameEntity("smobservation", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smobservation($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMOBSERVATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMOBSERVATION~', ff($strTableNameSMOBSERVATION), $strSQL);
...
function deleteExtension_smobservation($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMOBSERVATIONExtension = getTableNameEntityExtension("smobservation");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smobservation($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMOBSERVATIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMOBSERVATIONEXTENSION~', ff($strTableNameSMOBSERVATIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_smobservation($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMOBSERVATION = getTableNameEntity("smobservation", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESMOBSERVATION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESMOBSERVATION~', ff($strTableNameSMOBSERVATION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESMOBSERVATION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SMOBSERVATION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_smobservation($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSMOBSERVATION = getTableNameEntity("smobservation", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESMOBSERVATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMOBSERVATION~', ff($strTableNameSMOBSERVATION), $strSQL);
...
function fetchValue_smobservation($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMOBSERVATION = getTableNameEntity("smobservation", false);	

	$strWhere = buildWhere_smobservation($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESMOBSERVATION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMOBSERVATION~', ff($strTableNameSMOBSERVATION), $strSQL);
...
function buildWhere_smobservation($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_smobservation($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SMOBSERVATION");
}