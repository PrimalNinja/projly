...
function add_internalmessage($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_internalmessage($objConn_a);
	return addbulk_internalmessage($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_internalmessage($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDescription = "INTERNALMESSAGE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g8938a0d4-8efc-41ed-a420-4af7b808b3f0";
    ...
    $strSQL = "insert into ~TABLENAMEINTERNALMESSAGE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTERNALMESSAGE", $strResult, $strJSONData);
...
function delete_internalmessage($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGE = getTableNameEntity("internalmessage", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_internalmessage($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTERNALMESSAGE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGE~', ff($strTableNameINTERNALMESSAGE), $strSQL);
...
function deleteExtension_internalmessage($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGEExtension = getTableNameEntityExtension("internalmessage");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_internalmessage($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTERNALMESSAGEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGEEXTENSION~', ff($strTableNameINTERNALMESSAGEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_internalmessage($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGE = getTableNameEntity("internalmessage", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTERNALMESSAGE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTERNALMESSAGE~', ff($strTableNameINTERNALMESSAGE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTERNALMESSAGE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTERNALMESSAGE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_internalmessage($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTERNALMESSAGE = getTableNameEntity("internalmessage", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTERNALMESSAGE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGE~', ff($strTableNameINTERNALMESSAGE), $strSQL);
...
function fetchValue_internalmessage($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGE = getTableNameEntity("internalmessage", false);	

	$strWhere = buildWhere_internalmessage($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTERNALMESSAGE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGE~', ff($strTableNameINTERNALMESSAGE), $strSQL);
...
function buildWhere_internalmessage($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_internalmessage($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTERNALMESSAGE");
}