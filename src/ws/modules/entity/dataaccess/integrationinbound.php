...
function add_integrationinbound($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationinbound($objConn_a);
	return addbulk_integrationinbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationinbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "integrationinbound");
	$strCode = "INTEGRATIONINBOUND";
	$strDescription = "INTEGRATIONINBOUND";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g8fbd6661-d8e6-4279-b9a5-5e1042970937";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONINBOUND~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONINBOUND", $strResult, $strJSONData);
...
function delete_integrationinbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUND = getTableNameEntity("integrationinbound", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationinbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONINBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUND~', ff($strTableNameINTEGRATIONINBOUND), $strSQL);
...
function deleteExtension_integrationinbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUNDExtension = getTableNameEntityExtension("integrationinbound");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationinbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONINBOUNDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUNDEXTENSION~', ff($strTableNameINTEGRATIONINBOUNDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationinbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUND = getTableNameEntity("integrationinbound", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONINBOUND~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUND~', ff($strTableNameINTEGRATIONINBOUND), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONINBOUND~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONINBOUND", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationinbound($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONINBOUND = getTableNameEntity("integrationinbound", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONINBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUND~', ff($strTableNameINTEGRATIONINBOUND), $strSQL);
...
function fetchValue_integrationinbound($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONINBOUND = getTableNameEntity("integrationinbound", false);	

	$strWhere = buildWhere_integrationinbound($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONINBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONINBOUND~', ff($strTableNameINTEGRATIONINBOUND), $strSQL);
...
function buildWhere_integrationinbound($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationinbound($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONINBOUND");
}