...
function add_reportsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_reportsection_field_property($objConn_a);
	return addbulk_reportsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_reportsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameREPORTSECTION_FIELD_PROPERTY = getTableNameEntity("reportsection_field_property", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "reportsection_field_property");
	$strCode = "REPORTSECTION_FIELD_PROPERTY";
	$strDescription = "REPORTSECTION_FIELD_PROPERTY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g93c39f5a-6910-492e-85fd-b781702bcf0dr";
    ...
    $strSQL = "insert into ~TABLENAMEREPORTSECTION_FIELD_PROPERTY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEREPORTSECTION_FIELD_PROPERTY~', ff($strTableNameREPORTSECTION_FIELD_PROPERTY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTSECTION_FIELD_PROPERTY", $strResult, $strJSONData);
...
function delete_reportsection_field_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTION_FIELD_PROPERTY = getTableNameEntity("reportsection_field_property", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportsection_field_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTSECTION_FIELD_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTION_FIELD_PROPERTY~', ff($strTableNameREPORTSECTION_FIELD_PROPERTY), $strSQL);
...
function deleteExtension_reportsection_field_property($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTION_FIELD_PROPERTYExtension = getTableNameEntityExtension("reportsection_field_property");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportsection_field_property($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTSECTION_FIELD_PROPERTYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTION_FIELD_PROPERTYEXTENSION~', ff($strTableNameREPORTSECTION_FIELD_PROPERTYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_reportsection_field_property($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTION_FIELD_PROPERTY = getTableNameEntity("reportsection_field_property", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREPORTSECTION_FIELD_PROPERTY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREPORTSECTION_FIELD_PROPERTY~', ff($strTableNameREPORTSECTION_FIELD_PROPERTY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREPORTSECTION_FIELD_PROPERTY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTSECTION_FIELD_PROPERTY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_reportsection_field_property($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREPORTSECTION_FIELD_PROPERTY = getTableNameEntity("reportsection_field_property", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREPORTSECTION_FIELD_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTION_FIELD_PROPERTY~', ff($strTableNameREPORTSECTION_FIELD_PROPERTY), $strSQL);
...
function fetchValue_reportsection_field_property($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTION_FIELD_PROPERTY = getTableNameEntity("reportsection_field_property", false);	

	$strWhere = buildWhere_reportsection_field_property($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREPORTSECTION_FIELD_PROPERTY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTION_FIELD_PROPERTY~', ff($strTableNameREPORTSECTION_FIELD_PROPERTY), $strSQL);
...
function buildWhere_reportsection_field_property($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_reportsection_field_property($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REPORTSECTION_FIELD_PROPERTY");
}