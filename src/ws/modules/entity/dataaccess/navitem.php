...
function add_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_navitem($objConn_a);
	return addbulk_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "navitem");
	$strCode = "NAVITEM";
	$strDescription = "NAVITEM";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g871c4f64-236d-4e98-939d-8d5a88b7d62b";
    ...
    $strSQL = "insert into ~TABLENAMENAVITEM~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "NAVITEM", $strResult, $strJSONData);
...
function delete_navitem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNAVITEM = getTableNameEntity("navitem", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_navitem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMENAVITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENAVITEM~', ff($strTableNameNAVITEM), $strSQL);
...
function deleteExtension_navitem($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNAVITEMExtension = getTableNameEntityExtension("navitem");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_navitem($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMENAVITEMEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENAVITEMEXTENSION~', ff($strTableNameNAVITEMExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_navitem($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNAVITEM = getTableNameEntity("navitem", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMENAVITEM~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMENAVITEM~', ff($strTableNameNAVITEM), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMENAVITEM~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "NAVITEM", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_navitem($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameNAVITEM = getTableNameEntity("navitem", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMENAVITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENAVITEM~', ff($strTableNameNAVITEM), $strSQL);
...
function fetchValue_navitem($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNAVITEM = getTableNameEntity("navitem", false);	

	$strWhere = buildWhere_navitem($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMENAVITEM~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENAVITEM~', ff($strTableNameNAVITEM), $strSQL);
...
function buildWhere_navitem($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_navitem($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "NAVITEM");
}