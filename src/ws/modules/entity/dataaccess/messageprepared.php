...
function add_messageprepared($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_messageprepared($objConn_a);
	return addbulk_messageprepared($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_messageprepared($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameMESSAGEPREPARED = getTableNameEntity("messageprepared", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "messageprepared");
	$strCode = "MESSAGEPREPARED";
	$strDescription = "MESSAGEPREPARED";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g088d79ab-f1bf-4349-8abd-c8c1749d6f6a";
    ...
    $strSQL = "insert into ~TABLENAMEMESSAGEPREPARED~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEMESSAGEPREPARED~', ff($strTableNameMESSAGEPREPARED), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MESSAGEPREPARED", $strResult, $strJSONData);
...
function delete_messageprepared($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGEPREPARED = getTableNameEntity("messageprepared", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_messageprepared($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMESSAGEPREPARED~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGEPREPARED~', ff($strTableNameMESSAGEPREPARED), $strSQL);
...
function deleteExtension_messageprepared($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGEPREPAREDExtension = getTableNameEntityExtension("messageprepared");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_messageprepared($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMESSAGEPREPAREDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGEPREPAREDEXTENSION~', ff($strTableNameMESSAGEPREPAREDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_messageprepared($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGEPREPARED = getTableNameEntity("messageprepared", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMESSAGEPREPARED~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMESSAGEPREPARED~', ff($strTableNameMESSAGEPREPARED), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMESSAGEPREPARED~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MESSAGEPREPARED", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_messageprepared($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMESSAGEPREPARED = getTableNameEntity("messageprepared", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMESSAGEPREPARED~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGEPREPARED~', ff($strTableNameMESSAGEPREPARED), $strSQL);
...
function fetchValue_messageprepared($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGEPREPARED = getTableNameEntity("messageprepared", false);	

	$strWhere = buildWhere_messageprepared($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMESSAGEPREPARED~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGEPREPARED~', ff($strTableNameMESSAGEPREPARED), $strSQL);
...
function buildWhere_messageprepared($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_messageprepared($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MESSAGEPREPARED");
}