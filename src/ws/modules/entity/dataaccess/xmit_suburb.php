...
function add_xmit_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_xmit_suburb($objConn_a);
	return addbulk_xmit_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_xmit_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameXMIT_SUBURB = getTableNameEntity("xmit_suburb", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "xmit_suburb");
	$strCode = "XMIT_SUBURB";
	$strDescription = "XMIT_SUBURB";
    ...
    $strSQL = "insert into ~TABLENAMEXMIT_SUBURB~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEXMIT_SUBURB~', ff($strTableNameXMIT_SUBURB), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "XMIT_SUBURB", $strResult, $strJSONData);
...
function delete_xmit_suburb($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_SUBURB = getTableNameEntity("xmit_suburb", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_xmit_suburb($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEXMIT_SUBURB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_SUBURB~', ff($strTableNameXMIT_SUBURB), $strSQL);
...
function deleteExtension_xmit_suburb($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_SUBURBExtension = getTableNameEntityExtension("xmit_suburb");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_xmit_suburb($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEXMIT_SUBURBEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_SUBURBEXTENSION~', ff($strTableNameXMIT_SUBURBExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_xmit_suburb($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_SUBURB = getTableNameEntity("xmit_suburb", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEXMIT_SUBURB~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEXMIT_SUBURB~', ff($strTableNameXMIT_SUBURB), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEXMIT_SUBURB~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "XMIT_SUBURB", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_xmit_suburb($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameXMIT_SUBURB = getTableNameEntity("xmit_suburb", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEXMIT_SUBURB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_SUBURB~', ff($strTableNameXMIT_SUBURB), $strSQL);
...
function fetchValue_xmit_suburb($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_SUBURB = getTableNameEntity("xmit_suburb", false);	

	$strWhere = buildWhere_xmit_suburb($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEXMIT_SUBURB~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_SUBURB~', ff($strTableNameXMIT_SUBURB), $strSQL);
...
function buildWhere_xmit_suburb($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_xmit_suburb($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "XMIT_SUBURB");
}