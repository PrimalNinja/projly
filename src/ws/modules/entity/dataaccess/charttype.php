...
function add_charttype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_charttype($objConn_a);
	return addbulk_charttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_charttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCHARTTYPE = getTableNameEntity("charttype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "charttype");
	$strCode = "CHARTTYPE";
	$strDescription = "CHARTTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g8358b7c1-e7c9-4e57-8e62-8c2251af006c";
    ...
    $strSQL = "insert into ~TABLENAMECHARTTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECHARTTYPE~', ff($strTableNameCHARTTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CHARTTYPE", $strResult, $strJSONData);
...
function delete_charttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHARTTYPE = getTableNameEntity("charttype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_charttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECHARTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHARTTYPE~', ff($strTableNameCHARTTYPE), $strSQL);
...
function deleteExtension_charttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHARTTYPEExtension = getTableNameEntityExtension("charttype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_charttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECHARTTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHARTTYPEEXTENSION~', ff($strTableNameCHARTTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_charttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHARTTYPE = getTableNameEntity("charttype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECHARTTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECHARTTYPE~', ff($strTableNameCHARTTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECHARTTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CHARTTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_charttype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCHARTTYPE = getTableNameEntity("charttype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECHARTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHARTTYPE~', ff($strTableNameCHARTTYPE), $strSQL);
...
function fetchValue_charttype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCHARTTYPE = getTableNameEntity("charttype", false);	

	$strWhere = buildWhere_charttype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECHARTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECHARTTYPE~', ff($strTableNameCHARTTYPE), $strSQL);
...
function buildWhere_charttype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_charttype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CHARTTYPE");
}