...
function add_filetype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_filetype($objConn_a);
	return addbulk_filetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_filetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "filetype");
	$strCode = "FILETYPE";
	$strDescription = "FILETYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gc5913c98-7aff-4753-9b43-6f388e7b707f";
    ...
    $strSQL = "insert into ~TABLENAMEFILETYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILETYPE", $strResult, $strJSONData);
...
function delete_filetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILETYPE = getTableNameEntity("filetype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILETYPE~', ff($strTableNameFILETYPE), $strSQL);
...
function deleteExtension_filetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILETYPEExtension = getTableNameEntityExtension("filetype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILETYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILETYPEEXTENSION~', ff($strTableNameFILETYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_filetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILETYPE = getTableNameEntity("filetype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILETYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILETYPE~', ff($strTableNameFILETYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILETYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILETYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_filetype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILETYPE = getTableNameEntity("filetype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILETYPE~', ff($strTableNameFILETYPE), $strSQL);
...
function fetchValue_filetype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILETYPE = getTableNameEntity("filetype", false);	

	$strWhere = buildWhere_filetype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILETYPE~', ff($strTableNameFILETYPE), $strSQL);
...
function buildWhere_filetype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_filetype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILETYPE");
}