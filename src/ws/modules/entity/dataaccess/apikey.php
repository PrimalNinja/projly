...
function add_apikey($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_apikey($objConn_a);
	return addbulk_apikey($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_apikey($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameAPIKEY = getTableNameEntity("apikey", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "apikey");
	$strCode = "APIKEY";
	$strDescription = "APIKEY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g8e575ee5-80d2-461f-bbad-2cd948b475fe";
    ...
    $strSQL = "insert into ~TABLENAMEAPIKEY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEAPIKEY~', ff($strTableNameAPIKEY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "APIKEY", $strResult, $strJSONData);
...
function delete_apikey($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPIKEY = getTableNameEntity("apikey", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_apikey($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPIKEY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPIKEY~', ff($strTableNameAPIKEY), $strSQL);
...
function deleteExtension_apikey($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPIKEYExtension = getTableNameEntityExtension("apikey");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_apikey($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAPIKEYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPIKEYEXTENSION~', ff($strTableNameAPIKEYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_apikey($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPIKEY = getTableNameEntity("apikey", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAPIKEY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAPIKEY~', ff($strTableNameAPIKEY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAPIKEY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "APIKEY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_apikey($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAPIKEY = getTableNameEntity("apikey", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAPIKEY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPIKEY~', ff($strTableNameAPIKEY), $strSQL);
...
function fetchValue_apikey($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAPIKEY = getTableNameEntity("apikey", false);	

	$strWhere = buildWhere_apikey($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAPIKEY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAPIKEY~', ff($strTableNameAPIKEY), $strSQL);
...
function buildWhere_apikey($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_apikey($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "APIKEY");
}