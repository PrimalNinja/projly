...
function add_projectissuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectissuestatus($objConn_a);
	return addbulk_projectissuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectissuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "projectissuestatus");
	$strCode = "PROJECTISSUESTATUS";
	$strDescription = "PROJECTISSUESTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g3a968f00-7b4e-4910-bad8-55ac8be2d610";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTISSUESTATUS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUESTATUS", $strResult, $strJSONData);
...
function delete_projectissuestatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUESTATUS = getTableNameEntity("projectissuestatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissuestatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUESTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUESTATUS~', ff($strTableNamePROJECTISSUESTATUS), $strSQL);
...
function deleteExtension_projectissuestatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUESTATUSExtension = getTableNameEntityExtension("projectissuestatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissuestatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUESTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUESTATUSEXTENSION~', ff($strTableNamePROJECTISSUESTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectissuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUESTATUS = getTableNameEntity("projectissuestatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTISSUESTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTISSUESTATUS~', ff($strTableNamePROJECTISSUESTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTISSUESTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUESTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectissuestatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTISSUESTATUS = getTableNameEntity("projectissuestatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTISSUESTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUESTATUS~', ff($strTableNamePROJECTISSUESTATUS), $strSQL);
...
function fetchValue_projectissuestatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUESTATUS = getTableNameEntity("projectissuestatus", false);	

	$strWhere = buildWhere_projectissuestatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTISSUESTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUESTATUS~', ff($strTableNamePROJECTISSUESTATUS), $strSQL);
...
function buildWhere_projectissuestatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectissuestatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTISSUESTATUS");
}