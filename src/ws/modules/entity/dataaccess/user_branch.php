...
function add_user_branch($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_user_branch($objConn_a);
	return addbulk_user_branch($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_user_branch($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameUSER_BRANCH = getTableNameEntity("user_branch", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "user_branch");
	$strCode = "USER_BRANCH";
	$strDescription = "USER_BRANCH";
    ...
    $strSQL = "insert into ~TABLENAMEUSER_BRANCH~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEUSER_BRANCH~', ff($strTableNameUSER_BRANCH), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "USER_BRANCH", $strResult, $strJSONData);
...
function delete_user_branch($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_BRANCH = getTableNameEntity("user_branch", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_user_branch($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSER_BRANCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_BRANCH~', ff($strTableNameUSER_BRANCH), $strSQL);
...
function deleteExtension_user_branch($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_BRANCHExtension = getTableNameEntityExtension("user_branch");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_user_branch($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSER_BRANCHEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_BRANCHEXTENSION~', ff($strTableNameUSER_BRANCHExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_user_branch($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_BRANCH = getTableNameEntity("user_branch", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEUSER_BRANCH~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEUSER_BRANCH~', ff($strTableNameUSER_BRANCH), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEUSER_BRANCH~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "USER_BRANCH", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_user_branch($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameUSER_BRANCH = getTableNameEntity("user_branch", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEUSER_BRANCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_BRANCH~', ff($strTableNameUSER_BRANCH), $strSQL);
...
function fetchValue_user_branch($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER_BRANCH = getTableNameEntity("user_branch", false);	

	$strWhere = buildWhere_user_branch($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEUSER_BRANCH~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER_BRANCH~', ff($strTableNameUSER_BRANCH), $strSQL);
...
function buildWhere_user_branch($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_user_branch($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "USER_BRANCH");
}