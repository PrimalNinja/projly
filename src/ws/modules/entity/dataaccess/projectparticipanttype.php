...
function add_projectparticipanttype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectparticipanttype($objConn_a);
	return addbulk_projectparticipanttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectparticipanttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTPARTICIPANTTYPE = getTableNameEntity("projectparticipanttype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectparticipanttype");
	$strCode = "PROJECTPARTICIPANTTYPE";
	$strDescription = "PROJECTPARTICIPANTTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffedcb7ebf-276b-40d9-9cc4-1f7ac1e2bc24";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTPARTICIPANTTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTTYPE~', ff($strTableNamePROJECTPARTICIPANTTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPARTICIPANTTYPE", $strResult, $strJSONData);
...
function delete_projectparticipanttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTTYPE = getTableNameEntity("projectparticipanttype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectparticipanttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPARTICIPANTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTTYPE~', ff($strTableNamePROJECTPARTICIPANTTYPE), $strSQL);
...
function deleteExtension_projectparticipanttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTTYPEExtension = getTableNameEntityExtension("projectparticipanttype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectparticipanttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTPARTICIPANTTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTTYPEEXTENSION~', ff($strTableNamePROJECTPARTICIPANTTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectparticipanttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTTYPE = getTableNameEntity("projectparticipanttype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTPARTICIPANTTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTTYPE~', ff($strTableNamePROJECTPARTICIPANTTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTPARTICIPANTTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTPARTICIPANTTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectparticipanttype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTPARTICIPANTTYPE = getTableNameEntity("projectparticipanttype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTPARTICIPANTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTTYPE~', ff($strTableNamePROJECTPARTICIPANTTYPE), $strSQL);
...
function fetchValue_projectparticipanttype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTPARTICIPANTTYPE = getTableNameEntity("projectparticipanttype", false);	

	$strWhere = buildWhere_projectparticipanttype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTPARTICIPANTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTPARTICIPANTTYPE~', ff($strTableNamePROJECTPARTICIPANTTYPE), $strSQL);
...
function buildWhere_projectparticipanttype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectparticipanttype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTPARTICIPANTTYPE");
}