...
function add_formlayout($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_formlayout($objConn_a);
	return addbulk_formlayout($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_formlayout($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "formlayout");
	$strCode = "FORMLAYOUT";
	$strDescription = "FORMLAYOUT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g38e1d8b1-b01e-44d6-bd90-102f0aa92f60";
    ...
    $strSQL = "insert into ~TABLENAMEFORMLAYOUT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FORMLAYOUT", $strResult, $strJSONData);
...
function delete_formlayout($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMLAYOUT = getTableNameEntity("formlayout", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formlayout($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMLAYOUT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMLAYOUT~', ff($strTableNameFORMLAYOUT), $strSQL);
...
function deleteExtension_formlayout($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMLAYOUTExtension = getTableNameEntityExtension("formlayout");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formlayout($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMLAYOUTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMLAYOUTEXTENSION~', ff($strTableNameFORMLAYOUTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_formlayout($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMLAYOUT = getTableNameEntity("formlayout", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFORMLAYOUT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFORMLAYOUT~', ff($strTableNameFORMLAYOUT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFORMLAYOUT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FORMLAYOUT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_formlayout($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFORMLAYOUT = getTableNameEntity("formlayout", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFORMLAYOUT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMLAYOUT~', ff($strTableNameFORMLAYOUT), $strSQL);
...
function fetchValue_formlayout($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMLAYOUT = getTableNameEntity("formlayout", false);	

	$strWhere = buildWhere_formlayout($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFORMLAYOUT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMLAYOUT~', ff($strTableNameFORMLAYOUT), $strSQL);
...
function buildWhere_formlayout($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_formlayout($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FORMLAYOUT");
}