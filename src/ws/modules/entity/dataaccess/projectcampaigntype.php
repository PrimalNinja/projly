...
function add_projectcampaigntype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectcampaigntype($objConn_a);
	return addbulk_projectcampaigntype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectcampaigntype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTCAMPAIGNTYPE = getTableNameEntity("projectcampaigntype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectcampaigntype");
	$strCode = "PROJECTCAMPAIGNTYPE";
	$strDescription = "PROJECTCAMPAIGNTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g338d2433-7a5b-45de-a505-0bd38e088fd3";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTCAMPAIGNTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTYPE~', ff($strTableNamePROJECTCAMPAIGNTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGNTYPE", $strResult, $strJSONData);
...
function delete_projectcampaigntype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTYPE = getTableNameEntity("projectcampaigntype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaigntype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGNTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTYPE~', ff($strTableNamePROJECTCAMPAIGNTYPE), $strSQL);
...
function deleteExtension_projectcampaigntype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTYPEExtension = getTableNameEntityExtension("projectcampaigntype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcampaigntype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCAMPAIGNTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTYPEEXTENSION~', ff($strTableNamePROJECTCAMPAIGNTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectcampaigntype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTYPE = getTableNameEntity("projectcampaigntype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTCAMPAIGNTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTYPE~', ff($strTableNamePROJECTCAMPAIGNTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTCAMPAIGNTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCAMPAIGNTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectcampaigntype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTCAMPAIGNTYPE = getTableNameEntity("projectcampaigntype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTCAMPAIGNTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTYPE~', ff($strTableNamePROJECTCAMPAIGNTYPE), $strSQL);
...
function fetchValue_projectcampaigntype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCAMPAIGNTYPE = getTableNameEntity("projectcampaigntype", false);	

	$strWhere = buildWhere_projectcampaigntype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTCAMPAIGNTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCAMPAIGNTYPE~', ff($strTableNamePROJECTCAMPAIGNTYPE), $strSQL);
...
function buildWhere_projectcampaigntype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectcampaigntype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTCAMPAIGNTYPE");
}