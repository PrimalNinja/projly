...
function add_xmit_gender($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_xmit_gender($objConn_a);
	return addbulk_xmit_gender($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_xmit_gender($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameXMIT_GENDER = getTableNameEntity("xmit_gender", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "xmit_gender");
	$strCode = "XMIT_GENDER";
	$strDescription = "XMIT_GENDER";
    ...
    $strSQL = "insert into ~TABLENAMEXMIT_GENDER~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEXMIT_GENDER~', ff($strTableNameXMIT_GENDER), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "XMIT_GENDER", $strResult, $strJSONData);
...
function delete_xmit_gender($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_GENDER = getTableNameEntity("xmit_gender", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_xmit_gender($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEXMIT_GENDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_GENDER~', ff($strTableNameXMIT_GENDER), $strSQL);
...
function deleteExtension_xmit_gender($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_GENDERExtension = getTableNameEntityExtension("xmit_gender");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_xmit_gender($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEXMIT_GENDEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_GENDEREXTENSION~', ff($strTableNameXMIT_GENDERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_xmit_gender($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_GENDER = getTableNameEntity("xmit_gender", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEXMIT_GENDER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEXMIT_GENDER~', ff($strTableNameXMIT_GENDER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEXMIT_GENDER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "XMIT_GENDER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_xmit_gender($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameXMIT_GENDER = getTableNameEntity("xmit_gender", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEXMIT_GENDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_GENDER~', ff($strTableNameXMIT_GENDER), $strSQL);
...
function fetchValue_xmit_gender($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameXMIT_GENDER = getTableNameEntity("xmit_gender", false);	

	$strWhere = buildWhere_xmit_gender($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEXMIT_GENDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEXMIT_GENDER~', ff($strTableNameXMIT_GENDER), $strSQL);
...
function buildWhere_xmit_gender($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_xmit_gender($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "XMIT_GENDER");
}