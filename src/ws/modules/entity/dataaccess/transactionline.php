...
function add_transactionline($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_transactionline($objConn_a);
	return addbulk_transactionline($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_transactionline($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "TRANSACTIONLINE";
	$strDescription = "TRANSACTIONLINE";
    ...
    $strSQL = "insert into ~TABLENAMETRANSACTIONLINE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONLINE", $strResult, $strJSONData);
...
function delete_transactionline($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINE = getTableNameEntity("transactionline", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionline($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONLINE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINE~', ff($strTableNameTRANSACTIONLINE), $strSQL);
...
function deleteExtension_transactionline($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINEExtension = getTableNameEntityExtension("transactionline");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transactionline($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONLINEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINEEXTENSION~', ff($strTableNameTRANSACTIONLINEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_transactionline($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINE = getTableNameEntity("transactionline", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETRANSACTIONLINE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETRANSACTIONLINE~', ff($strTableNameTRANSACTIONLINE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETRANSACTIONLINE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTIONLINE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_transactionline($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTRANSACTIONLINE = getTableNameEntity("transactionline", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETRANSACTIONLINE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINE~', ff($strTableNameTRANSACTIONLINE), $strSQL);
...
function fetchValue_transactionline($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONLINE = getTableNameEntity("transactionline", false);	

	$strWhere = buildWhere_transactionline($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETRANSACTIONLINE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONLINE~', ff($strTableNameTRANSACTIONLINE), $strSQL);
...
function buildWhere_transactionline($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_transactionline($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TRANSACTIONLINE");
}