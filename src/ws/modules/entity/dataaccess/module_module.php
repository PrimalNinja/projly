...
function add_module_module($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_module_module($objConn_a);
	return addbulk_module_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_module_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameMODULE_MODULE = getTableNameEntity("module_module", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "module_module");
	$strCode = "MODULE_MODULE";
	$strDescription = "MODULE_MODULE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ged463129-a2f4-4a2c-a9e9-8b48be17efff";
    ...
    $strSQL = "insert into ~TABLENAMEMODULE_MODULE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEMODULE_MODULE~', ff($strTableNameMODULE_MODULE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MODULE_MODULE", $strResult, $strJSONData);
...
function delete_module_module($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULE_MODULE = getTableNameEntity("module_module", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_module_module($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMODULE_MODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULE_MODULE~', ff($strTableNameMODULE_MODULE), $strSQL);
...
function deleteExtension_module_module($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULE_MODULEExtension = getTableNameEntityExtension("module_module");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_module_module($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMODULE_MODULEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULE_MODULEEXTENSION~', ff($strTableNameMODULE_MODULEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_module_module($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULE_MODULE = getTableNameEntity("module_module", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMODULE_MODULE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMODULE_MODULE~', ff($strTableNameMODULE_MODULE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMODULE_MODULE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MODULE_MODULE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_module_module($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMODULE_MODULE = getTableNameEntity("module_module", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMODULE_MODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULE_MODULE~', ff($strTableNameMODULE_MODULE), $strSQL);
...
function fetchValue_module_module($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMODULE_MODULE = getTableNameEntity("module_module", false);	

	$strWhere = buildWhere_module_module($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMODULE_MODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMODULE_MODULE~', ff($strTableNameMODULE_MODULE), $strSQL);
...
function buildWhere_module_module($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_module_module($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MODULE_MODULE");
}