...
function add_layout_reportsection($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_layout_reportsection($objConn_a);
	return addbulk_layout_reportsection($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_layout_reportsection($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameLAYOUT_REPORTSECTION = getTableNameEntity("layout_reportsection", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "layout_reportsection");
	$strCode = "LAYOUT_REPORTSECTION";
	$strDescription = "LAYOUT_REPORTSECTION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g85af610e-698b-4353-bb44-5bf56c91eadfr";
    ...
    $strSQL = "insert into ~TABLENAMELAYOUT_REPORTSECTION~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMELAYOUT_REPORTSECTION~', ff($strTableNameLAYOUT_REPORTSECTION), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "LAYOUT_REPORTSECTION", $strResult, $strJSONData);
...
function delete_layout_reportsection($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_REPORTSECTION = getTableNameEntity("layout_reportsection", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_layout_reportsection($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMELAYOUT_REPORTSECTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_REPORTSECTION~', ff($strTableNameLAYOUT_REPORTSECTION), $strSQL);
...
function deleteExtension_layout_reportsection($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_REPORTSECTIONExtension = getTableNameEntityExtension("layout_reportsection");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_layout_reportsection($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMELAYOUT_REPORTSECTIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_REPORTSECTIONEXTENSION~', ff($strTableNameLAYOUT_REPORTSECTIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_layout_reportsection($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_REPORTSECTION = getTableNameEntity("layout_reportsection", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMELAYOUT_REPORTSECTION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMELAYOUT_REPORTSECTION~', ff($strTableNameLAYOUT_REPORTSECTION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMELAYOUT_REPORTSECTION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "LAYOUT_REPORTSECTION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_layout_reportsection($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameLAYOUT_REPORTSECTION = getTableNameEntity("layout_reportsection", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMELAYOUT_REPORTSECTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_REPORTSECTION~', ff($strTableNameLAYOUT_REPORTSECTION), $strSQL);
...
function fetchValue_layout_reportsection($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameLAYOUT_REPORTSECTION = getTableNameEntity("layout_reportsection", false);	

	$strWhere = buildWhere_layout_reportsection($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMELAYOUT_REPORTSECTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMELAYOUT_REPORTSECTION~', ff($strTableNameLAYOUT_REPORTSECTION), $strSQL);
...
function buildWhere_layout_reportsection($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_layout_reportsection($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "LAYOUT_REPORTSECTION");
}