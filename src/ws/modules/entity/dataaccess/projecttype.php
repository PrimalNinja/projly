...
function add_projecttype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projecttype($objConn_a);
	return addbulk_projecttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projecttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "projecttype");
	$strCode = "PROJECTTYPE";
	$strDescription = "PROJECTTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g18844c11-1d1a-442a-b55b-f43770a0dbf8";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTYPE", $strResult, $strJSONData);
...
function delete_projecttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTYPE = getTableNameEntity("projecttype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTYPE~', ff($strTableNamePROJECTTYPE), $strSQL);
...
function deleteExtension_projecttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTYPEExtension = getTableNameEntityExtension("projecttype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTYPEEXTENSION~', ff($strTableNamePROJECTTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projecttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTYPE = getTableNameEntity("projecttype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTTYPE~', ff($strTableNamePROJECTTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projecttype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTTYPE = getTableNameEntity("projecttype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTYPE~', ff($strTableNamePROJECTTYPE), $strSQL);
...
function fetchValue_projecttype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTYPE = getTableNameEntity("projecttype", false);	

	$strWhere = buildWhere_projecttype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTYPE~', ff($strTableNamePROJECTTYPE), $strSQL);
...
function buildWhere_projecttype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projecttype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTTYPE");
}