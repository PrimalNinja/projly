...
function add_documenttype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_documenttype($objConn_a);
	return addbulk_documenttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_documenttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "documenttype");
	$strCode = "DOCUMENTTYPE";
	$strDescription = "DOCUMENTTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff30c76a92-2b89-4a4f-9710-7d9a3de8c8b2";
    ...
    $strSQL = "insert into ~TABLENAMEDOCUMENTTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENTTYPE", $strResult, $strJSONData);
...
function delete_documenttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTTYPE = getTableNameEntity("documenttype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_documenttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTTYPE~', ff($strTableNameDOCUMENTTYPE), $strSQL);
...
function deleteExtension_documenttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTTYPEExtension = getTableNameEntityExtension("documenttype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_documenttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENTTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTTYPEEXTENSION~', ff($strTableNameDOCUMENTTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_documenttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTTYPE = getTableNameEntity("documenttype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDOCUMENTTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDOCUMENTTYPE~', ff($strTableNameDOCUMENTTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDOCUMENTTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENTTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_documenttype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDOCUMENTTYPE = getTableNameEntity("documenttype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDOCUMENTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTTYPE~', ff($strTableNameDOCUMENTTYPE), $strSQL);
...
function fetchValue_documenttype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTTYPE = getTableNameEntity("documenttype", false);	

	$strWhere = buildWhere_documenttype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDOCUMENTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTTYPE~', ff($strTableNameDOCUMENTTYPE), $strSQL);
...
function buildWhere_documenttype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_documenttype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DOCUMENTTYPE");
}