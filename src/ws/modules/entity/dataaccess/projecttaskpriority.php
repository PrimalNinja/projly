...
function add_projecttaskpriority($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projecttaskpriority($objConn_a);
	return addbulk_projecttaskpriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projecttaskpriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTTASKPRIORITY = getTableNameEntity("projecttaskpriority", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projecttaskpriority");
	$strCode = "PROJECTTASKPRIORITY";
	$strDescription = "PROJECTTASKPRIORITY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff1fa9cfa1-c379-4e30-978f-18c734213de3";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTTASKPRIORITY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTTASKPRIORITY~', ff($strTableNamePROJECTTASKPRIORITY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTASKPRIORITY", $strResult, $strJSONData);
...
function delete_projecttaskpriority($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKPRIORITY = getTableNameEntity("projecttaskpriority", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttaskpriority($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTASKPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKPRIORITY~', ff($strTableNamePROJECTTASKPRIORITY), $strSQL);
...
function deleteExtension_projecttaskpriority($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKPRIORITYExtension = getTableNameEntityExtension("projecttaskpriority");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttaskpriority($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTASKPRIORITYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKPRIORITYEXTENSION~', ff($strTableNamePROJECTTASKPRIORITYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projecttaskpriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKPRIORITY = getTableNameEntity("projecttaskpriority", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTTASKPRIORITY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTTASKPRIORITY~', ff($strTableNamePROJECTTASKPRIORITY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTTASKPRIORITY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTASKPRIORITY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projecttaskpriority($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTTASKPRIORITY = getTableNameEntity("projecttaskpriority", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTTASKPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKPRIORITY~', ff($strTableNamePROJECTTASKPRIORITY), $strSQL);
...
function fetchValue_projecttaskpriority($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKPRIORITY = getTableNameEntity("projecttaskpriority", false);	

	$strWhere = buildWhere_projecttaskpriority($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTTASKPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKPRIORITY~', ff($strTableNamePROJECTTASKPRIORITY), $strSQL);
...
function buildWhere_projecttaskpriority($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projecttaskpriority($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTTASKPRIORITY");
}