...
function add_authenticationplugin($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_authenticationplugin($objConn_a);
	return addbulk_authenticationplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_authenticationplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameAUTHENTICATIONPLUGIN = getTableNameEntity("authenticationplugin", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "authenticationplugin");
	$strCode = "AUTHENTICATIONPLUGIN";
	$strDescription = "AUTHENTICATIONPLUGIN";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gf65959b5-cfc6-4134-8adf-5a622a52cb80";
    ...
    $strSQL = "insert into ~TABLENAMEAUTHENTICATIONPLUGIN~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONPLUGIN~', ff($strTableNameAUTHENTICATIONPLUGIN), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "AUTHENTICATIONPLUGIN", $strResult, $strJSONData);
...
function delete_authenticationplugin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONPLUGIN = getTableNameEntity("authenticationplugin", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_authenticationplugin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAUTHENTICATIONPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONPLUGIN~', ff($strTableNameAUTHENTICATIONPLUGIN), $strSQL);
...
function deleteExtension_authenticationplugin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONPLUGINExtension = getTableNameEntityExtension("authenticationplugin");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_authenticationplugin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEAUTHENTICATIONPLUGINEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONPLUGINEXTENSION~', ff($strTableNameAUTHENTICATIONPLUGINExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_authenticationplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONPLUGIN = getTableNameEntity("authenticationplugin", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEAUTHENTICATIONPLUGIN~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEAUTHENTICATIONPLUGIN~', ff($strTableNameAUTHENTICATIONPLUGIN), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEAUTHENTICATIONPLUGIN~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "AUTHENTICATIONPLUGIN", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_authenticationplugin($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameAUTHENTICATIONPLUGIN = getTableNameEntity("authenticationplugin", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEAUTHENTICATIONPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONPLUGIN~', ff($strTableNameAUTHENTICATIONPLUGIN), $strSQL);
...
function fetchValue_authenticationplugin($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameAUTHENTICATIONPLUGIN = getTableNameEntity("authenticationplugin", false);	

	$strWhere = buildWhere_authenticationplugin($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEAUTHENTICATIONPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEAUTHENTICATIONPLUGIN~', ff($strTableNameAUTHENTICATIONPLUGIN), $strSQL);
...
function buildWhere_authenticationplugin($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_authenticationplugin($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "AUTHENTICATIONPLUGIN");
}