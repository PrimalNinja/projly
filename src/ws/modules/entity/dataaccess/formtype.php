...
function add_formtype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_formtype($objConn_a);
	return addbulk_formtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_formtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFORMTYPE = getTableNameEntity("formtype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "formtype");
	$strCode = "FORMTYPE";
	$strDescription = "FORMTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g6250a04a-0898-6250-5043-6250a04a1";
    ...
    $strSQL = "insert into ~TABLENAMEFORMTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFORMTYPE~', ff($strTableNameFORMTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FORMTYPE", $strResult, $strJSONData);
...
function delete_formtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMTYPE = getTableNameEntity("formtype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMTYPE~', ff($strTableNameFORMTYPE), $strSQL);
...
function deleteExtension_formtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMTYPEExtension = getTableNameEntityExtension("formtype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMTYPEEXTENSION~', ff($strTableNameFORMTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_formtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMTYPE = getTableNameEntity("formtype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFORMTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFORMTYPE~', ff($strTableNameFORMTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFORMTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FORMTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_formtype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFORMTYPE = getTableNameEntity("formtype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFORMTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMTYPE~', ff($strTableNameFORMTYPE), $strSQL);
...
function fetchValue_formtype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMTYPE = getTableNameEntity("formtype", false);	

	$strWhere = buildWhere_formtype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFORMTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMTYPE~', ff($strTableNameFORMTYPE), $strSQL);
...
function buildWhere_formtype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_formtype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FORMTYPE");
}