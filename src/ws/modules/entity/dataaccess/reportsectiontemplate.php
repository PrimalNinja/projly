...
function add_reportsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_reportsectiontemplate($objConn_a);
	return addbulk_reportsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_reportsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameREPORTSECTIONTEMPLATE = getTableNameEntity("reportsectiontemplate", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "reportsectiontemplate");
	$strCode = "REPORTSECTIONTEMPLATE";
	$strDescription = "REPORTSECTIONTEMPLATE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gc4b6cc8f-e5d8-4c37-b96a-232f447bedfdr";
    ...
    $strSQL = "insert into ~TABLENAMEREPORTSECTIONTEMPLATE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE~', ff($strTableNameREPORTSECTIONTEMPLATE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTSECTIONTEMPLATE", $strResult, $strJSONData);
...
function delete_reportsectiontemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATE = getTableNameEntity("reportsectiontemplate", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportsectiontemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTSECTIONTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE~', ff($strTableNameREPORTSECTIONTEMPLATE), $strSQL);
...
function deleteExtension_reportsectiontemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATEExtension = getTableNameEntityExtension("reportsectiontemplate");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_reportsectiontemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEREPORTSECTIONTEMPLATEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATEEXTENSION~', ff($strTableNameREPORTSECTIONTEMPLATEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_reportsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATE = getTableNameEntity("reportsectiontemplate", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEREPORTSECTIONTEMPLATE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE~', ff($strTableNameREPORTSECTIONTEMPLATE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEREPORTSECTIONTEMPLATE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "REPORTSECTIONTEMPLATE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_reportsectiontemplate($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameREPORTSECTIONTEMPLATE = getTableNameEntity("reportsectiontemplate", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEREPORTSECTIONTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE~', ff($strTableNameREPORTSECTIONTEMPLATE), $strSQL);
...
function fetchValue_reportsectiontemplate($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameREPORTSECTIONTEMPLATE = getTableNameEntity("reportsectiontemplate", false);	

	$strWhere = buildWhere_reportsectiontemplate($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEREPORTSECTIONTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEREPORTSECTIONTEMPLATE~', ff($strTableNameREPORTSECTIONTEMPLATE), $strSQL);
...
function buildWhere_reportsectiontemplate($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_reportsectiontemplate($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "REPORTSECTIONTEMPLATE");
}