...
function add_projecttask($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projecttask($objConn_a);
	return addbulk_projecttask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projecttask($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "projecttask");
	$strCode = "PROJECTTASK";
	$strDescription = "PROJECTTASK";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffe63e2a32-d72a-474c-82d0-b664eef1268a";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTTASK~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTASK", $strResult, $strJSONData);
...
function delete_projecttask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASK = getTableNameEntity("projecttask", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASK~', ff($strTableNamePROJECTTASK), $strSQL);
...
function deleteExtension_projecttask($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKExtension = getTableNameEntityExtension("projecttask");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttask($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTASKEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKEXTENSION~', ff($strTableNamePROJECTTASKExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projecttask($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASK = getTableNameEntity("projecttask", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTTASK~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTTASK~', ff($strTableNamePROJECTTASK), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTTASK~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTASK", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projecttask($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTTASK = getTableNameEntity("projecttask", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASK~', ff($strTableNamePROJECTTASK), $strSQL);
...
function fetchValue_projecttask($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASK = getTableNameEntity("projecttask", false);	

	$strWhere = buildWhere_projecttask($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTTASK~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASK~', ff($strTableNamePROJECTTASK), $strSQL);
...
function buildWhere_projecttask($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projecttask($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTTASK");
}