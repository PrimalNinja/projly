...
function add_projectissue($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectissue($objConn_a);
	return addbulk_projectissue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectissue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "projectissue");
	$strCode = "PROJECTISSUE";
	$strDescription = "PROJECTISSUE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffa99b8231-dfd1-424f-be14-c9052b2bdbe6";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTISSUE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUE", $strResult, $strJSONData);
...
function delete_projectissue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUE = getTableNameEntity("projectissue", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUE~', ff($strTableNamePROJECTISSUE), $strSQL);
...
function deleteExtension_projectissue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUEExtension = getTableNameEntityExtension("projectissue");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUEEXTENSION~', ff($strTableNamePROJECTISSUEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectissue($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUE = getTableNameEntity("projectissue", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTISSUE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTISSUE~', ff($strTableNamePROJECTISSUE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTISSUE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectissue($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTISSUE = getTableNameEntity("projectissue", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTISSUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUE~', ff($strTableNamePROJECTISSUE), $strSQL);
...
function fetchValue_projectissue($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUE = getTableNameEntity("projectissue", false);	

	$strWhere = buildWhere_projectissue($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTISSUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUE~', ff($strTableNamePROJECTISSUE), $strSQL);
...
function buildWhere_projectissue($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectissue($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTISSUE");
}