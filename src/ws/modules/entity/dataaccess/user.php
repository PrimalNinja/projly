...
function add_user($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_user($objConn_a);
	return addbulk_user($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_user($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "1d3fbeab-d440-4329-9b9c-aa93f6fe7c16";
    ...
    $strSQL = "insert into ~TABLENAMEUSER~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "USER", $strResult, $strJSONData);
...
function delete_user($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER = getTableNameEntity("user", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_user($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER~', ff($strTableNameUSER), $strSQL);
...
function deleteExtension_user($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSERExtension = getTableNameEntityExtension("user");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_user($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEUSEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSEREXTENSION~', ff($strTableNameUSERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_user($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER = getTableNameEntity("user", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEUSER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEUSER~', ff($strTableNameUSER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEUSER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "USER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_user($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameUSER = getTableNameEntity("user", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEUSER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER~', ff($strTableNameUSER), $strSQL);
...
function fetchValue_user($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameUSER = getTableNameEntity("user", false);	

	$strWhere = buildWhere_user($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEUSER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEUSER~', ff($strTableNameUSER), $strSQL);
...
function buildWhere_user($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_user($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "USER");
}