...
function add_formsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_formsectiontemplate($objConn_a);
	return addbulk_formsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_formsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFORMSECTIONTEMPLATE = getTableNameEntity("formsectiontemplate", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "formsectiontemplate");
	$strCode = "FORMSECTIONTEMPLATE";
	$strDescription = "FORMSECTIONTEMPLATE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gc4b6cc8f-e5d8-4c37-b96a-232f447bedfd";
    ...
    $strSQL = "insert into ~TABLENAMEFORMSECTIONTEMPLATE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFORMSECTIONTEMPLATE~', ff($strTableNameFORMSECTIONTEMPLATE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FORMSECTIONTEMPLATE", $strResult, $strJSONData);
...
function delete_formsectiontemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTIONTEMPLATE = getTableNameEntity("formsectiontemplate", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formsectiontemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMSECTIONTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTIONTEMPLATE~', ff($strTableNameFORMSECTIONTEMPLATE), $strSQL);
...
function deleteExtension_formsectiontemplate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTIONTEMPLATEExtension = getTableNameEntityExtension("formsectiontemplate");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_formsectiontemplate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFORMSECTIONTEMPLATEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTIONTEMPLATEEXTENSION~', ff($strTableNameFORMSECTIONTEMPLATEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_formsectiontemplate($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTIONTEMPLATE = getTableNameEntity("formsectiontemplate", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFORMSECTIONTEMPLATE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFORMSECTIONTEMPLATE~', ff($strTableNameFORMSECTIONTEMPLATE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFORMSECTIONTEMPLATE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FORMSECTIONTEMPLATE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_formsectiontemplate($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFORMSECTIONTEMPLATE = getTableNameEntity("formsectiontemplate", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFORMSECTIONTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTIONTEMPLATE~', ff($strTableNameFORMSECTIONTEMPLATE), $strSQL);
...
function fetchValue_formsectiontemplate($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFORMSECTIONTEMPLATE = getTableNameEntity("formsectiontemplate", false);	

	$strWhere = buildWhere_formsectiontemplate($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFORMSECTIONTEMPLATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFORMSECTIONTEMPLATE~', ff($strTableNameFORMSECTIONTEMPLATE), $strSQL);
...
function buildWhere_formsectiontemplate($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_formsectiontemplate($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FORMSECTIONTEMPLATE");
}