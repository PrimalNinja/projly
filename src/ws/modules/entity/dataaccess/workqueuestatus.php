...
function add_workqueuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_workqueuestatus($objConn_a);
	return addbulk_workqueuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_workqueuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "workqueuestatus");
	$strCode = "WORKQUEUESTATUS";
	$strDescription = "WORKQUEUESTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff24319a52-14a2-4aae-a91a-4c3a5e7f42ce";
    ...
    $strSQL = "insert into ~TABLENAMEWORKQUEUESTATUS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "WORKQUEUESTATUS", $strResult, $strJSONData);
...
function delete_workqueuestatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUESTATUS = getTableNameEntity("workqueuestatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_workqueuestatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEWORKQUEUESTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUESTATUS~', ff($strTableNameWORKQUEUESTATUS), $strSQL);
...
function deleteExtension_workqueuestatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUESTATUSExtension = getTableNameEntityExtension("workqueuestatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_workqueuestatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEWORKQUEUESTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUESTATUSEXTENSION~', ff($strTableNameWORKQUEUESTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_workqueuestatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUESTATUS = getTableNameEntity("workqueuestatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEWORKQUEUESTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEWORKQUEUESTATUS~', ff($strTableNameWORKQUEUESTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEWORKQUEUESTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "WORKQUEUESTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_workqueuestatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameWORKQUEUESTATUS = getTableNameEntity("workqueuestatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEWORKQUEUESTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUESTATUS~', ff($strTableNameWORKQUEUESTATUS), $strSQL);
...
function fetchValue_workqueuestatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUESTATUS = getTableNameEntity("workqueuestatus", false);	

	$strWhere = buildWhere_workqueuestatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEWORKQUEUESTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUESTATUS~', ff($strTableNameWORKQUEUESTATUS), $strSQL);
...
function buildWhere_workqueuestatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_workqueuestatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "WORKQUEUESTATUS");
}