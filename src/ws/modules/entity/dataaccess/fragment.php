...
function add_fragment($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_fragment($objConn_a);
	return addbulk_fragment($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_fragment($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFRAGMENT = getTableNameEntity("fragment", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "fragment");
	$strCode = "FRAGMENT";
	$strDescription = "FRAGMENT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g1d98b15a-00b7-41e8-839a-ed03b2589293";
    ...
    $strSQL = "insert into ~TABLENAMEFRAGMENT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFRAGMENT~', ff($strTableNameFRAGMENT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FRAGMENT", $strResult, $strJSONData);
...
function delete_fragment($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFRAGMENT = getTableNameEntity("fragment", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fragment($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFRAGMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFRAGMENT~', ff($strTableNameFRAGMENT), $strSQL);
...
function deleteExtension_fragment($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFRAGMENTExtension = getTableNameEntityExtension("fragment");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fragment($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFRAGMENTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFRAGMENTEXTENSION~', ff($strTableNameFRAGMENTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_fragment($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFRAGMENT = getTableNameEntity("fragment", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFRAGMENT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFRAGMENT~', ff($strTableNameFRAGMENT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFRAGMENT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FRAGMENT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_fragment($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFRAGMENT = getTableNameEntity("fragment", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFRAGMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFRAGMENT~', ff($strTableNameFRAGMENT), $strSQL);
...
function fetchValue_fragment($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFRAGMENT = getTableNameEntity("fragment", false);	

	$strWhere = buildWhere_fragment($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFRAGMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFRAGMENT~', ff($strTableNameFRAGMENT), $strSQL);
...
function buildWhere_fragment($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_fragment($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FRAGMENT");
}