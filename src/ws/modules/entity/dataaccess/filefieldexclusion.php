...
function add_filefieldexclusion($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_filefieldexclusion($objConn_a);
	return addbulk_filefieldexclusion($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_filefieldexclusion($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "filefieldexclusion");
	$strCode = "FILEFIELDEXCLUSION";
	$strDescription = "FILEFIELDEXCLUSION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g44107c60-2977-4385-9eb0-2bfc71bb76e4";
    ...
    $strSQL = "insert into ~TABLENAMEFILEFIELDEXCLUSION~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFIELDEXCLUSION", $strResult, $strJSONData);
...
function delete_filefieldexclusion($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDEXCLUSION = getTableNameEntity("filefieldexclusion", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filefieldexclusion($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFIELDEXCLUSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDEXCLUSION~', ff($strTableNameFILEFIELDEXCLUSION), $strSQL);
...
function deleteExtension_filefieldexclusion($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDEXCLUSIONExtension = getTableNameEntityExtension("filefieldexclusion");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_filefieldexclusion($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFIELDEXCLUSIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDEXCLUSIONEXTENSION~', ff($strTableNameFILEFIELDEXCLUSIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_filefieldexclusion($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDEXCLUSION = getTableNameEntity("filefieldexclusion", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILEFIELDEXCLUSION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILEFIELDEXCLUSION~', ff($strTableNameFILEFIELDEXCLUSION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILEFIELDEXCLUSION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFIELDEXCLUSION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_filefieldexclusion($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILEFIELDEXCLUSION = getTableNameEntity("filefieldexclusion", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILEFIELDEXCLUSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDEXCLUSION~', ff($strTableNameFILEFIELDEXCLUSION), $strSQL);
...
function fetchValue_filefieldexclusion($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFIELDEXCLUSION = getTableNameEntity("filefieldexclusion", false);	

	$strWhere = buildWhere_filefieldexclusion($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILEFIELDEXCLUSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFIELDEXCLUSION~', ff($strTableNameFILEFIELDEXCLUSION), $strSQL);
...
function buildWhere_filefieldexclusion($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_filefieldexclusion($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILEFIELDEXCLUSION");
}