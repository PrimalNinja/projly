...
function add_negotiatedrate($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_negotiatedrate($objConn_a);
	return addbulk_negotiatedrate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_negotiatedrate($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "negotiatedrate");
	$strCode = "NEGOTIATEDRATE";
	$strDescription = "NEGOTIATEDRATE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g56ec2ec9-a7c4-43cc-a6ac-9f6a710e3e94";
    ...
    $strSQL = "insert into ~TABLENAMENEGOTIATEDRATE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "NEGOTIATEDRATE", $strResult, $strJSONData);
...
function delete_negotiatedrate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDRATE = getTableNameEntity("negotiatedrate", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_negotiatedrate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMENEGOTIATEDRATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDRATE~', ff($strTableNameNEGOTIATEDRATE), $strSQL);
...
function deleteExtension_negotiatedrate($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDRATEExtension = getTableNameEntityExtension("negotiatedrate");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_negotiatedrate($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMENEGOTIATEDRATEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDRATEEXTENSION~', ff($strTableNameNEGOTIATEDRATEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_negotiatedrate($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDRATE = getTableNameEntity("negotiatedrate", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMENEGOTIATEDRATE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMENEGOTIATEDRATE~', ff($strTableNameNEGOTIATEDRATE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMENEGOTIATEDRATE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "NEGOTIATEDRATE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_negotiatedrate($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameNEGOTIATEDRATE = getTableNameEntity("negotiatedrate", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMENEGOTIATEDRATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDRATE~', ff($strTableNameNEGOTIATEDRATE), $strSQL);
...
function fetchValue_negotiatedrate($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameNEGOTIATEDRATE = getTableNameEntity("negotiatedrate", false);	

	$strWhere = buildWhere_negotiatedrate($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMENEGOTIATEDRATE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMENEGOTIATEDRATE~', ff($strTableNameNEGOTIATEDRATE), $strSQL);
...
function buildWhere_negotiatedrate($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_negotiatedrate($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "NEGOTIATEDRATE");
}