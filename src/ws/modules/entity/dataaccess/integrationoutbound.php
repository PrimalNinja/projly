...
function add_integrationoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationoutbound($objConn_a);
	return addbulk_integrationoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "integrationoutbound");
	$strCode = "INTEGRATIONOUTBOUND";
	$strDescription = "INTEGRATIONOUTBOUND";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g561d6970-cd14-4e7b-8ac4-460ae9f6a750";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONOUTBOUND~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONOUTBOUND", $strResult, $strJSONData);
...
function delete_integrationoutbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUND = getTableNameEntity("integrationoutbound", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationoutbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONOUTBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUND~', ff($strTableNameINTEGRATIONOUTBOUND), $strSQL);
...
function deleteExtension_integrationoutbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDExtension = getTableNameEntityExtension("integrationoutbound");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationoutbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONOUTBOUNDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDEXTENSION~', ff($strTableNameINTEGRATIONOUTBOUNDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationoutbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUND = getTableNameEntity("integrationoutbound", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONOUTBOUND~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUND~', ff($strTableNameINTEGRATIONOUTBOUND), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONOUTBOUND~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONOUTBOUND", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationoutbound($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONOUTBOUND = getTableNameEntity("integrationoutbound", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONOUTBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUND~', ff($strTableNameINTEGRATIONOUTBOUND), $strSQL);
...
function fetchValue_integrationoutbound($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUND = getTableNameEntity("integrationoutbound", false);	

	$strWhere = buildWhere_integrationoutbound($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONOUTBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUND~', ff($strTableNameINTEGRATIONOUTBOUND), $strSQL);
...
function buildWhere_integrationoutbound($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationoutbound($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONOUTBOUND");
}