...
function add_config($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_config($objConn_a);
	return addbulk_config($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_config($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "config");
	$strCode = "CONFIG";
	$strDescription = "CONFIG";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gb018840e-bc4f-45fc-b8a6-2c6338ca1be1";
    ...
    $strSQL = "insert into ~TABLENAMECONFIG~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CONFIG", $strResult, $strJSONData);
...
function delete_config($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIG = getTableNameEntity("config", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_config($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONFIG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIG~', ff($strTableNameCONFIG), $strSQL);
...
function deleteExtension_config($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIGExtension = getTableNameEntityExtension("config");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_config($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONFIGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIGEXTENSION~', ff($strTableNameCONFIGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_config($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIG = getTableNameEntity("config", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECONFIG~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECONFIG~', ff($strTableNameCONFIG), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECONFIG~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CONFIG", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_config($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCONFIG = getTableNameEntity("config", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECONFIG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIG~', ff($strTableNameCONFIG), $strSQL);
...
function fetchValue_config($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONFIG = getTableNameEntity("config", false);	

	$strWhere = buildWhere_config($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECONFIG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONFIG~', ff($strTableNameCONFIG), $strSQL);
...
function buildWhere_config($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_config($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CONFIG");
}