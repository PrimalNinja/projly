...
function add_internalmessagefolder($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_internalmessagefolder($objConn_a);
	return addbulk_internalmessagefolder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_internalmessagefolder($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "internalmessagefolder");
	$strCode = "INTERNALMESSAGEFOLDER";
	$strDescription = "INTERNALMESSAGEFOLDER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gc37b1759-00e2-4ef2-a20e-f8b43637ac05";
    ...
    $strSQL = "insert into ~TABLENAMEINTERNALMESSAGEFOLDER~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTERNALMESSAGEFOLDER", $strResult, $strJSONData);
...
function delete_internalmessagefolder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGEFOLDER = getTableNameEntity("internalmessagefolder", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_internalmessagefolder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTERNALMESSAGEFOLDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGEFOLDER~', ff($strTableNameINTERNALMESSAGEFOLDER), $strSQL);
...
function deleteExtension_internalmessagefolder($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGEFOLDERExtension = getTableNameEntityExtension("internalmessagefolder");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_internalmessagefolder($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTERNALMESSAGEFOLDEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGEFOLDEREXTENSION~', ff($strTableNameINTERNALMESSAGEFOLDERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_internalmessagefolder($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGEFOLDER = getTableNameEntity("internalmessagefolder", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTERNALMESSAGEFOLDER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTERNALMESSAGEFOLDER~', ff($strTableNameINTERNALMESSAGEFOLDER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTERNALMESSAGEFOLDER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTERNALMESSAGEFOLDER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_internalmessagefolder($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTERNALMESSAGEFOLDER = getTableNameEntity("internalmessagefolder", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTERNALMESSAGEFOLDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGEFOLDER~', ff($strTableNameINTERNALMESSAGEFOLDER), $strSQL);
...
function fetchValue_internalmessagefolder($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTERNALMESSAGEFOLDER = getTableNameEntity("internalmessagefolder", false);	

	$strWhere = buildWhere_internalmessagefolder($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTERNALMESSAGEFOLDER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTERNALMESSAGEFOLDER~', ff($strTableNameINTERNALMESSAGEFOLDER), $strSQL);
...
function buildWhere_internalmessagefolder($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_internalmessagefolder($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTERNALMESSAGEFOLDER");
}