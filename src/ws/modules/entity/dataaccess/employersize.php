...
function add_employersize($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_employersize($objConn_a);
	return addbulk_employersize($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_employersize($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameEMPLOYERSIZE = getTableNameEntity("employersize", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "employersize");
	$strCode = "EMPLOYERSIZE";
	$strDescription = "EMPLOYERSIZE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "a9dfd9e5-7f70-455a-9dc7-dbda4dd9b38d";
    ...
    $strSQL = "insert into ~TABLENAMEEMPLOYERSIZE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEEMPLOYERSIZE~', ff($strTableNameEMPLOYERSIZE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "EMPLOYERSIZE", $strResult, $strJSONData);
...
function delete_employersize($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameEMPLOYERSIZE = getTableNameEntity("employersize", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_employersize($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEEMPLOYERSIZE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEEMPLOYERSIZE~', ff($strTableNameEMPLOYERSIZE), $strSQL);
...
function deleteExtension_employersize($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameEMPLOYERSIZEExtension = getTableNameEntityExtension("employersize");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_employersize($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEEMPLOYERSIZEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEEMPLOYERSIZEEXTENSION~', ff($strTableNameEMPLOYERSIZEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_employersize($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameEMPLOYERSIZE = getTableNameEntity("employersize", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEEMPLOYERSIZE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEEMPLOYERSIZE~', ff($strTableNameEMPLOYERSIZE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEEMPLOYERSIZE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "EMPLOYERSIZE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_employersize($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameEMPLOYERSIZE = getTableNameEntity("employersize", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEEMPLOYERSIZE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEEMPLOYERSIZE~', ff($strTableNameEMPLOYERSIZE), $strSQL);
...
function fetchValue_employersize($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameEMPLOYERSIZE = getTableNameEntity("employersize", false);	

	$strWhere = buildWhere_employersize($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEEMPLOYERSIZE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEEMPLOYERSIZE~', ff($strTableNameEMPLOYERSIZE), $strSQL);
...
function buildWhere_employersize($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_employersize($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "EMPLOYERSIZE");
}