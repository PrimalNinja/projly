...
function add_systemflagvalue($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemflagvalue($objConn_a);
	return addbulk_systemflagvalue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemflagvalue($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSYSTEMFLAGVALUE = getTableNameEntity("systemflagvalue", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "systemflagvalue");
	$strCode = "SYSTEMFLAGVALUE";
	$strDescription = "SYSTEMFLAGVALUE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ge13ca38f-41a4-4363-a9ea-c9262cfc65ac";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMFLAGVALUE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGVALUE~', ff($strTableNameSYSTEMFLAGVALUE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMFLAGVALUE", $strResult, $strJSONData);
...
function delete_systemflagvalue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGVALUE = getTableNameEntity("systemflagvalue", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemflagvalue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMFLAGVALUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGVALUE~', ff($strTableNameSYSTEMFLAGVALUE), $strSQL);
...
function deleteExtension_systemflagvalue($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGVALUEExtension = getTableNameEntityExtension("systemflagvalue");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemflagvalue($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMFLAGVALUEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGVALUEEXTENSION~', ff($strTableNameSYSTEMFLAGVALUEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemflagvalue($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGVALUE = getTableNameEntity("systemflagvalue", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMFLAGVALUE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMFLAGVALUE~', ff($strTableNameSYSTEMFLAGVALUE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMFLAGVALUE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMFLAGVALUE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemflagvalue($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMFLAGVALUE = getTableNameEntity("systemflagvalue", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMFLAGVALUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGVALUE~', ff($strTableNameSYSTEMFLAGVALUE), $strSQL);
...
function fetchValue_systemflagvalue($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMFLAGVALUE = getTableNameEntity("systemflagvalue", false);	

	$strWhere = buildWhere_systemflagvalue($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMFLAGVALUE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMFLAGVALUE~', ff($strTableNameSYSTEMFLAGVALUE), $strSQL);
...
function buildWhere_systemflagvalue($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemflagvalue($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMFLAGVALUE");
}