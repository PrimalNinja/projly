...
function add_fileformattemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_fileformattemplate_field($objConn_a);
	return addbulk_fileformattemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_fileformattemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFILEFORMATTEMPLATE_FIELD = getTableNameEntity("fileformattemplate_field", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "fileformattemplate_field");
	$strCode = "FILEFORMATTEMPLATE_FIELD";
	$strDescription = "FILEFORMATTEMPLATE_FIELD";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g17b76dbc-4845-4184-9757-5b7c5d52818b";
    ...
    $strSQL = "insert into ~TABLENAMEFILEFORMATTEMPLATE_FIELD~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE_FIELD~', ff($strTableNameFILEFORMATTEMPLATE_FIELD), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMATTEMPLATE_FIELD", $strResult, $strJSONData);
...
function delete_fileformattemplate_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATE_FIELD = getTableNameEntity("fileformattemplate_field", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformattemplate_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMATTEMPLATE_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE_FIELD~', ff($strTableNameFILEFORMATTEMPLATE_FIELD), $strSQL);
...
function deleteExtension_fileformattemplate_field($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATE_FIELDExtension = getTableNameEntityExtension("fileformattemplate_field");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_fileformattemplate_field($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFILEFORMATTEMPLATE_FIELDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE_FIELDEXTENSION~', ff($strTableNameFILEFORMATTEMPLATE_FIELDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_fileformattemplate_field($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATE_FIELD = getTableNameEntity("fileformattemplate_field", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFILEFORMATTEMPLATE_FIELD~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE_FIELD~', ff($strTableNameFILEFORMATTEMPLATE_FIELD), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFILEFORMATTEMPLATE_FIELD~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FILEFORMATTEMPLATE_FIELD", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_fileformattemplate_field($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFILEFORMATTEMPLATE_FIELD = getTableNameEntity("fileformattemplate_field", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFILEFORMATTEMPLATE_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE_FIELD~', ff($strTableNameFILEFORMATTEMPLATE_FIELD), $strSQL);
...
function fetchValue_fileformattemplate_field($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFILEFORMATTEMPLATE_FIELD = getTableNameEntity("fileformattemplate_field", false);	

	$strWhere = buildWhere_fileformattemplate_field($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFILEFORMATTEMPLATE_FIELD~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFILEFORMATTEMPLATE_FIELD~', ff($strTableNameFILEFORMATTEMPLATE_FIELD), $strSQL);
...
function buildWhere_fileformattemplate_field($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_fileformattemplate_field($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FILEFORMATTEMPLATE_FIELD");
}