...
function add_%%ENTITY_LCASE%%($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_%%ENTITY_LCASE%%($objConn_a);
	return addbulk_%%ENTITY_LCASE%%($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_%%ENTITY_LCASE%%($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableName%%ENTITY_UCASE%% = getTableNameEntity("%%ENTITY_LCASE%%", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "%%ENTITY_LCASE%%");
	$strCode = "%%ENTITY_UCASE%%";
	$strDescription = "%%ENTITY_UCASE%%";
    ...
    $strSQL = "insert into ~TABLENAME%%ENTITY_UCASE%%~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAME%%ENTITY_UCASE%%~', ff($strTableName%%ENTITY_UCASE%%), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "%%ENTITY_UCASE%%", $strResult, $strJSONData);
...
function delete_%%ENTITY_LCASE%%($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableName%%ENTITY_UCASE%% = getTableNameEntity("%%ENTITY_LCASE%%", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_%%ENTITY_LCASE%%($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAME%%ENTITY_UCASE%%~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAME%%ENTITY_UCASE%%~', ff($strTableName%%ENTITY_UCASE%%), $strSQL);
...
function deleteExtension_%%ENTITY_LCASE%%($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableName%%ENTITY_UCASE%%Extension = getTableNameEntityExtension("%%ENTITY_LCASE%%");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_%%ENTITY_LCASE%%($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAME%%ENTITY_UCASE%%EXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAME%%ENTITY_UCASE%%EXTENSION~', ff($strTableName%%ENTITY_UCASE%%Extension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_%%ENTITY_LCASE%%($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableName%%ENTITY_UCASE%% = getTableNameEntity("%%ENTITY_LCASE%%", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAME%%ENTITY_UCASE%%~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAME%%ENTITY_UCASE%%~', ff($strTableName%%ENTITY_UCASE%%), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAME%%ENTITY_UCASE%%~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "%%ENTITY_UCASE%%", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_%%ENTITY_LCASE%%($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableName%%ENTITY_UCASE%% = getTableNameEntity("%%ENTITY_LCASE%%", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAME%%ENTITY_UCASE%%~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAME%%ENTITY_UCASE%%~', ff($strTableName%%ENTITY_UCASE%%), $strSQL);
...
function fetchValue_%%ENTITY_LCASE%%($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableName%%ENTITY_UCASE%% = getTableNameEntity("%%ENTITY_LCASE%%", false);	

	$strWhere = buildWhere_%%ENTITY_LCASE%%($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAME%%ENTITY_UCASE%%~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAME%%ENTITY_UCASE%%~', ff($strTableName%%ENTITY_UCASE%%), $strSQL);
...
function buildWhere_%%ENTITY_LCASE%%($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_%%ENTITY_LCASE%%($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "%%ENTITY_UCASE%%");
}