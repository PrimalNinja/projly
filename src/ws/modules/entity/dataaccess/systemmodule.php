...
function add_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemmodule($objConn_a);
	return addbulk_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "systemmodule");
	$strCode = "SYSTEMMODULE";
	$strDescription = "SYSTEMMODULE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gdb7a73ca-38b3-43bf-9d17-a599f23507e5";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMMODULE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULE", $strResult, $strJSONData);
...
function delete_systemmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE = getTableNameEntity("systemmodule", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE~', ff($strTableNameSYSTEMMODULE), $strSQL);
...
function deleteExtension_systemmodule($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULEExtension = getTableNameEntityExtension("systemmodule");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemmodule($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMMODULEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULEEXTENSION~', ff($strTableNameSYSTEMMODULEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemmodule($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE = getTableNameEntity("systemmodule", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMMODULE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMMODULE~', ff($strTableNameSYSTEMMODULE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMMODULE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMMODULE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemmodule($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMMODULE = getTableNameEntity("systemmodule", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE~', ff($strTableNameSYSTEMMODULE), $strSQL);
...
function fetchValue_systemmodule($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMMODULE = getTableNameEntity("systemmodule", false);	

	$strWhere = buildWhere_systemmodule($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMMODULE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMMODULE~', ff($strTableNameSYSTEMMODULE), $strSQL);
...
function buildWhere_systemmodule($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemmodule($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMMODULE");
}