...
function add_printerpurpose($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_printerpurpose($objConn_a);
	return addbulk_printerpurpose($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_printerpurpose($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePRINTERPURPOSE = getTableNameEntity("printerpurpose", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "printerpurpose");
	$strCode = "PRINTERPURPOSE";
	$strDescription = "PRINTERPURPOSE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g7bcca045-efac-4e15-a5a2-53ba4c7ee19c";
    ...
    $strSQL = "insert into ~TABLENAMEPRINTERPURPOSE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPRINTERPURPOSE~', ff($strTableNamePRINTERPURPOSE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTERPURPOSE", $strResult, $strJSONData);
...
function delete_printerpurpose($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERPURPOSE = getTableNameEntity("printerpurpose", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printerpurpose($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTERPURPOSE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERPURPOSE~', ff($strTableNamePRINTERPURPOSE), $strSQL);
...
function deleteExtension_printerpurpose($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERPURPOSEExtension = getTableNameEntityExtension("printerpurpose");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_printerpurpose($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRINTERPURPOSEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERPURPOSEEXTENSION~', ff($strTableNamePRINTERPURPOSEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_printerpurpose($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERPURPOSE = getTableNameEntity("printerpurpose", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPRINTERPURPOSE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPRINTERPURPOSE~', ff($strTableNamePRINTERPURPOSE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPRINTERPURPOSE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PRINTERPURPOSE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_printerpurpose($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePRINTERPURPOSE = getTableNameEntity("printerpurpose", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPRINTERPURPOSE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERPURPOSE~', ff($strTableNamePRINTERPURPOSE), $strSQL);
...
function fetchValue_printerpurpose($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRINTERPURPOSE = getTableNameEntity("printerpurpose", false);	

	$strWhere = buildWhere_printerpurpose($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPRINTERPURPOSE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRINTERPURPOSE~', ff($strTableNamePRINTERPURPOSE), $strSQL);
...
function buildWhere_printerpurpose($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_printerpurpose($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PRINTERPURPOSE");
}