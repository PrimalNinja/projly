...
function add_entity($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_entity($objConn_a);
	return addbulk_entity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_entity($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "entity");
	$strCode = "ENTITY";
	$strDescription = "ENTITY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffe65a2521-c3d0-42fc-8d25-dedd43876fff";
    ...
    $strSQL = "insert into ~TABLENAMEENTITY~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ENTITY", $strResult, $strJSONData);
...
function delete_entity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITY = getTableNameEntity("entity", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITY~', ff($strTableNameENTITY), $strSQL);
...
function deleteExtension_entity($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITYExtension = getTableNameEntityExtension("entity");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_entity($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEENTITYEXTENSION~ where ~WHERE~";
...
function update_entity($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITY = getTableNameEntity("entity", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEENTITY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEENTITY~', ff($strTableNameENTITY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEENTITY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ENTITY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_entity($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameENTITY = getTableNameEntity("entity", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEENTITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITY~', ff($strTableNameENTITY), $strSQL);
...
function fetchValue_entity($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameENTITY = getTableNameEntity("entity", false);	

	$strWhere = buildWhere_entity($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEENTITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEENTITY~', ff($strTableNameENTITY), $strSQL);
...
function buildWhere_entity($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_entity($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ENTITY");
}