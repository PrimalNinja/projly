...
function add_integrationoutboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationoutboundtype($objConn_a);
	return addbulk_integrationoutboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationoutboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "integrationoutboundtype");
	$strCode = "INTEGRATIONOUTBOUNDTYPE";
	$strDescription = "INTEGRATIONOUTBOUNDTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g407d9323-cadb-42f2-ac4d-ae6528520610";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONOUTBOUNDTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONOUTBOUNDTYPE", $strResult, $strJSONData);
...
function delete_integrationoutboundtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDTYPE = getTableNameEntity("integrationoutboundtype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationoutboundtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONOUTBOUNDTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDTYPE~', ff($strTableNameINTEGRATIONOUTBOUNDTYPE), $strSQL);
...
function deleteExtension_integrationoutboundtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDTYPEExtension = getTableNameEntityExtension("integrationoutboundtype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationoutboundtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONOUTBOUNDTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDTYPEEXTENSION~', ff($strTableNameINTEGRATIONOUTBOUNDTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationoutboundtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDTYPE = getTableNameEntity("integrationoutboundtype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONOUTBOUNDTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDTYPE~', ff($strTableNameINTEGRATIONOUTBOUNDTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONOUTBOUNDTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONOUTBOUNDTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationoutboundtype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONOUTBOUNDTYPE = getTableNameEntity("integrationoutboundtype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONOUTBOUNDTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDTYPE~', ff($strTableNameINTEGRATIONOUTBOUNDTYPE), $strSQL);
...
function fetchValue_integrationoutboundtype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDTYPE = getTableNameEntity("integrationoutboundtype", false);	

	$strWhere = buildWhere_integrationoutboundtype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONOUTBOUNDTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDTYPE~', ff($strTableNameINTEGRATIONOUTBOUNDTYPE), $strSQL);
...
function buildWhere_integrationoutboundtype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationoutboundtype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONOUTBOUNDTYPE");
}