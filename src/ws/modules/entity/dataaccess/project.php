...
function add_project($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_project($objConn_a);
	return addbulk_project($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_project($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "project");
	$strCode = "PROJECT";
	$strDescription = "PROJECT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff304902c1-b136-4ea9-b67d-72eecc322697";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECT", $strResult, $strJSONData);
...
function delete_project($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECT = getTableNameEntity("project", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_project($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECT~', ff($strTableNamePROJECT), $strSQL);
...
function deleteExtension_project($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTExtension = getTableNameEntityExtension("project");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_project($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTEXTENSION~', ff($strTableNamePROJECTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_project($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECT = getTableNameEntity("project", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECT~', ff($strTableNamePROJECT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_project($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECT = getTableNameEntity("project", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECT~', ff($strTableNamePROJECT), $strSQL);
...
function fetchValue_project($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECT = getTableNameEntity("project", false);	

	$strWhere = buildWhere_project($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECT~', ff($strTableNamePROJECT), $strSQL);
...
function buildWhere_project($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_project($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECT");
}