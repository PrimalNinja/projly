...
function add_integrationoutboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationoutboundplugin($objConn_a);
	return addbulk_integrationoutboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationoutboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "integrationoutboundplugin");
	$strCode = "INTEGRATIONOUTBOUNDPLUGIN";
	$strDescription = "INTEGRATIONOUTBOUNDPLUGIN";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gdaa1313b-b447-4e21-9dd6-7984dd3b554c";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONOUTBOUNDPLUGIN", $strResult, $strJSONData);
...
function delete_integrationoutboundplugin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDPLUGIN = getTableNameEntity("integrationoutboundplugin", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationoutboundplugin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~', ff($strTableNameINTEGRATIONOUTBOUNDPLUGIN), $strSQL);
...
function deleteExtension_integrationoutboundplugin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDPLUGINExtension = getTableNameEntityExtension("integrationoutboundplugin");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationoutboundplugin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONOUTBOUNDPLUGINEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDPLUGINEXTENSION~', ff($strTableNameINTEGRATIONOUTBOUNDPLUGINExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationoutboundplugin($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDPLUGIN = getTableNameEntity("integrationoutboundplugin", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~', ff($strTableNameINTEGRATIONOUTBOUNDPLUGIN), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONOUTBOUNDPLUGIN", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationoutboundplugin($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONOUTBOUNDPLUGIN = getTableNameEntity("integrationoutboundplugin", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~', ff($strTableNameINTEGRATIONOUTBOUNDPLUGIN), $strSQL);
...
function fetchValue_integrationoutboundplugin($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONOUTBOUNDPLUGIN = getTableNameEntity("integrationoutboundplugin", false);	

	$strWhere = buildWhere_integrationoutboundplugin($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONOUTBOUNDPLUGIN~', ff($strTableNameINTEGRATIONOUTBOUNDPLUGIN), $strSQL);
...
function buildWhere_integrationoutboundplugin($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationoutboundplugin($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONOUTBOUNDPLUGIN");
}