...
function add_producttype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_producttype($objConn_a);
	return addbulk_producttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_producttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "producttype");
	$strCode = "PRODUCTTYPE";
	$strDescription = "PRODUCTTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff2d2d0483-0613-4eef-ad95-4c7e4e7089ca";
    ...
    $strSQL = "insert into ~TABLENAMEPRODUCTTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PRODUCTTYPE", $strResult, $strJSONData);
...
function delete_producttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCTTYPE = getTableNameEntity("producttype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_producttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRODUCTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCTTYPE~', ff($strTableNamePRODUCTTYPE), $strSQL);
...
function deleteExtension_producttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCTTYPEExtension = getTableNameEntityExtension("producttype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_producttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPRODUCTTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCTTYPEEXTENSION~', ff($strTableNamePRODUCTTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_producttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCTTYPE = getTableNameEntity("producttype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPRODUCTTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPRODUCTTYPE~', ff($strTableNamePRODUCTTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPRODUCTTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PRODUCTTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_producttype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePRODUCTTYPE = getTableNameEntity("producttype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPRODUCTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCTTYPE~', ff($strTableNamePRODUCTTYPE), $strSQL);
...
function fetchValue_producttype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePRODUCTTYPE = getTableNameEntity("producttype", false);	

	$strWhere = buildWhere_producttype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPRODUCTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPRODUCTTYPE~', ff($strTableNamePRODUCTTYPE), $strSQL);
...
function buildWhere_producttype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_producttype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PRODUCTTYPE");
}