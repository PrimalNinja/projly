...
function add_clientdashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_clientdashboard_chart($objConn_a);
	return addbulk_clientdashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_clientdashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCLIENTDASHBOARD_CHART = getTableNameEntity("clientdashboard_chart", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "clientdashboard_chart");
	$strCode = "CLIENTDASHBOARD_CHART";
	$strDescription = "CLIENTDASHBOARD_CHART";
    ...
    $strSQL = "insert into ~TABLENAMECLIENTDASHBOARD_CHART~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECLIENTDASHBOARD_CHART~', ff($strTableNameCLIENTDASHBOARD_CHART), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENTDASHBOARD_CHART", $strResult, $strJSONData);
...
function delete_clientdashboard_chart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTDASHBOARD_CHART = getTableNameEntity("clientdashboard_chart", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_clientdashboard_chart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENTDASHBOARD_CHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTDASHBOARD_CHART~', ff($strTableNameCLIENTDASHBOARD_CHART), $strSQL);
...
function deleteExtension_clientdashboard_chart($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTDASHBOARD_CHARTExtension = getTableNameEntityExtension("clientdashboard_chart");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_clientdashboard_chart($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECLIENTDASHBOARD_CHARTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTDASHBOARD_CHARTEXTENSION~', ff($strTableNameCLIENTDASHBOARD_CHARTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_clientdashboard_chart($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTDASHBOARD_CHART = getTableNameEntity("clientdashboard_chart", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECLIENTDASHBOARD_CHART~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECLIENTDASHBOARD_CHART~', ff($strTableNameCLIENTDASHBOARD_CHART), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECLIENTDASHBOARD_CHART~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CLIENTDASHBOARD_CHART", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_clientdashboard_chart($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCLIENTDASHBOARD_CHART = getTableNameEntity("clientdashboard_chart", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECLIENTDASHBOARD_CHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTDASHBOARD_CHART~', ff($strTableNameCLIENTDASHBOARD_CHART), $strSQL);
...
function fetchValue_clientdashboard_chart($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCLIENTDASHBOARD_CHART = getTableNameEntity("clientdashboard_chart", false);	

	$strWhere = buildWhere_clientdashboard_chart($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECLIENTDASHBOARD_CHART~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECLIENTDASHBOARD_CHART~', ff($strTableNameCLIENTDASHBOARD_CHART), $strSQL);
...
function buildWhere_clientdashboard_chart($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_clientdashboard_chart($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CLIENTDASHBOARD_CHART");
}