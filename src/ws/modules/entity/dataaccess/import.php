...
function add_import($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_import($objConn_a);
	return addbulk_import($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_import($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "import");
	$strCode = "IMPORT";
	$strDescription = "IMPORT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gdc026800-9533-4d67-a347-7946a5401f72";
    ...
    $strSQL = "insert into ~TABLENAMEIMPORT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEIMPORT~', ff($strTableNameIMPORT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "IMPORT", $strResult, $strJSONData);
...
function delete_import($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameIMPORT = getTableNameEntity("import", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_import($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEIMPORT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEIMPORT~', ff($strTableNameIMPORT), $strSQL);
...
function deleteExtension_import($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameIMPORTExtension = getTableNameEntityExtension("import");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_import($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEIMPORTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEIMPORTEXTENSION~', ff($strTableNameIMPORTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_import($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameIMPORT = getTableNameEntity("import", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEIMPORT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEIMPORT~', ff($strTableNameIMPORT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEIMPORT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "IMPORT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_import($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameIMPORT = getTableNameEntity("import", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEIMPORT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEIMPORT~', ff($strTableNameIMPORT), $strSQL);
...
function fetchValue_import($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameIMPORT = getTableNameEntity("import", false);	

	$strWhere = buildWhere_import($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEIMPORT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEIMPORT~', ff($strTableNameIMPORT), $strSQL);
...
function buildWhere_import($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_import($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "IMPORT");
}