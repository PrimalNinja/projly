...
function add_roster($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_roster($objConn_a);
	return addbulk_roster($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_roster($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "roster");
	$strCode = "ROSTER";
	$strDescription = "ROSTER";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g7b9e72db-4138-48fc-9ef1-24595d3e3fb9";
    ...
    $strSQL = "insert into ~TABLENAMEROSTER~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTER", $strResult, $strJSONData);
...
function delete_roster($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTER = getTableNameEntity("roster", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_roster($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTER~', ff($strTableNameROSTER), $strSQL);
...
function deleteExtension_roster($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTERExtension = getTableNameEntityExtension("roster");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_roster($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEROSTEREXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTEREXTENSION~', ff($strTableNameROSTERExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_roster($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTER = getTableNameEntity("roster", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEROSTER~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEROSTER~', ff($strTableNameROSTER), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEROSTER~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ROSTER", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_roster($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameROSTER = getTableNameEntity("roster", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEROSTER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTER~', ff($strTableNameROSTER), $strSQL);
...
function fetchValue_roster($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameROSTER = getTableNameEntity("roster", false);	

	$strWhere = buildWhere_roster($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEROSTER~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEROSTER~', ff($strTableNameROSTER), $strSQL);
...
function buildWhere_roster($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_roster($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ROSTER");
}