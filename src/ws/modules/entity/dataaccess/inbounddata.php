...
function add_inbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_inbounddata($objConn_a);
	return addbulk_inbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_inbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g6a1d8868-7b8d-4f1f-90be-b80fde55ca43";
    ...
    $strSQL = "insert into ~TABLENAMEINBOUNDDATA~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INBOUNDDATA", $strResult, $strJSONData);
...
function delete_inbounddata($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUNDDATA = getTableNameEntity("inbounddata", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_inbounddata($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINBOUNDDATA~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUNDDATA~', ff($strTableNameINBOUNDDATA), $strSQL);
...
function deleteExtension_inbounddata($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUNDDATAExtension = getTableNameEntityExtension("inbounddata");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_inbounddata($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINBOUNDDATAEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUNDDATAEXTENSION~', ff($strTableNameINBOUNDDATAExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_inbounddata($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUNDDATA = getTableNameEntity("inbounddata", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINBOUNDDATA~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINBOUNDDATA~', ff($strTableNameINBOUNDDATA), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINBOUNDDATA~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INBOUNDDATA", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_inbounddata($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINBOUNDDATA = getTableNameEntity("inbounddata", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINBOUNDDATA~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUNDDATA~', ff($strTableNameINBOUNDDATA), $strSQL);
...
function fetchValue_inbounddata($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINBOUNDDATA = getTableNameEntity("inbounddata", false);	

	$strWhere = buildWhere_inbounddata($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINBOUNDDATA~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINBOUNDDATA~', ff($strTableNameINBOUNDDATA), $strSQL);
...
function buildWhere_inbounddata($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_inbounddata($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INBOUNDDATA");
}