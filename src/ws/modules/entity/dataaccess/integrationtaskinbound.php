...
function add_integrationtaskinbound($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_integrationtaskinbound($objConn_a);
	return addbulk_integrationtaskinbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_integrationtaskinbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strCode = "INTEGRATIONTASKINBOUND";
	$strDescription = "INTEGRATIONTASKINBOUND";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ga360982d-60fb-4feb-bae2-35c27318493d";
    ...
    $strSQL = "insert into ~TABLENAMEINTEGRATIONTASKINBOUND~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONTASKINBOUND", $strResult, $strJSONData);
...
function delete_integrationtaskinbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKINBOUND = getTableNameEntity("integrationtaskinbound", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationtaskinbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONTASKINBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKINBOUND~', ff($strTableNameINTEGRATIONTASKINBOUND), $strSQL);
...
function deleteExtension_integrationtaskinbound($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKINBOUNDExtension = getTableNameEntityExtension("integrationtaskinbound");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_integrationtaskinbound($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEINTEGRATIONTASKINBOUNDEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKINBOUNDEXTENSION~', ff($strTableNameINTEGRATIONTASKINBOUNDExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_integrationtaskinbound($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKINBOUND = getTableNameEntity("integrationtaskinbound", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEINTEGRATIONTASKINBOUND~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKINBOUND~', ff($strTableNameINTEGRATIONTASKINBOUND), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEINTEGRATIONTASKINBOUND~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "INTEGRATIONTASKINBOUND", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_integrationtaskinbound($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameINTEGRATIONTASKINBOUND = getTableNameEntity("integrationtaskinbound", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEINTEGRATIONTASKINBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKINBOUND~', ff($strTableNameINTEGRATIONTASKINBOUND), $strSQL);
...
function fetchValue_integrationtaskinbound($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameINTEGRATIONTASKINBOUND = getTableNameEntity("integrationtaskinbound", false);	

	$strWhere = buildWhere_integrationtaskinbound($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEINTEGRATIONTASKINBOUND~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEINTEGRATIONTASKINBOUND~', ff($strTableNameINTEGRATIONTASKINBOUND), $strSQL);
...
function buildWhere_integrationtaskinbound($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_integrationtaskinbound($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "INTEGRATIONTASKINBOUND");
}