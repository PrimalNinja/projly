...
function add_transaction($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_transaction($objConn_a);
	return addbulk_transaction($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_transaction($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
    ...
    $strSQL = "insert into ~TABLENAMETRANSACTION~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTION", $strResult, $strJSONData);
...
function delete_transaction($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTION = getTableNameEntity("transaction", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transaction($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTION~', ff($strTableNameTRANSACTION), $strSQL);
...
function deleteExtension_transaction($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTIONExtension = getTableNameEntityExtension("transaction");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_transaction($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETRANSACTIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTIONEXTENSION~', ff($strTableNameTRANSACTIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_transaction($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTION = getTableNameEntity("transaction", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETRANSACTION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETRANSACTION~', ff($strTableNameTRANSACTION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETRANSACTION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TRANSACTION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_transaction($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTRANSACTION = getTableNameEntity("transaction", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETRANSACTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTION~', ff($strTableNameTRANSACTION), $strSQL);
...
function fetchValue_transaction($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTRANSACTION = getTableNameEntity("transaction", false);	

	$strWhere = buildWhere_transaction($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETRANSACTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETRANSACTION~', ff($strTableNameTRANSACTION), $strSQL);
...
function buildWhere_transaction($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_transaction($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TRANSACTION");
}