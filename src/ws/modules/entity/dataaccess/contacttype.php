...
function add_contacttype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_contacttype($objConn_a);
	return addbulk_contacttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_contacttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCONTACTTYPE = getTableNameEntity("contacttype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "contacttype");
	$strCode = "CONTACTTYPE";
	$strDescription = "CONTACTTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g50eaed78-be29-4370-bffc-908e871af2cc";
    ...
    $strSQL = "insert into ~TABLENAMECONTACTTYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECONTACTTYPE~', ff($strTableNameCONTACTTYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CONTACTTYPE", $strResult, $strJSONData);
...
function delete_contacttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTTYPE = getTableNameEntity("contacttype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_contacttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONTACTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTTYPE~', ff($strTableNameCONTACTTYPE), $strSQL);
...
function deleteExtension_contacttype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTTYPEExtension = getTableNameEntityExtension("contacttype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_contacttype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECONTACTTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTTYPEEXTENSION~', ff($strTableNameCONTACTTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_contacttype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTTYPE = getTableNameEntity("contacttype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECONTACTTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECONTACTTYPE~', ff($strTableNameCONTACTTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECONTACTTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CONTACTTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_contacttype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCONTACTTYPE = getTableNameEntity("contacttype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECONTACTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTTYPE~', ff($strTableNameCONTACTTYPE), $strSQL);
...
function fetchValue_contacttype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCONTACTTYPE = getTableNameEntity("contacttype", false);	

	$strWhere = buildWhere_contacttype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECONTACTTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECONTACTTYPE~', ff($strTableNameCONTACTTYPE), $strSQL);
...
function buildWhere_contacttype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_contacttype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CONTACTTYPE");
}