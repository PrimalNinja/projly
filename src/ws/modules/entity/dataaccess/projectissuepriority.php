...
function add_projectissuepriority($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectissuepriority($objConn_a);
	return addbulk_projectissuepriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectissuepriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTISSUEPRIORITY = getTableNameEntity("projectissuepriority", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectissuepriority");
	$strCode = "PROJECTISSUEPRIORITY";
	$strDescription = "PROJECTISSUEPRIORITY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffa6654f60-0bb4-4938-833b-ae2b2dc37e69";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTISSUEPRIORITY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTISSUEPRIORITY~', ff($strTableNamePROJECTISSUEPRIORITY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUEPRIORITY", $strResult, $strJSONData);
...
function delete_projectissuepriority($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUEPRIORITY = getTableNameEntity("projectissuepriority", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissuepriority($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUEPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUEPRIORITY~', ff($strTableNamePROJECTISSUEPRIORITY), $strSQL);
...
function deleteExtension_projectissuepriority($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUEPRIORITYExtension = getTableNameEntityExtension("projectissuepriority");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectissuepriority($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTISSUEPRIORITYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUEPRIORITYEXTENSION~', ff($strTableNamePROJECTISSUEPRIORITYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectissuepriority($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUEPRIORITY = getTableNameEntity("projectissuepriority", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTISSUEPRIORITY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTISSUEPRIORITY~', ff($strTableNamePROJECTISSUEPRIORITY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTISSUEPRIORITY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTISSUEPRIORITY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectissuepriority($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTISSUEPRIORITY = getTableNameEntity("projectissuepriority", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTISSUEPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUEPRIORITY~', ff($strTableNamePROJECTISSUEPRIORITY), $strSQL);
...
function fetchValue_projectissuepriority($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTISSUEPRIORITY = getTableNameEntity("projectissuepriority", false);	

	$strWhere = buildWhere_projectissuepriority($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTISSUEPRIORITY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTISSUEPRIORITY~', ff($strTableNamePROJECTISSUEPRIORITY), $strSQL);
...
function buildWhere_projectissuepriority($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectissuepriority($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTISSUEPRIORITY");
}