...
function add_projecttaskstatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projecttaskstatus($objConn_a);
	return addbulk_projecttaskstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projecttaskstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "projecttaskstatus");
	$strCode = "PROJECTTASKSTATUS";
	$strDescription = "PROJECTTASKSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffc91cc8e5-3fb9-427c-b346-06fcb98415b8";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTTASKSTATUS~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTASKSTATUS", $strResult, $strJSONData);
...
function delete_projecttaskstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKSTATUS = getTableNameEntity("projecttaskstatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttaskstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTASKSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKSTATUS~', ff($strTableNamePROJECTTASKSTATUS), $strSQL);
...
function deleteExtension_projecttaskstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKSTATUSExtension = getTableNameEntityExtension("projecttaskstatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projecttaskstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTTASKSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKSTATUSEXTENSION~', ff($strTableNamePROJECTTASKSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projecttaskstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKSTATUS = getTableNameEntity("projecttaskstatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTTASKSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTTASKSTATUS~', ff($strTableNamePROJECTTASKSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTTASKSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTTASKSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projecttaskstatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTTASKSTATUS = getTableNameEntity("projecttaskstatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTTASKSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKSTATUS~', ff($strTableNamePROJECTTASKSTATUS), $strSQL);
...
function fetchValue_projecttaskstatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTTASKSTATUS = getTableNameEntity("projecttaskstatus", false);	

	$strWhere = buildWhere_projecttaskstatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTTASKSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTTASKSTATUS~', ff($strTableNamePROJECTTASKSTATUS), $strSQL);
...
function buildWhere_projecttaskstatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projecttaskstatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTTASKSTATUS");
}