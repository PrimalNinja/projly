...
function add_faq($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_faq($objConn_a);
	return addbulk_faq($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_faq($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameFAQ = getTableNameEntity("faq", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "faq");
	$strCode = "FAQ";
	$strDescription = "FAQ";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g29a00f7a-6ca5-40d5-8e68-bd8259559f30";
    ...
    $strSQL = "insert into ~TABLENAMEFAQ~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEFAQ~', ff($strTableNameFAQ), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "FAQ", $strResult, $strJSONData);
...
function delete_faq($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFAQ = getTableNameEntity("faq", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_faq($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFAQ~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFAQ~', ff($strTableNameFAQ), $strSQL);
...
function deleteExtension_faq($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFAQExtension = getTableNameEntityExtension("faq");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_faq($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEFAQEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFAQEXTENSION~', ff($strTableNameFAQExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_faq($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFAQ = getTableNameEntity("faq", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEFAQ~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEFAQ~', ff($strTableNameFAQ), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEFAQ~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "FAQ", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_faq($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameFAQ = getTableNameEntity("faq", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEFAQ~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFAQ~', ff($strTableNameFAQ), $strSQL);
...
function fetchValue_faq($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameFAQ = getTableNameEntity("faq", false);	

	$strWhere = buildWhere_faq($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEFAQ~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEFAQ~', ff($strTableNameFAQ), $strSQL);
...
function buildWhere_faq($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_faq($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "FAQ");
}