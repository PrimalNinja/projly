...
function add_projectstatus($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectstatus($objConn_a);
	return addbulk_projectstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTSTATUS = getTableNameEntity("projectstatus", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectstatus");
	$strCode = "PROJECTSTATUS";
	$strDescription = "PROJECTSTATUS";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff763a3529-f7b3-439e-93c0-25e1c85e6eeb";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTSTATUS~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTSTATUS~', ff($strTableNamePROJECTSTATUS), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTSTATUS", $strResult, $strJSONData);
...
function delete_projectstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTATUS = getTableNameEntity("projectstatus", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTATUS~', ff($strTableNamePROJECTSTATUS), $strSQL);
...
function deleteExtension_projectstatus($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTATUSExtension = getTableNameEntityExtension("projectstatus");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectstatus($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTSTATUSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTATUSEXTENSION~', ff($strTableNamePROJECTSTATUSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectstatus($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTATUS = getTableNameEntity("projectstatus", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTSTATUS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTSTATUS~', ff($strTableNamePROJECTSTATUS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTSTATUS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTSTATUS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectstatus($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTSTATUS = getTableNameEntity("projectstatus", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTATUS~', ff($strTableNamePROJECTSTATUS), $strSQL);
...
function fetchValue_projectstatus($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTSTATUS = getTableNameEntity("projectstatus", false);	

	$strWhere = buildWhere_projectstatus($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTSTATUS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTSTATUS~', ff($strTableNamePROJECTSTATUS), $strSQL);
...
function buildWhere_projectstatus($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectstatus($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTSTATUS");
}