...
function add_projectcostcategory($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectcostcategory($objConn_a);
	return addbulk_projectcostcategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectcostcategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTCOSTCATEGORY = getTableNameEntity("projectcostcategory", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectcostcategory");
	$strCode = "PROJECTCOSTCATEGORY";
	$strDescription = "PROJECTCOSTCATEGORY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffd2895d59-de03-4d21-9489-20ff6ea8081d";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTCOSTCATEGORY~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTCOSTCATEGORY~', ff($strTableNamePROJECTCOSTCATEGORY), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCOSTCATEGORY", $strResult, $strJSONData);
...
function delete_projectcostcategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCOSTCATEGORY = getTableNameEntity("projectcostcategory", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcostcategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCOSTCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCOSTCATEGORY~', ff($strTableNamePROJECTCOSTCATEGORY), $strSQL);
...
function deleteExtension_projectcostcategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCOSTCATEGORYExtension = getTableNameEntityExtension("projectcostcategory");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectcostcategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTCOSTCATEGORYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCOSTCATEGORYEXTENSION~', ff($strTableNamePROJECTCOSTCATEGORYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectcostcategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCOSTCATEGORY = getTableNameEntity("projectcostcategory", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTCOSTCATEGORY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTCOSTCATEGORY~', ff($strTableNamePROJECTCOSTCATEGORY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTCOSTCATEGORY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTCOSTCATEGORY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectcostcategory($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTCOSTCATEGORY = getTableNameEntity("projectcostcategory", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTCOSTCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCOSTCATEGORY~', ff($strTableNamePROJECTCOSTCATEGORY), $strSQL);
...
function fetchValue_projectcostcategory($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTCOSTCATEGORY = getTableNameEntity("projectcostcategory", false);	

	$strWhere = buildWhere_projectcostcategory($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTCOSTCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTCOSTCATEGORY~', ff($strTableNamePROJECTCOSTCATEGORY), $strSQL);
...
function buildWhere_projectcostcategory($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectcostcategory($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTCOSTCATEGORY");
}