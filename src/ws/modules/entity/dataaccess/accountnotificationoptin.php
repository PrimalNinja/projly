...
function add_accountnotificationoptin($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_accountnotificationoptin($objConn_a);
	return addbulk_accountnotificationoptin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_accountnotificationoptin($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameACCOUNTNOTIFICATIONOPTIN = getTableNameEntity("accountnotificationoptin", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "accountnotificationoptin");
	$strCode = "ACCOUNTNOTIFICATIONOPTIN";
	$strDescription = "ACCOUNTNOTIFICATIONOPTIN";
    ...
    $strSQL = "insert into ~TABLENAMEACCOUNTNOTIFICATIONOPTIN~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEACCOUNTNOTIFICATIONOPTIN~', ff($strTableNameACCOUNTNOTIFICATIONOPTIN), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "ACCOUNTNOTIFICATIONOPTIN", $strResult, $strJSONData);
...
function delete_accountnotificationoptin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNTNOTIFICATIONOPTIN = getTableNameEntity("accountnotificationoptin", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_accountnotificationoptin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEACCOUNTNOTIFICATIONOPTIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNTNOTIFICATIONOPTIN~', ff($strTableNameACCOUNTNOTIFICATIONOPTIN), $strSQL);
...
function deleteExtension_accountnotificationoptin($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNTNOTIFICATIONOPTINExtension = getTableNameEntityExtension("accountnotificationoptin");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_accountnotificationoptin($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEACCOUNTNOTIFICATIONOPTINEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNTNOTIFICATIONOPTINEXTENSION~', ff($strTableNameACCOUNTNOTIFICATIONOPTINExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_accountnotificationoptin($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNTNOTIFICATIONOPTIN = getTableNameEntity("accountnotificationoptin", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEACCOUNTNOTIFICATIONOPTIN~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEACCOUNTNOTIFICATIONOPTIN~', ff($strTableNameACCOUNTNOTIFICATIONOPTIN), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEACCOUNTNOTIFICATIONOPTIN~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "ACCOUNTNOTIFICATIONOPTIN", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_accountnotificationoptin($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameACCOUNTNOTIFICATIONOPTIN = getTableNameEntity("accountnotificationoptin", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEACCOUNTNOTIFICATIONOPTIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNTNOTIFICATIONOPTIN~', ff($strTableNameACCOUNTNOTIFICATIONOPTIN), $strSQL);
...
function fetchValue_accountnotificationoptin($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameACCOUNTNOTIFICATIONOPTIN = getTableNameEntity("accountnotificationoptin", false);	

	$strWhere = buildWhere_accountnotificationoptin($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEACCOUNTNOTIFICATIONOPTIN~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEACCOUNTNOTIFICATIONOPTIN~', ff($strTableNameACCOUNTNOTIFICATIONOPTIN), $strSQL);
...
function buildWhere_accountnotificationoptin($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_accountnotificationoptin($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "ACCOUNTNOTIFICATIONOPTIN");
}