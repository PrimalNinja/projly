...
function add_devicelog($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_devicelog($objConn_a);
	return addbulk_devicelog($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_devicelog($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff26af4dd-710e-4510-8153-052ed948e34e";
    ...
    $strSQL = "insert into ~TABLENAMEDEVICELOG~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DEVICELOG", $strResult, $strJSONData);
...
function delete_devicelog($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICELOG = getTableNameEntity("devicelog", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_devicelog($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDEVICELOG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICELOG~', ff($strTableNameDEVICELOG), $strSQL);
...
function deleteExtension_devicelog($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICELOGExtension = getTableNameEntityExtension("devicelog");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_devicelog($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDEVICELOGEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICELOGEXTENSION~', ff($strTableNameDEVICELOGExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_devicelog($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICELOG = getTableNameEntity("devicelog", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDEVICELOG~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDEVICELOG~', ff($strTableNameDEVICELOG), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDEVICELOG~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DEVICELOG", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_devicelog($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDEVICELOG = getTableNameEntity("devicelog", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDEVICELOG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICELOG~', ff($strTableNameDEVICELOG), $strSQL);
...
function fetchValue_devicelog($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDEVICELOG = getTableNameEntity("devicelog", false);	

	$strWhere = buildWhere_devicelog($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDEVICELOG~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDEVICELOG~', ff($strTableNameDEVICELOG), $strSQL);
...
function buildWhere_devicelog($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_devicelog($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DEVICELOG");
}