...
function add_videocategory($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_videocategory($objConn_a);
	return addbulk_videocategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_videocategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "videocategory");
	$strCode = "VIDEOCATEGORY";
	$strDescription = "VIDEOCATEGORY";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gdef18c2e-390f-4abf-a767-77a1c6364eea";
    ...
    $strSQL = "insert into ~TABLENAMEVIDEOCATEGORY~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "VIDEOCATEGORY", $strResult, $strJSONData);
...
function delete_videocategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOCATEGORY = getTableNameEntity("videocategory", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_videocategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEVIDEOCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOCATEGORY~', ff($strTableNameVIDEOCATEGORY), $strSQL);
...
function deleteExtension_videocategory($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOCATEGORYExtension = getTableNameEntityExtension("videocategory");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_videocategory($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEVIDEOCATEGORYEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOCATEGORYEXTENSION~', ff($strTableNameVIDEOCATEGORYExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_videocategory($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOCATEGORY = getTableNameEntity("videocategory", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEVIDEOCATEGORY~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEVIDEOCATEGORY~', ff($strTableNameVIDEOCATEGORY), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEVIDEOCATEGORY~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "VIDEOCATEGORY", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_videocategory($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameVIDEOCATEGORY = getTableNameEntity("videocategory", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEVIDEOCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOCATEGORY~', ff($strTableNameVIDEOCATEGORY), $strSQL);
...
function fetchValue_videocategory($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOCATEGORY = getTableNameEntity("videocategory", false);	

	$strWhere = buildWhere_videocategory($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEVIDEOCATEGORY~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOCATEGORY~', ff($strTableNameVIDEOCATEGORY), $strSQL);
...
function buildWhere_videocategory($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_videocategory($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "VIDEOCATEGORY");
}