...
function add_document($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_document($objConn_a);
	return addbulk_document($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_document($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "document");
	$strCode = "DOCUMENT";
	$strDescription = "DOCUMENT";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffe70fd7f1-401b-4c5c-b096-76f67d466982";
    ...
    $strSQL = "insert into ~TABLENAMEDOCUMENT~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENT", $strResult, $strJSONData);
...
function delete_document($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENT = getTableNameEntity("document", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_document($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENT~', ff($strTableNameDOCUMENT), $strSQL);
...
function deleteExtension_document($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENTExtension = getTableNameEntityExtension("document");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_document($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEDOCUMENTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENTEXTENSION~', ff($strTableNameDOCUMENTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_document($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENT = getTableNameEntity("document", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEDOCUMENT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEDOCUMENT~', ff($strTableNameDOCUMENT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEDOCUMENT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "DOCUMENT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_document($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameDOCUMENT = getTableNameEntity("document", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEDOCUMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENT~', ff($strTableNameDOCUMENT), $strSQL);
...
function fetchValue_document($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameDOCUMENT = getTableNameEntity("document", false);	

	$strWhere = buildWhere_document($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEDOCUMENT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEDOCUMENT~', ff($strTableNameDOCUMENT), $strSQL);
...
function buildWhere_document($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_document($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "DOCUMENT");
}