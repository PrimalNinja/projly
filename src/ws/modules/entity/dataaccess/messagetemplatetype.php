...
function add_messagetemplatetype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_messagetemplatetype($objConn_a);
	return addbulk_messagetemplatetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_messagetemplatetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameMESSAGETEMPLATETYPE = getTableNameEntity("messagetemplatetype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "messagetemplatetype");
	$strCode = "MESSAGETEMPLATETYPE";
	$strDescription = "MESSAGETEMPLATETYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ffc61e4cb1-cdaf-4bce-8fa5-bc6c8bfeec08";
    ...
    $strSQL = "insert into ~TABLENAMEMESSAGETEMPLATETYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATETYPE~', ff($strTableNameMESSAGETEMPLATETYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "MESSAGETEMPLATETYPE", $strResult, $strJSONData);
...
function delete_messagetemplatetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATETYPE = getTableNameEntity("messagetemplatetype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_messagetemplatetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMESSAGETEMPLATETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATETYPE~', ff($strTableNameMESSAGETEMPLATETYPE), $strSQL);
...
function deleteExtension_messagetemplatetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATETYPEExtension = getTableNameEntityExtension("messagetemplatetype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_messagetemplatetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEMESSAGETEMPLATETYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATETYPEEXTENSION~', ff($strTableNameMESSAGETEMPLATETYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_messagetemplatetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATETYPE = getTableNameEntity("messagetemplatetype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEMESSAGETEMPLATETYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATETYPE~', ff($strTableNameMESSAGETEMPLATETYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEMESSAGETEMPLATETYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "MESSAGETEMPLATETYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_messagetemplatetype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameMESSAGETEMPLATETYPE = getTableNameEntity("messagetemplatetype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEMESSAGETEMPLATETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATETYPE~', ff($strTableNameMESSAGETEMPLATETYPE), $strSQL);
...
function fetchValue_messagetemplatetype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameMESSAGETEMPLATETYPE = getTableNameEntity("messagetemplatetype", false);	

	$strWhere = buildWhere_messagetemplatetype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEMESSAGETEMPLATETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEMESSAGETEMPLATETYPE~', ff($strTableNameMESSAGETEMPLATETYPE), $strSQL);
...
function buildWhere_messagetemplatetype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_messagetemplatetype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "MESSAGETEMPLATETYPE");
}