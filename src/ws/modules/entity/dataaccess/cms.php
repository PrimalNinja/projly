...
function add_cms($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_cms($objConn_a);
	return addbulk_cms($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_cms($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameCMS = getTableNameEntity("cms", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "cms");
	$strCode = "CMS";
	$strDescription = "CMS";
    ...
    $strSQL = "insert into ~TABLENAMECMS~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMECMS~', ff($strTableNameCMS), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "CMS", $strResult, $strJSONData);
...
function delete_cms($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCMS = getTableNameEntity("cms", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_cms($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECMS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECMS~', ff($strTableNameCMS), $strSQL);
...
function deleteExtension_cms($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCMSExtension = getTableNameEntityExtension("cms");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_cms($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMECMSEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECMSEXTENSION~', ff($strTableNameCMSExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_cms($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCMS = getTableNameEntity("cms", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMECMS~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMECMS~', ff($strTableNameCMS), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMECMS~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "CMS", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_cms($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameCMS = getTableNameEntity("cms", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMECMS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECMS~', ff($strTableNameCMS), $strSQL);
...
function fetchValue_cms($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameCMS = getTableNameEntity("cms", false);	

	$strWhere = buildWhere_cms($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMECMS~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMECMS~', ff($strTableNameCMS), $strSQL);
...
function buildWhere_cms($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_cms($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "CMS");
}