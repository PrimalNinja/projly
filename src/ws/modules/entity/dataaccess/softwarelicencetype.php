...
function add_softwarelicencetype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_softwarelicencetype($objConn_a);
	return addbulk_softwarelicencetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_softwarelicencetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSOFTWARELICENCETYPE = getTableNameEntity("softwarelicencetype", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "softwarelicencetype");
	$strCode = "SOFTWARELICENCETYPE";
	$strDescription = "SOFTWARELICENCETYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gd4c26ea6-8213-4774-a5cb-9fc1fad50d71";
    ...
    $strSQL = "insert into ~TABLENAMESOFTWARELICENCETYPE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCETYPE~', ff($strTableNameSOFTWARELICENCETYPE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SOFTWARELICENCETYPE", $strResult, $strJSONData);
...
function delete_softwarelicencetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCETYPE = getTableNameEntity("softwarelicencetype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_softwarelicencetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESOFTWARELICENCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCETYPE~', ff($strTableNameSOFTWARELICENCETYPE), $strSQL);
...
function deleteExtension_softwarelicencetype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCETYPEExtension = getTableNameEntityExtension("softwarelicencetype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_softwarelicencetype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESOFTWARELICENCETYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCETYPEEXTENSION~', ff($strTableNameSOFTWARELICENCETYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_softwarelicencetype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCETYPE = getTableNameEntity("softwarelicencetype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESOFTWARELICENCETYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESOFTWARELICENCETYPE~', ff($strTableNameSOFTWARELICENCETYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESOFTWARELICENCETYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SOFTWARELICENCETYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_softwarelicencetype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSOFTWARELICENCETYPE = getTableNameEntity("softwarelicencetype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESOFTWARELICENCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCETYPE~', ff($strTableNameSOFTWARELICENCETYPE), $strSQL);
...
function fetchValue_softwarelicencetype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSOFTWARELICENCETYPE = getTableNameEntity("softwarelicencetype", false);	

	$strWhere = buildWhere_softwarelicencetype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESOFTWARELICENCETYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESOFTWARELICENCETYPE~', ff($strTableNameSOFTWARELICENCETYPE), $strSQL);
...
function buildWhere_softwarelicencetype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_softwarelicencetype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SOFTWARELICENCETYPE");
}