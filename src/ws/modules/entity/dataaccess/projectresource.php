...
function add_projectresource($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_projectresource($objConn_a);
	return addbulk_projectresource($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_projectresource($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNamePROJECTRESOURCE = getTableNameEntity("projectresource", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "projectresource");
	$strCode = "PROJECTRESOURCE";
	$strDescription = "PROJECTRESOURCE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff31dde036-5ea2-4d57-9b43-704671d5ae78";
    ...
    $strSQL = "insert into ~TABLENAMEPROJECTRESOURCE~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCE~', ff($strTableNamePROJECTRESOURCE), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTRESOURCE", $strResult, $strJSONData);
...
function delete_projectresource($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCE = getTableNameEntity("projectresource", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectresource($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTRESOURCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCE~', ff($strTableNamePROJECTRESOURCE), $strSQL);
...
function deleteExtension_projectresource($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCEExtension = getTableNameEntityExtension("projectresource");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_projectresource($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEPROJECTRESOURCEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCEEXTENSION~', ff($strTableNamePROJECTRESOURCEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_projectresource($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCE = getTableNameEntity("projectresource", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEPROJECTRESOURCE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEPROJECTRESOURCE~', ff($strTableNamePROJECTRESOURCE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEPROJECTRESOURCE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "PROJECTRESOURCE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_projectresource($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNamePROJECTRESOURCE = getTableNameEntity("projectresource", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEPROJECTRESOURCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCE~', ff($strTableNamePROJECTRESOURCE), $strSQL);
...
function fetchValue_projectresource($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNamePROJECTRESOURCE = getTableNameEntity("projectresource", false);	

	$strWhere = buildWhere_projectresource($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEPROJECTRESOURCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEPROJECTRESOURCE~', ff($strTableNamePROJECTRESOURCE), $strSQL);
...
function buildWhere_projectresource($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_projectresource($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "PROJECTRESOURCE");
}