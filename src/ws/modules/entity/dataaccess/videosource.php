...
function add_videosource($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_videosource($objConn_a);
	return addbulk_videosource($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_videosource($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "videosource");
	$strCode = "VIDEOSOURCE";
	$strDescription = "VIDEOSOURCE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "gfcc4c781-3d1d-4546-a357-157df9eb1d98";
    ...
    $strSQL = "insert into ~TABLENAMEVIDEOSOURCE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "VIDEOSOURCE", $strResult, $strJSONData);
...
function delete_videosource($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOSOURCE = getTableNameEntity("videosource", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_videosource($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEVIDEOSOURCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOSOURCE~', ff($strTableNameVIDEOSOURCE), $strSQL);
...
function deleteExtension_videosource($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOSOURCEExtension = getTableNameEntityExtension("videosource");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_videosource($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEVIDEOSOURCEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOSOURCEEXTENSION~', ff($strTableNameVIDEOSOURCEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_videosource($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOSOURCE = getTableNameEntity("videosource", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEVIDEOSOURCE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEVIDEOSOURCE~', ff($strTableNameVIDEOSOURCE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEVIDEOSOURCE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "VIDEOSOURCE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_videosource($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameVIDEOSOURCE = getTableNameEntity("videosource", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEVIDEOSOURCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOSOURCE~', ff($strTableNameVIDEOSOURCE), $strSQL);
...
function fetchValue_videosource($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameVIDEOSOURCE = getTableNameEntity("videosource", false);	

	$strWhere = buildWhere_videosource($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEVIDEOSOURCE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEVIDEOSOURCE~', ff($strTableNameVIDEOSOURCE), $strSQL);
...
function buildWhere_videosource($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_videosource($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "VIDEOSOURCE");
}