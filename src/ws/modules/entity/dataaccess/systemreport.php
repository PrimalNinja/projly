...
function add_systemreport($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_systemreport($objConn_a);
	return addbulk_systemreport($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_systemreport($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSYSTEMREPORT = getTableNameEntity("systemreport", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "systemreport");
	$strCode = "SYSTEMREPORT";
	$strDescription = "SYSTEMREPORT";
    ...
    $strSQL = "insert into ~TABLENAMESYSTEMREPORT~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESYSTEMREPORT~', ff($strTableNameSYSTEMREPORT), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMREPORT", $strResult, $strJSONData);
...
function delete_systemreport($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMREPORT = getTableNameEntity("systemreport", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemreport($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMREPORT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMREPORT~', ff($strTableNameSYSTEMREPORT), $strSQL);
...
function deleteExtension_systemreport($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMREPORTExtension = getTableNameEntityExtension("systemreport");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_systemreport($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESYSTEMREPORTEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMREPORTEXTENSION~', ff($strTableNameSYSTEMREPORTExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_systemreport($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMREPORT = getTableNameEntity("systemreport", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESYSTEMREPORT~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESYSTEMREPORT~', ff($strTableNameSYSTEMREPORT), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESYSTEMREPORT~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SYSTEMREPORT", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_systemreport($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSYSTEMREPORT = getTableNameEntity("systemreport", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESYSTEMREPORT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMREPORT~', ff($strTableNameSYSTEMREPORT), $strSQL);
...
function fetchValue_systemreport($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSYSTEMREPORT = getTableNameEntity("systemreport", false);	

	$strWhere = buildWhere_systemreport($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESYSTEMREPORT~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESYSTEMREPORT~', ff($strTableNameSYSTEMREPORT), $strSQL);
...
function buildWhere_systemreport($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_systemreport($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SYSTEMREPORT");
}