...
function add_smquestion($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_smquestion($objConn_a);
	return addbulk_smquestion($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_smquestion($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameSMQUESTION = getTableNameEntity("smquestion", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "smquestion");
	$strCode = "SMQUESTION";
	$strDescription = "SMQUESTION";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g8178685c-fbcc-4521-975d-4533c67361d9";
    ...
    $strSQL = "insert into ~TABLENAMESMQUESTION~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMESMQUESTION~', ff($strTableNameSMQUESTION), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "SMQUESTION", $strResult, $strJSONData);
...
function delete_smquestion($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMQUESTION = getTableNameEntity("smquestion", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smquestion($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMQUESTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMQUESTION~', ff($strTableNameSMQUESTION), $strSQL);
...
function deleteExtension_smquestion($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMQUESTIONExtension = getTableNameEntityExtension("smquestion");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_smquestion($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMESMQUESTIONEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMQUESTIONEXTENSION~', ff($strTableNameSMQUESTIONExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_smquestion($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMQUESTION = getTableNameEntity("smquestion", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMESMQUESTION~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMESMQUESTION~', ff($strTableNameSMQUESTION), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMESMQUESTION~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "SMQUESTION", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_smquestion($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameSMQUESTION = getTableNameEntity("smquestion", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMESMQUESTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMQUESTION~', ff($strTableNameSMQUESTION), $strSQL);
...
function fetchValue_smquestion($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameSMQUESTION = getTableNameEntity("smquestion", false);	

	$strWhere = buildWhere_smquestion($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMESMQUESTION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMESMQUESTION~', ff($strTableNameSMQUESTION), $strSQL);
...
function buildWhere_smquestion($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_smquestion($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "SMQUESTION");
}