...
function add_todo($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_todo($objConn_a);
	return addbulk_todo($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_todo($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	$strTableNameTODO = getTableNameEntity("todo", false);	

	$strResult = "";
	
	$strClientID = $_SESSION['server_loggedin_clientid'];
	$strEntityID = getEntityID($objConn_a, "systemform");
	$strDataEntityID = getEntityID($objConn_a, "todo");
	$strCode = "TODO";
	$strDescription = "TODO";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "g3a581000-5d59-4332-b5b1-e24a1c302444";
    ...
    $strSQL = "insert into ~TABLENAMETODO~ (~FIELDS~) values (~VALUES~)";
    $strSQL = str_replace('~FIELDS~', $strSQLFields, $strSQL);
    $strSQL = str_replace('~VALUES~', $strSQLValues, $strSQL);
	$strSQL = str_replace('~TABLENAMETODO~', ff($strTableNameTODO), $strSQL);
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "TODO", $strResult, $strJSONData);
...
function delete_todo($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODO = getTableNameEntity("todo", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_todo($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETODO~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODO~', ff($strTableNameTODO), $strSQL);
...
function deleteExtension_todo($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODOExtension = getTableNameEntityExtension("todo");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_todo($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMETODOEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODOEXTENSION~', ff($strTableNameTODOExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_todo($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODO = getTableNameEntity("todo", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMETODO~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMETODO~', ff($strTableNameTODO), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMETODO~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "TODO", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_todo($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameTODO = getTableNameEntity("todo", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMETODO~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODO~', ff($strTableNameTODO), $strSQL);
...
function fetchValue_todo($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameTODO = getTableNameEntity("todo", false);	

	$strWhere = buildWhere_todo($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMETODO~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMETODO~', ff($strTableNameTODO), $strSQL);
...
function buildWhere_todo($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_todo($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "TODO");
}