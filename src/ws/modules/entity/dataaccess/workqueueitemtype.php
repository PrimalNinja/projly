...
function add_workqueueitemtype($objConn_a, $arrFormFields_a, $arrDBFields_a)
{
	$arrJSONData = getFormTemplate_workqueueitemtype($objConn_a);
	return addbulk_workqueueitemtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData);
}

// bulk add method where template is fetched beforehand
function addbulk_workqueueitemtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $arrJSONData_a)
{
	...
	$strDataEntityID = getEntityID($objConn_a, "workqueueitemtype");
	$strCode = "WORKQUEUEITEMTYPE";
	$strDescription = "WORKQUEUEITEMTYPE";
	...
	foreach ($arrFormFields_a as $arrField)
	{
		...
		$strFormSection = "ff6155f1a0-0a1d-4130-ac46-4a4f95b3e4df";
    ...
    $strSQL = "insert into ~TABLENAMEWORKQUEUEITEMTYPE~ (~FIELDS~) values (~VALUES~)";
	...
	exposeEntityData($objConn_a, "SYSTEMFORM", "WORKQUEUEITEMTYPE", $strResult, $strJSONData);
...
function delete_workqueueitemtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUEITEMTYPE = getTableNameEntity("workqueueitemtype", false);	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_workqueueitemtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEWORKQUEUEITEMTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUEITEMTYPE~', ff($strTableNameWORKQUEUEITEMTYPE), $strSQL);
...
function deleteExtension_workqueueitemtype($objConn_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUEITEMTYPEExtension = getTableNameEntityExtension("workqueueitemtype");	

	dbBeginTrans($objConn_a, __FUNCTION__);

	$strWhere = buildWhere_workqueueitemtype($strID_a, $varCustomWhere_a);

	$strSQL = "delete from ~TABLENAMEWORKQUEUEITEMTYPEEXTENSION~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUEITEMTYPEEXTENSION~', ff($strTableNameWORKQUEUEITEMTYPEExtension), $strSQL);
    $strSQL = str_replace('~WHERE~', $strWhere, $strSQL);	
	dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
...
function update_workqueueitemtype($objConn_a, $arrFormFields_a, $arrDBFields_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUEITEMTYPE = getTableNameEntity("workqueueitemtype", false);	
	...
	if ($blnExpose)
	{
		$strSQL = "select jsondata returnvalue from ~TABLENAMEWORKQUEUEITEMTYPE~ where ~WHERE~";
		$strSQL = str_replace('~TABLENAMEWORKQUEUEITEMTYPE~', ff($strTableNameWORKQUEUEITEMTYPE), $strSQL);    
		$strSQL = str_replace('~WHERE~', $strWhere, $strSQL);
	...
	$strSQL = "update ~TABLENAMEWORKQUEUEITEMTYPE~ set ~UPDATEFIELDS~ where ~WHERE~";
	...
	if ($blnExpose)
	{
		exposeEntityData($objConn_a, "SYSTEMFORM", "WORKQUEUEITEMTYPE", $strID_a, $strJSONData);
	}
	
	return dbEndTrans($objConn_a, __FUNCTION__);
}

function fetch_workqueueitemtype($objConn_a, $strDBFields_a, $strID_a, $varCustomWhere_a, $strOrderBy_a = "")
{
	$strTableNameWORKQUEUEITEMTYPE = getTableNameEntity("workqueueitemtype", false);	
	...
	$strSQL = "select ~DBFIELDS~ from ~TABLENAMEWORKQUEUEITEMTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUEITEMTYPE~', ff($strTableNameWORKQUEUEITEMTYPE), $strSQL);
...
function fetchValue_workqueueitemtype($objConn_a, $strFieldName_a, $strID_a, $varCustomWhere_a)
{
	$strTableNameWORKQUEUEITEMTYPE = getTableNameEntity("workqueueitemtype", false);	

	$strWhere = buildWhere_workqueueitemtype($strID_a, $varCustomWhere_a);

	$strSQL = "select ~FIELDNAME~ returnvalue from ~TABLENAMEWORKQUEUEITEMTYPE~ where ~WHERE~";
	$strSQL = str_replace('~TABLENAMEWORKQUEUEITEMTYPE~', ff($strTableNameWORKQUEUEITEMTYPE), $strSQL);
...
function buildWhere_workqueueitemtype($strID_a, $varCustomWhere_a)
{
	$strResult = "";
...
function getFormTemplate_workqueueitemtype($objConn_a)
{
	return formTemplateGetFromDBByEntityCode($objConn_a, "WORKQUEUEITEMTYPE");
}