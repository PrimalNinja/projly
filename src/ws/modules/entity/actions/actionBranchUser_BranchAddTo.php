...
function actionBranchUser_BranchAddTo($objConn_a, $strSecurityToken_a, $strDataID_a, $arrParameters_a) 
{	
	$strResult = "";
	
    if (dependencies('security/branchUser_BranchAddTo')) 
	{
		// permission check
		...
		$strResult = branchUser_BranchAddTo($objConn_a, $strClientID, $strFormEntityIDList, $strRelativeID, $strRelationship);
		
		if (dbEndTrans($objConn_a, __FUNCTION__)) {

			$strResult = createJSONResponse($strDataID_a, RESPONSE_OK, 'User added.', array());
		} 
		else 
		{
			...
			{
				$strDescription = "Error adding user to branch.";
			}
