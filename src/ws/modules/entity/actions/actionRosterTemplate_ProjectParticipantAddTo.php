...
function actionRosterTemplate_ProjectParticipantAddTo($objConn_a, $strSecurityToken_a, $strDataID_a, $arrParameters_a) 
{	
	$strResult = "";
	
        
    if (dependencies('projly/rosterTemplate_projectParticipantAddTo')) 
	{
		// permission check
		...
		$strResult = rosterTemplate_projectParticipantAddTo($objConn_a, $strClientID, $strFormEntityIDList, $strRelativeID, $strRelationship);
		
		if (dbEndTrans($objConn_a, __FUNCTION__)) {

			$strResult = createJSONResponse($strDataID_a, RESPONSE_OK, 'Project participant added.', array());
		} 
		else 
		{
			...
			{
				$strDescription = "Error adding project participant to roster template.";
			}
