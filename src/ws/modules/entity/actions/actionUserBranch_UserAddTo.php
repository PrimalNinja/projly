...
function actionUserBranch_UserAddTo($objConn_a, $strSecurityToken_a, $strDataID_a, $arrParameters_a) 
{	
	$strResult = "";
	
    if (dependencies('security/userBranch_UserAddTo')) 
	{
		// permission check
		...
		$strResult = userBranch_UserAddTo($objConn_a, $strClientID, $strFormEntityIDList, $strRelativeID, $strRelationship);
		
		if (dbEndTrans($objConn_a, __FUNCTION__)) {

			$strResult = createJSONResponse($strDataID_a, RESPONSE_OK, 'Branch added.', array());
		} 
		else 
		{
			...
			{
				$strDescription = "Error adding branch to user.";
			}
