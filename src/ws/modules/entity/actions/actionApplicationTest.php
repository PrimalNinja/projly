<?php
function actionApplicationTest($objConn_a, $strSecurityToken_a, $strDataID_a, $arrParameters_a)
{	
    ...
    if ($arrRow = dbReadRecord($objResult))
    {
        ...
        if (strlen($strDocumentID) > 0)
        {
            ...
            if ($arrDocRow = dbReadRecord($objResult2))
            {
                ...
                $strResponseMessage = ""; //"Application (" . $strApplicationDescription . ") run test!";
            }
            else
            {
                $strResponseMessage = "Application (" . $strApplicationDescription . ") JS Logic document file not found. Build your application first.";
                $blnResult = false;
            }
        ...
        {
            $strResponseMessage = "Application (" . $strApplicationDescription . ") JS Logic document not found.";
            $blnResult = false;
        }  
