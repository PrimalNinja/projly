...
function profileDeactivate($objConn_a, $strClientID_a, $strProfileID_a)
{
	...
	$arrJSONData = formValueUpdateBySectionCodeFieldCode($arrJSONData, "f9a0f134f-54b8-4e11-81b3-02d4f51dadd0", "ISENABLED", 'N');
	
	$strJSONData = json_encode($arrJSONData);

	$strSQL = "update ~TABLENAMEPROFILE~ set jsondata = '~JSONDATA~', is_enabled = 'N', modifyuser = '~MODIFYUSER~', modifydatetime = '~MODIFYDATETIME~' where client_id = ~CLIENTID~ and id = ~PROFILEID~";
	$strSQL = str_replace('~TABLENAMEPROFILE~', ff($strTableNameProfile), $strSQL);
	$strSQL = str_replace('~CLIENTID~', ff($strClientID_a), $strSQL);
	$strSQL = str_replace('~PROFILEID~', ff($strProfileID_a), $strSQL);
