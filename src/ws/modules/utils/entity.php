...
//				transferEntityDataRowCoreValues($objConn_a, $strFormEntityCode_a, $strID_a, $strCodeSource_a, $arrJSONData_a)
//
//			entity retrieval:
//
...
//			general utils:
//
//				canSearchField($arrFields_a, $strSearchableField_a);
...
//				exposeEntityData($objConn_a, $strEntityCode_a, $strFormEntityCode_a, $strRowID_a, $strJSONData_a)
//				exposeEntityDataAll($objConn_a, $strEntityCode_a, $strFormEntityCode_a, $strJSONData_a, $intLimit_a = 0)
//				exposeEntityDataDeferred($objConn_a, $strClientID_a, $strEntityCode_a, $strFormEntityCode_a, $strJSONData_a)
//				filterSearcheableFields($arrFields_a, $arrSearchableFields_a)
...
//				updateForeignKey($objConn_a, $strEntityCode_a, $strSectionCode_a, $strFieldCode_a, $strOldID_a, $strNewID_a)
//
...
function insertEntity($objConn_a, $strClientID_a, $arrJSONDataFrom_a, $strSectionFrom_a, $strEntityCode_a, $strSectionTo_a, $arrJSONSetFields_a)
{
	...
	{
		$strFieldName = $arrJSONSetFields_a[$intI]['fieldname'];
		$strFieldValue = $arrJSONSetFields_a[$intI]['value'];
...
function transferEntityData($objConn_a, $strEntityCode_a, $strFormEntityCode_a, $strClientID_a, $strID_a, $strJSONData_a)
{
	...
	if (is_array($arrJSON))
	{
		...
		// {
			// $strSearchableFields .= ",";
		// }
		// $strSearchableFields .= "code,description";
		...
		if (strlen($strSearchableFields) > 0)
		{
			...
			$strCodeSource = transferEntityDataRow($objConn_a, $strFormEntityCode_a, $strID, $arrJSONData, $arrSearchableFields, $blnIsExtended);
			transferEntityDataRowCoreValues($objConn_a, $strFormEntityCode_a, $strID, $strCodeSource, $arrJSONData);
		}
	}
}
...
// also try identify the source we want to use of the code and description and return it
...
//logDebug(print_r($arrJSONData_a, true), '');
		...
		foreach ($arrJSONData_a as $keySection => $arrSection)
		{
			...
			if (($strSectionType == "INTERNALUSEFORMHEADER") || ($strSectionType == "INTERNALUSE") || ($strSectionType == "DATA"))
			{
				...
				foreach ($arrFields as $keyField => $arrField)
				{
                    ...
                    $strFieldSearchable = $arrField['p_searchable'];
					
					if (strlen($strResult) == 0)
					{
						// identify and return the first sectioncode for the first occurance of the code field as the codesource
						if ($strFieldCode == 'CODE')
						{
							$strResult = $strSectionCode;
						}
					}
	...
	//debug($strField . ":" . $strTemp);
					if (in_array($strField, $arrTransferFields_a) == TRUE && toBoolean($strFieldSearchable))
					{
					...
					{
						if (in_array($strIDField, $arrTransferFields_a) == TRUE && toBoolean($strFieldSearchable))
						{
							if (strlen($strSetValues) > 0)
							{
								$strSetValues .= ", ";
...
function transferEntityDataRowCoreValues($objConn_a, $strFormEntityCode_a, $strID_a, $strCodeSource_a, $arrJSONData_a)
{
	//return; // ENTITYTRANSFERBUG some entities need this line uncommented, some entities creation needs it uncommented

	$strTableNameEntity = getTableNameEntity($strFormEntityCode_a, false);

	$strCode = formValueGetBySectionCodeFieldCode($arrJSONData_a, $strCodeSource_a, 'code');
	$strDescription = formValueGetBySectionCodeFieldCode($arrJSONData_a, $strCodeSource_a, 'description');
	$strIsEnabled = formValueGetBySectionCodeFieldCode($arrJSONData_a, $strCodeSource_a, 'isenabled');

	// get the values from the first data if not yet found
	if (strlen($strCode) == 0) { $strCode = formValueGetBySectionTypeFieldCode($arrJSONData_a, 'DATA', 'code'); }
	if (strlen($strDescription) == 0) { $strDescription = formValueGetBySectionTypeFieldCode($arrJSONData_a, 'DATA', 'description'); }
	if (strlen($strIsEnabled) == 0) { $strIsEnabled = formValueGetBySectionTypeFieldCode($arrJSONData_a, 'DATA', 'isenabled'); }
...
// remove the fields for where clause
function canSearchField($arrFields_a, $strSearchableField_a)
{
	$blnResult = true;
	
	if (strRight($strSearchableField_a, 3) == '_id')
	{
		$blnResult = false;
	}
...
// transfer data from a JSON row for a single row in a table
function exposeEntityData($objConn_a, $strEntityCode_a, $strFormEntityCode_a, $strRowID_a, $strJSONData_a)
{
	$strTableNameEntity = getTableNameEntity($strFormEntityCode_a, false);
	$strTableNameEntityExtension = getTableNameEntityExtension($strFormEntityCode_a);
	$strTableNameSystemForm = getTableNameEntity("systemform", false);

	$arrLatestFormJSON = json_decode($strJSONData_a, true);

	if (is_array($arrLatestFormJSON) && (ffel($strEntityCode_a) == 'systemform'))
	{
		$blnIsExtended = isExtended($objConn_a, $strFormEntityCode_a);
		$strSystemClientID = getSystemClientID($objConn_a);
...
// if ($strFormEntityCode_a == 'LAYOUT')
// {
	// logDebug('jc5:' . print_r($arrSearchableFields, true), '');
// }
		// read json
		...
		if ($arrRow = dbReadRecord($objResult))
		{
			...
			// START BUG FIX
			// NOTE: 1 MARCH 2022 THIS FIXES A BUG WHERE WE WANT TO UPDATE FIELD VALUES IN THE JSON THAT MAY NOT EXIST IN THE STORED
			// VERSION OF THE JSON AGAINST THE RECORD, WE ARE FORCING THE NEW VERSION OF THE JSON NOW WHEN PUBLISHING SO THAT UPDATES
			// ARE MORE LIKELY TO WORK
			
			// need to take the latest version of the published form here, not the currently stored one
			// transfer from arrJSON to arrJSONData
			//compare
			...
			{
//debug(print_r($arrJSONData, true));
//debug(print_r($arrLatestFormJSON, true));
...
function exposeEntityDataAll($objConn_a, $strEntityCode_a, $strFormEntityCode_a, $strJSONData_a, $intLimit_a = 0)
{
	$strTableNameEntity = getTableNameEntity($strFormEntityCode_a, false);
	$strTableNameEntityExtension = getTableNameEntityExtension($strFormEntityCode_a);
	$strTableNameSystemForm = getTableNameEntity("systemform", false);
...
//debug(print_r($arrLatestFormJSON, true));
				...
				$strJSONData = json_encode($arrJSONData);
				$strSQL = "update ~TABLENAMEENTITY~ set jsondata = '~JSONDATA~' where id = ~ID~";
				$strSQL = str_replace('~TABLENAMEENTITY~', ff($strTableNameEntity), $strSQL);
				$strSQL = str_replace('~JSONDATA~', ff($strJSONData), $strSQL);
				...
				if (($strSectionType == "INTERNALUSEFORMHEADER") || ($strSectionType == "INTERNALUSE") || ($strSectionType == "DATA"))
				{
					...
					foreach ($arrFields as $keyField => $arrField)
					{
						...
						if ($blnSearchable)
						{
							...
							{
								if (($strDataType == "d_representation") || ($strDataType == "d_list"))
								{
...
//debug('all fields:' . $strAllDBFormFields);
...
function filterSearcheableFields($arrFields_a, $arrSearchableFields_a)
{
	$arrResult = array();

	foreach ($arrSearchableFields_a as $strSearchableField) 
	{ 
		if (canSearchField($arrFields_a, $strSearchableField))
		{
			$arrResult[] = $strSearchableField;
		}
	}
...
function getTableNameDataForm($strClientID_a, $strFormEntityCode_a, $blnHistory_a)
{
	...
	if ($blnHistory_a)
	{
		if (ENABLE_CLIENTDATABASES == 'TRUE')
		{
			...
			{
				$strResult = DBCLIENTHISTORY_DATABASENAME . ".hdf_~CLIENTID~_tbl~ENTITYCODE~";
			}
			else if (getSessionDB(__FUNCTION__) == "system")
			{
				$strResult = DBSYSTEMHISTORY_DATABASENAME . ".hdf_~CLIENTID~_tbl~ENTITYCODE~";
			}
		}
		else
		{
	...
	{
		$strResult = "df_~CLIENTID~_tbl~ENTITYCODE~";
	}
...
function getTableNameEntity($strFormEntityCode_a, $blnHistory_a)
{
	...
	if ($blnHistory_a)
	{
		if (ENABLE_CLIENTDATABASES == 'TRUE')
		{
			if (getSessionDB(__FUNCTION__) == "client")
			{
				$strResult = DBCLIENTHISTORY_DATABASENAME . ".h_~ENTITYCODE~";
			}
			else if (getSessionDB(__FUNCTION__) == "system")
			{
				$strResult = DBSYSTEMHISTORY_DATABASENAME . ".h_~ENTITYCODE~";
			}
		}
		else
		{
...
function updateForeignKey($objConn_a, $strForeignKey_a, $strOldID_a, $strNewID_a, $strNewDescription_a, $strEntityCode_a, $strSectionCode_a, $strFieldCode_a)
{
    $strTableNameDynamic = getTableNameEntity($strEntityCode_a, false);

	if ((strlen($strSectionCode_a) > 0) && (strlen($strFieldCode_a) > 0))
	{
		// iterate and update the json
		$strSQL = "select id, jsondata from ~TABLENAMEDYNAMIC~ where ~FOREIGNKEY~ = ~OLDID~";
		$strSQL = str_replace('~TABLENAMEDYNAMIC~', ff($strTableNameDynamic), $strSQL);
		$strSQL = str_replace('~FOREIGNKEY~', ff($strForeignKey_a), $strSQL);
		$strSQL = str_replace('~OLDID~', ff($strOldID_a), $strSQL);
		$objResult = dbOpenRecordset($objConn_a, $strSQL, __FUNCTION__);
		...
		while ($arrRow = dbReadRecord($objResult))
		{
			...
			if (strlen($strNewDescription_a) > 0)
			{
				$arrJSONData = formValueAndDescriptionUpdateBySectionCodeFieldCode($arrJSONData, $strSectionCode_a, $strFieldCode_a, $strNewID_a, $strNewDescription_a);
			}
			else
			{
				$arrJSONData = formValueUpdateBySectionCodeFieldCode($arrJSONData, $strSectionCode_a, $strFieldCode_a, $strNewID_a);
			}

			$strJSONData = json_encode($arrJSONData);

			$strSQL = "update ~TABLENAMEDYNAMIC~ set ~FOREIGNKEY~ = ~NEWID~, jsondata = '~JSONDATA~' where id = ~ID~";
			$strSQL = str_replace('~TABLENAMEDYNAMIC~', ff($strTableNameDynamic), $strSQL);
			$strSQL = str_replace('~FOREIGNKEY~', ff($strForeignKey_a), $strSQL);
			$strSQL = str_replace('~JSONDATA~', ff($strJSONData), $strSQL);
			$strSQL = str_replace('~NEWID~', ff($strNewID_a), $strSQL);
			$strSQL = str_replace('~ID~', ff($strID), $strSQL);
			$blnResult = dbExecuteSQL($objConn_a, $strSQL, __FUNCTION__);
		}
	...
	{
		// just update the manually added foreign key
		$strSQL = "update ~TABLENAMEDYNAMIC~ set ~FOREIGNKEY~ = ~NEWID~ where ~FOREIGNKEY~ = ~OLDID~";
		$strSQL = str_replace('~TABLENAMEDYNAMIC~', ff($strTableNameDynamic), $strSQL);
		$strSQL = str_replace('~FOREIGNKEY~', ff($strForeignKey_a), $strSQL);
		$strSQL = str_replace('~NEWID~', ff($strNewID_a), $strSQL);
		$strSQL = str_replace('~OLDID~', ff($strOldID_a), $strSQL);
