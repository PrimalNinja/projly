...
// return
function actionPublicReturn($objConn_a, $strSecurityToken_a, $strDataID_a, $arrParameters_a)
{
    $strResult = "";
	$strTableNameClient = getTableNameEntity("client", false);

//file_put_contents("d:\dev\session.log", " actionCoreReturn.php 1:" . $strFunction . "\n", FILE_APPEND);	// DEBUGSESSION

    if (dependencies('security/deviceAuthenticate,security/userReturn,system/analytics')) 
	{
		// permission check
...
//file_put_contents("d:\dev\session.log", " actionCoreLogout.php 2:" . $strFunction . "\n", FILE_APPEND);	// DEBUGSESSION

        userReturn($objConn_a);
//file_put_contents("d:\dev\session.log", " actionCoreLogout.php 3:" . $strFunction . "\n", FILE_APPEND);	// DEBUGSESSION
