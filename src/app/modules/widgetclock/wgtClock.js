...
function widgetclock_wgtClock(objOS_a, strFormID_a, objParameters_a)
{
	var os = objOS_a;
	var m_objThis = this;
	...
	function showClock()
	{
		var clock = new Clock(m_strFormID + ' .ge-clock');
		os.element(m_strFormID, '.timeshift').click(function (e)
		{
			clock.moveBack(1);
			e.preventDefault();
			return false;
	...
	this.Form_onLoad = function ()
	{
		...
		var intFormWidth = os.element(m_strFormID, '.gb-form').width();
		var intFormHeight = os.element(m_strFormID, '.gb-form').width();
		os.repositionForm(m_strFormID, intViewPortHeight-(intFormHeight*1.1), intViewPortWidth-(intFormWidth*1.1));
