...
function dbBeginTrans($objConn_a, $strCaller_a = "")
{
	...
	if ($g_intNestedTransactions == 0) {
		...
		//logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'nested start transaction: ' . $g_intNestedTransactions, '');
		//logSQL(LOG_DEBUG_SQL_FILE, false, 'nested start transaction: ' . $g_intNestedTransactions, '', 0);
		logSQL($objConn_a->logFile, false, $strCaller_a . ': ' . getSessionDB() . ':' . 'nested start transaction: ' . $g_intNestedTransactions, '', 0);
	}
...
// bind blob
function dbBindB($objStatement_a, $str_a)
{
	$objStatement_a->bindParam($strParam_a, $str_a, PDO::PARAM_LOB);
}

// bind string
function dbBindS($objStatement_a, $str_a)
{
...
function dbBuildOrderBy($arrFields_a, $arrOrder_a, $strAppendFields_a = "")
{
    ...
    if (is_array($arrOrder_a)) {
        foreach ($arrOrder_a as $objField) {
            ...
            $blnIgnore = (InStr($arrFields_a[$strOrderField], 'ignore(') >= 0);
            ...
            if ($blnIgnore == false) {
                if ($strOrderField != null) {
                    ...
                    if ($arrFields_a[$strOrderField] != null) {
                        if ($strAscending == '1') {
                            $strResult .= $arrFields_a[$strOrderField];
                        } else {
                            $strResult .= $arrFields_a[$strOrderField] . ' desc';
                        }
                    }
                ...
                if ($blnIgnore == false) {
                    if (($strFilterField != null) && ($strFilterValue != null)) {
                        if ($blnSoundEx) {
                            ...
                            if (($blnFilterWildcards) && ($blnBoolean == false) && ($blnNumeric == false)) {
...
{
    $strSQL = "insert into core_tblsystem (code, value) values ('~code~', '1')";
	...
	if ($g_intNestedTransactions == 0) {
		try
		{
			if ($g_blnExecuteError == false) {
				...
				logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'commit transaction', '');
				if ($blnResult) {
					$g_blnExecuteError = false;
					//$g_intErrorCode = 0;
					//$g_strErrorDescription = "";
				...
				logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'rollback transaction due to: ' . dbErrorDescription(false), '');
				$blnResult = false;
			}
		} catch (PDOException $e) {
			dbExecuteSQL($objConn_a, "rollback", $strCaller_a);
			logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'rollback transaction (error) due to: ' . dbErrorDescription(false), '');
			$blnResult = false;
		}
	} else {
		$blnResult = !$g_blnExecuteError;
		if ($blnResult) {
			//logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'nested commit: ' . $g_intNestedTransactions, '');
			//logSQL(LOG_DEBUG_SQL_FILE, false, 'nested commit: ' . $g_intNestedTransactions, '', 0);
			logSQL($objConn_a->logFile, false, $strCaller_a . ': ' . getSessionDB() . ':' . 'nested commit: ' . $g_intNestedTransactions, '', 0);
		} else {
			//logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'nested rollback: ' . $g_intNestedTransactions . ' due to: ' . dbErrorDescription(false), '');
			//logSQL(LOG_DEBUG_SQL_FILE, false, 'nested rollback: ' . $g_intNestedTransactions, '', 0);
			logSQL($objConn_a->logFile, false, $strCaller_a . ': ' . getSessionDB() . ':' . 'nested rollback: ' . $g_intNestedTransactions . ' due to: ' . dbErrorDescription(false), '', 0);
		}
	}
...
function dbExecuteSQL($objConn_a, $strSQL_a, $strCaller_a = "")
{
    ...
    logSQL($objConn_a->logFile, false, $strCaller_a . ': ' . getSessionDB() . ':' . $strSQL_a . ' - ' . $intProcessTimeframe . ' seconds', $g_strErrorDescription, $intAffected);
...
function dbExecuteST($objConn_a, $objStatement_a, $strCaller_a = "")
{
    ...
    logSQL($objConn_a->logFile, false, $strCaller_a . ': ' . getSessionDB() . ':' . $g_strLastStatement . ' - ' . $intProcessTimeframe . ' seconds', $g_strErrorDescription, $intAffected);
	$g_intLastAffected = $intAffected;
...
function dbExport($strHostname_a, $strLogin_a, $strPassword_a, $strDatabase_a, $strTarget_a, $blnSchema_a, $blnData_a, $intChunk_a = 0, $intChunkOf_a = 0, $intChunkSize_a = 0)
{
    ...
    foreach ($arrTables as $arrTable) {
        if ($blnSchema_a) {
            $strTemp = $arrTable['create'] . ";\n\n-- ENDOFSTATEMENT\n\n";
            $strTemp = str_replace('utf8mb3', 'utf8', $strTemp);
            $strTemp = str_replace('bigint(20)', 'bigint', $strTemp);
            appendFile($strTarget_a,$strTemp); //";\n\n");
        }
        ...
        if ($blnData_a) {
            ...
            logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'DDD' . $strTable, '');
...
function dbOpenRecordset($objConn_a, $strSQL_a, $strCaller_a = "", $blnFatal_a = true)
{
    ...
    logSQL($objConn_a->logFile, $blnFatal_a, $strCaller_a . ': ' . getSessionDB() . ':' . $strSQL_a, $strError, 0);
    return $objResult;
}
...
function dbReadValue($objConn_a, $strSQL_a, $strCaller_a = "")
{
	...
	logDebug($strCaller_a . ': ' . getSessionDB() . ':' . 'SQLHERE: ' . $strSQL_a, '');
	$objStatement = $objConn_a->PDO->prepare($strSQL_a);
	$objStatement->execute();
...
{
    $strSQL = "select value returnvalue from core_tblsystem where code = '~code~' limit 1";
...
{
    $strSQL = "update core_tblsystem set value = '~value~' where code = '~code~'";
    $strSQL = str_replace('~code~', ff($strKey_a), $strSQL);
